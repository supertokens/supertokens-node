...
import { getProviderConfigForClient, verifyIdTokenFromJWKSEndpoint } from "./utils";
...
export default function NewProvider(input: ProviderInput): TypeProvider {
    let impl: TypeProvider = {
        ...
        getAuthorisationRedirectURL: async function ({ config, redirectURIOnProviderDashboard }) {
            ...
            if (config.clientSecret === undefined || config.forcePKCE) {
            ...
            for (const key in config.authorizationEndpointQueryParams || {}) {
                if (config.authorizationEndpointQueryParams[key] === null) {
                    delete queryParams[key];
                } else {
                    queryParams[key] = config.authorizationEndpointQueryParams[key];
                }
            }
        ...
        exchangeAuthCodeForOAuthTokens: async function ({ config, redirectURIInfo }) {
            const tokenAPIURL = config.tokenEndpoint;
            ...
            for (const key in config.tokenEndpointBodyParams || {}) {
                if (config.tokenEndpointBodyParams[key] === null) {
                    delete accessTokenAPIParams[key];
                } else {
                    accessTokenAPIParams[key] = config.tokenEndpointBodyParams[key];
                }
            }
        ...
        getUserInfo: async function ({ config, oAuthTokens }): Promise<UserInfo> {
            ...
            if (idToken && config.jwksURI !== undefined) {
                ...
                if (config.validateIdTokenPayload !== undefined) {
                    await config.validateIdTokenPayload({
            ...
            if (accessToken && config.userInfoEndpoint !== undefined) {
                const headers: { [key: string]: string } = {
                    Authorization: "Bearer " + accessToken,
                };
                const queryParams: { [key: string]: string } = {};

                for (const key in config.userInfoEndpointHeaders || {}) {
                    if (config.userInfoEndpointHeaders[key] === null) {
                        delete headers[key];
                    } else {
                        headers[key] = config.userInfoEndpointHeaders[key].toString();
                    }
                }

                for (const key in config.userInfoEndpointQueryParams) {
                    if (config.userInfoEndpointQueryParams[key] === null) {
                        delete queryParams[key];
                    } else {
                        queryParams[key] = config.userInfoEndpointQueryParams[key];
                    }
                }

                const userInfoFromAccessToken = (
                    await axios.get(config.userInfoEndpoint, { headers, params: queryParams })
                ).data;
                rawUserInfoFromProvider.fromUserInfoAPI = userInfoFromAccessToken;
            }

            const userInfoResult = getSupertokensUserInfoResultFromRawUserInfo(config, rawUserInfoFromProvider);

            if (config.tenantId !== undefined && config.tenantId !== DEFAULT_TENANT_ID) {
