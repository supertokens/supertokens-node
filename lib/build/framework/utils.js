...
async function inflate(stream) {
    ...
    if (encoding === "gzip" || encoding === "deflate") {
        ...
        try {
            for (var _h = true, stream_2 = __asyncValues(stream), stream_2_1; stream_2_1 = await stream_2.next(), _d = stream_2_1.done, !_d; _h = true) {
                _f = stream_2_1.value;
                _h = false;
                const chunk = _f;
...
function setHeaderForExpressLikeResponse(res, key, value, allowDuplicateKey) {
    ...
    try {
        ...
        throw new Error("Error while setting header with key: " +
            key +
            " and value: " +
            value +
            "\nError: " +
