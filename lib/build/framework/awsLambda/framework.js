...
class AWSRequest extends request_1.BaseRequest {
    constructor(event) {
        ...
        this.getHeaderValue = (key) => {
            ...
            return (0, utils_2.normalizeHeaderValue)((0, utils_1.getFromObjectCaseInsensitive)(key, this.event.headers));
        };
        this.getOriginalURL = () => {
...
class AWSResponse extends response_1.BaseResponse {
    constructor(event) {
        ...
        this.removeHeader = (key) => {
            this.event.supertokens.response.headers = this.event.supertokens.response.headers.filter((header) => header.key.toLowerCase() !== key.toLowerCase());
        };
        this.setCookie = (key, value, domain, secure, httpOnly, expires, path, sameSite) => {
            let serialisedCookie = (0, utils_2.serializeCookieValue)(key, value, domain, secure, httpOnly, expires, path, sameSite);
