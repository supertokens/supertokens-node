...
class Querier {
    ...
    constructor(hosts, rIdToCore) {
        ...
        // path should start with "/"
        this.sendDeleteRequest = async (template, body, params, userContext) => {
            var _a;
            this.invalidateCoreCallCache(userContext);
            const path = this.getPath(template);
            const { body: respBody } = await this.sendRequestHelper(
        ...
        // path should start with "/"
        this.sendGetRequest = async (template, params, userContext) => {
            var _a;
            const path = this.getPath(template);
            const { body: respBody } = await this.sendRequestHelper(
                path,
        ...
        this.sendGetRequestWithResponseHeaders = async (template, params, inpHeaders, userContext) => {
            var _a;
            const path = this.getPath(template);
        ...
        // path should start with "/"
        this.sendPutRequest = async (template, body, params, userContext) => {
            var _a;
            this.invalidateCoreCallCache(userContext);
            const path = this.getPath(template);
            const { body: respBody } = await this.sendRequestHelper(
