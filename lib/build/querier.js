...
exports.doFetch = exports.Querier = void 0;
...
class Querier {
    ...
    constructor(stInstance, hosts, rIdToCore) {
        ...
        this.getAPIVersion = async (userContext) => {
            ...
            processState_1.ProcessState.getInstance().addState(processState_1.PROCESS_STATE.CALLING_SERVICE_IN_GET_API_VERSION);
            ...
            let { body: response } = await this.sendRequestHelper(new normalisedURLPath_1.default("/apiversion"), "GET", async (url) => {
                let headers = {};
                ...
                let response = await (0, exports.doFetch)(url + `?${queryParams}`, {
                    method: "GET",
                    headers,
                });
                return response;
            ...
            let supportedVersion = (0, utils_1.getLargestVersionFromIntersection)(cdiSupportedByServer, version_1.cdiSupported);
            if (supportedVersion === undefined) {
                throw Error("The running SuperTokens core version is not compatible with this NodeJS SDK. Please visit https://supertokens.io/docs/community/compatibility to find the right versions");
            }
        ...
        this.sendPostRequest = async (templatePath, body, userContext) => {
            ...
            const { body: respBody } = await this.sendRequestHelper(path, "POST", async (url) => {
        ...
        this.sendDeleteRequest = async (templatePath, body, params, userContext) => {
            ...
            this.invalidateCoreCallCache(userContext);
            const path = this.getPath(templatePath);
            const { body: respBody } = await this.sendRequestHelper(path, "DELETE", async (url) => {
                let apiVersion = await this.getAPIVersion(userContext);
                ...
                return (0, exports.doFetch)(finalURL.toString(), {
                    method: "DELETE",
                    body: body !== undefined ? JSON.stringify(body) : undefined,
                    headers,
                });
        ...
        this.sendGetRequest = async (templatePath, params, userContext) => {
            var _a;
            const path = this.getPath(templatePath);
            const { body: respBody } = await this.sendRequestHelper(path, "GET", async (url) => {
                var _a, _b, _c, _d;
                let apiVersion = await this.getAPIVersion(userContext);
                ...
                if (((_a = userContext._default) === null || _a === void 0 ? void 0 : _a.globalCacheTag) !== Querier.globalCacheTag) {
                    this.invalidateCoreCallCache(userContext, false);
                }
                (0, logger_1.logDebugMessage)("Checking cache existence");
                if (!Querier.disableCache && uniqueKey in ((_c = (_b = userContext._default) === null || _b === void 0 ? void 0 : _b.coreCallCache) !== null && _c !== void 0 ? _c : {})) {
                ...
                const finalURL = new URL(url);
                const searchParams = new URLSearchParams(Object.entries(params).filter(([_, value]) => value !== undefined));
                finalURL.search = searchParams.toString();
                // Update cache and return
                let response = await (0, exports.doFetch)(finalURL.toString(), {
                    method: "GET",
                    headers,
                });
                ...
                if (response.status === 200 && !Querier.disableCache) {
                    ...
                    userContext._default = Object.assign(Object.assign({}, userContext._default), { coreCallCache: Object.assign(Object.assign({}, (_d = userContext._default) === null || _d === void 0 ? void 0 : _d.coreCallCache), { [uniqueKey]: responseClone }), globalCacheTag: Querier.globalCacheTag });
                }
                return response;
            }, ((_a = this.__hosts) === null || _a === void 0 ? void 0 : _a.length) || 0);
            return respBody;
        ...
        this.sendGetRequestWithResponseHeaders = async (templatePath, params, inpHeaders, userContext) => {
            var _a;
            const path = this.getPath(templatePath);
            return await this.sendRequestHelper(path, "GET", async (url) => {
                let apiVersion = await this.getAPIVersion(userContext);
        ...
        this.sendPutRequest = async (templatePath, body, params, userContext) => {
            ...
            this.invalidateCoreCallCache(userContext);
            const path = this.getPath(templatePath);
            const { body: respBody } = await this.sendRequestHelper(path, "PUT", async (url) => {
                let apiVersion = await this.getAPIVersion(userContext);
                let headers = { "cdi-version": apiVersion, "content-type": "application/json; charset=utf-8" };
                if (Querier.apiKey !== undefined) {
                    headers = Object.assign(Object.assign({}, headers), { "api-key": Querier.apiKey });
        ...
        this.sendPatchRequest = async (templatePath, body, userContext) => {
            ...
            this.invalidateCoreCallCache(userContext);
            const path = this.getPath(templatePath);
            const { body: respBody } = await this.sendRequestHelper(path, "PATCH", async (url) => {
                let apiVersion = await this.getAPIVersion(userContext);
                let headers = { "cdi-version": apiVersion, "content-type": "application/json; charset=utf-8" };
                if (Querier.apiKey !== undefined) {
                    headers = Object.assign(Object.assign({}, headers), { "api-key": Querier.apiKey });
        ...
        this.invalidateCoreCallCache = (userContext, updGlobalCacheTagIfNecessary = true) => {
            var _a;
            if (updGlobalCacheTagIfNecessary && ((_a = userContext._default) === null || _a === void 0 ? void 0 : _a.keepCacheAlive) !== true) {
        ...
        this.sendRequestHelper = async (path, method, requestFunc, numberOfTries, retryInfoMap) => {
            ...
            try {
                processState_1.ProcessState.getInstance().addState(processState_1.PROCESS_STATE.CALLING_SERVICE_IN_REQUEST_HELPER);
                ...
                if ("status" in err && "text" in err) {
                    ...
                    throw new Error("SuperTokens core threw an error for a " +
                        method +
                        " request to path: '" +
    ...
    static getNewInstanceOrThrowError(stInstance, rIdToCore) {
...
const doFetch = async (input, init) => {
    ...
    if (init === undefined) {
        processState_1.ProcessState.getInstance().addState(processState_1.PROCESS_STATE.ADDING_NO_CACHE_HEADER_IN_FETCH);
        init = {
            cache: "no-store",
            redirect: "manual",
        };
