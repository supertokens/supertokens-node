...
exports.getNonPublicConfig = getNonPublicConfig;
...
function getPublicConfig(config) {
    ...
    const publicConfig = Object.fromEntries(configEntries.filter(([key, _]) => !types_1.nonPublicConfigProperties.includes(key)));
    return publicConfig;
}
function getNonPublicConfig(config) {
    // `Entries<T>` will work fine assuming there are no extra properties in `TypeInput`.
    const configEntries = Object.entries(config);
    const publicConfig = Object.fromEntries(configEntries.filter(([key, _]) => types_1.nonPublicConfigProperties.includes(key)));
    return publicConfig;
