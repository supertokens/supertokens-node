...
function normaliseInputAppInfoOrThrowError(appInfo) {
    ...
    const origin = (req, userContext) =>
        __awaiter(this, void 0, void 0, function* () {
            if (typeof appInfo.origin === "string") {
                return new normalisedURLDomain_1.default(appInfo.origin);
            }
            let url = yield appInfo.origin(req, userContext);
            return new normalisedURLDomain_1.default(url);
        });
    const initialAPIDomainType = typeof appInfo.apiDomain;
    if (!["string", "function"].includes(initialAPIDomainType)) {
        throw new Error('Type of origin in supertokens.init can only be either "string" or "function"');
    }
    const apiDomain = (req, userContext) =>
        __awaiter(this, void 0, void 0, function* () {
            let url;
            if (typeof appInfo.apiDomain === "string") {
                return new normalisedURLDomain_1.default(appInfo.apiDomain);
            } else {
                url = yield appInfo.apiDomain(req, userContext);
            }
            return new normalisedURLDomain_1.default(url);
        });
    return {
        ...
        originBasePath:
            appInfo.originBasePath === undefined
                ? new normalisedURLPath_1.default("/auth")
                : new normalisedURLPath_1.default(appInfo.originBasePath),
        apiGatewayPath,
        initialOriginType,
