...
const doFetch = (input, init) => {
    ...
    return cross_fetch_1.default(input, init);
};
...
function getBackwardsCompatibleUserInfo(req, result) {
    ...
    if (doesRequestSupportFDI(req, "1.18")) {
        ...
        let loginMethod = result.user.loginMethods.find((lm) => lm.recipeUserId.getAsString() === result.session.getRecipeUserId().getAsString());
