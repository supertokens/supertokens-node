...
async function doGetRequest(url, queryParams, headers) {
    (0, logger_1.logDebugMessage)(`GET request to ${url}, with query params ${JSON.stringify(queryParams)} and headers ${JSON.stringify(headers)}`);
    ...
    let response = await (0, querier_1.doFetch)(finalURL.toString(), {
        headers: headers,
    });
...
async function doPostRequest(url, params, headers) {
    ...
    (0, logger_1.logDebugMessage)(`POST request to ${url}, with params ${JSON.stringify(params)} and headers ${JSON.stringify(headers)}`);
    const body = new URLSearchParams(params).toString();
    let response = await (0, querier_1.doFetch)(url, {
        method: "POST",
        body,
        headers,
    });
...
async function getOIDCDiscoveryInfo(issuer) {
    ...
    if (oidcInfo.status >= 400) {
        (0, logger_1.logDebugMessage)(`Received response with status ${oidcInfo.status} and body ${oidcInfo.stringResponse}`);
