...
class NextJS {
    ...
    static getAppDirRequestHandler() {
        return customFramework_1.handleAuthAPIRequest();
    }
    static async getSSRSession(cookies) {
        var _a;
        let accessToken = (_a = cookies.find((cookie) => cookie.name === "sAccessToken")) === null || _a === void 0 ? void 0 : _a.value;
        return await customFramework_1.getSessionForSSRUsingAccessToken(accessToken);
    }
    static async withSession(req, handler, options, userContext) {
        return await customFramework_1.withSession(req, handler, options, userContext);
    }
    static async withPreParsedRequestResponse(req, handler) {
        return customFramework_1.withPreParsedRequestResponse(req, handler);
    }
}
