...
exports.withSession = exports.getSSRSession = exports.getAppDirRequestHandler = exports.superTokensNextWrapper = void 0;
...
class NextJS {
    ...
    static async commonSSRSession(baseRequest, options) {
        let baseResponse = new custom_1.CollectingResponse();
        try {
            let session = await session_1.default.getSession(baseRequest, baseResponse, options);
            return {
                session,
            ...
            if (session_1.default.Error.isErrorFromSuperTokens(err)) {
                return {
                    hasToken: err.type !== session_1.default.Error.UNAUTHORISED,
    ...
    static async getSSRSession(cookies, headers, options) {
        ...
        const _a = await NextJS.commonSSRSession(baseRequest, options),
            { baseResponse, nextResponse } = _a,
            result = __rest(_a, ["baseResponse", "nextResponse"]);
        return result;
    }
    static async withSession(req, handler, options) {
        const query = Object.fromEntries(new URL(req.url).searchParams.entries());
        const cookies = Object.fromEntries(req.cookies.getAll().map((cookie) => [cookie.name, cookie.value]));
        let baseRequest = new custom_1.PreParsedRequest({
            method: req.method,
