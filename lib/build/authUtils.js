...
exports.AuthUtils = {
    getErrorStatusResponseWithReason(resp, errorCodeMap, errorStatus) {
        ...
        logger_1.logDebugMessage(`unmapped error status ${resp.status}`);
...
async function getValidSecondaryFactors(factorIds, createsNewLinkForTheSessionUser, sessionUser, session, userContext) {
    if (session === undefined) {
...
async function getPrimarySessionUser(session, tenantId, userContext) {
    logger_1.logDebugMessage(`getPrimarySessionUser called`);
    const accountLinkingInstance = recipe_1.default.getInstance();
    let sessionUser = await _1.getUser(session.getUserId(), userContext);
    if (sessionUser === undefined) {
        throw new error_1.default({
            type: error_1.default.UNAUTHORISED,
            message: "Session user not found",
        ...
        if (shouldDoAccountLinking.shouldAutomaticallyLink) {
            if (shouldDoAccountLinking.shouldRequireVerification && !sessionUser.loginMethods[0].verified) {
                ...
                if ((await session.getClaimValue(emailverification_1.EmailVerificationClaim, userContext)) !== false) {
                    logger_1.logDebugMessage(`getPrimarySessionUser updating emailverification status in session`);
            ...
            if (makeSessionUserPrimaryRes.status === "RECIPE_USER_ID_ALREADY_LINKED_WITH_PRIMARY_USER_ID_ERROR") {
                ...
                sessionUser = await _1.getUser(makeSessionUserPrimaryRes.primaryUserId, userContext);
                if (sessionUser === undefined) {
                    throw new error_1.default({
                        type: error_1.default.UNAUTHORISED,
                        message: "Session user not found",
...
async function checkAuthType(session, accountInfo, inputUser, tenantId, userContext) {
    // TODO: better name
    logger_1.logDebugMessage(`checkAuthType called`);
    let sessionUser = undefined;
    if (session === undefined) {
        logger_1.logDebugMessage(`checkAuthType returning first factor`);
        ...
        if (inputUser !== undefined && inputUser.id === session.getUserId()) {
            logger_1.logDebugMessage(`checkAuthType returning secondary factor, session and input user are the same`);
        ...
        logger_1.logDebugMessage(`checkAuthType loading session user`);
