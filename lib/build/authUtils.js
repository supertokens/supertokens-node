...
exports.AuthUtils = {
     ...
     */
    preAuthChecks: async function ({ authenticatingAccountInfo, tenantId, isSignUp, isVerified, signInVerifiesLoginMethod, authenticatingUser, factorIds, skipSessionUserUpdateInCore, session, shouldTryLinkingWithSessionUser, userContext, }) {
        let validFactorIds;
        ...
        const authTypeInfo = await exports.AuthUtils.checkAuthTypeAndLinkingStatus(session, shouldTryLinkingWithSessionUser, authenticatingAccountInfo, authenticatingUser, skipSessionUserUpdateInCore, userContext);
        ...
        if (authTypeInfo.isFirstFactor) {
            ...
            const validFirstFactors = await exports.AuthUtils.filterOutInvalidFirstFactorsOrThrowIfAllAreInvalid(factorIds, tenantId, session !== undefined, userContext);
            ...
            validFactorIds = await filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid(factorIds, authTypeInfo.inputUserAlreadyLinkedToSessionUser, authTypeInfo.sessionUser, session, userContext);
        }
        if (!isSignUp && authenticatingUser === undefined) {
            throw new Error("This should never happen: preAuthChecks called with isSignUp: false, authenticatingUser: undefined");
        }
        ...
        if (isSignUp) {
            ...
            let verifiedInSessionUser = !authTypeInfo.isFirstFactor &&
                authTypeInfo.sessionUser.loginMethods.some((lm) => lm.verified &&
            ...
            if (!(await recipe_1.default.getInstance().isSignUpAllowed({
            ...
            logger_1.logDebugMessage("preAuthChecks checking if the user is allowed to sign in");
            if (!(await recipe_1.default.getInstance().isSignInAllowed({
     ...
     */
    postAuthChecks: async function ({ authenticatedUser, recipeUserId, isSignUp, factorId, session, req, res, tenantId, userContext, }) {
        logger_1.logDebugMessage(`postAuthChecks called ${session !== undefined ? "with" : "without"} a session to ${isSignUp ? "sign in" : "sign up"} with ${factorId}`);
        const mfaInstance = recipe_2.default.getInstance();
        let respSession = session;
        if (session !== undefined) {
            const authenticatedUserLinkedToSessionUser = authenticatedUser.loginMethods.some((lm) => lm.recipeUserId.getAsString() === session.getRecipeUserId(userContext).getAsString());
            if (authenticatedUserLinkedToSessionUser) {
                ...
                let overwriteSessionDuringSignInUp = recipe_3.default.getInstanceOrThrowError().getNormalisedOverwriteSessionDuringSignInUp(req);
                if (overwriteSessionDuringSignInUp) {
                    respSession = await session_1.default.createNewSession(req, res, tenantId, recipeUserId, {}, {}, userContext);
                    if (mfaInstance !== undefined) {
                        await multifactorauth_1.default.markFactorAsCompleteInSession(respSession, factorId, userContext);
                    }
                }
     ...
     */
    getAuthenticatingUserAndAddToCurrentTenantIfRequired: async ({ recipeId, accountInfo, checkCredentialsOnTenant, tenantId, session, userContext, }) => {
        let i = 0;
        while (i++ < 300) {
            logger_1.logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired called with ${JSON.stringify(accountInfo)}`);
            ...
            logger_1.logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired got ${existingUsers.length} users from the core resp`);
            const usersWithMatchingLoginMethods = existingUsers
                .map((user) => ({
                user,
                loginMethod: user.loginMethods.find((lm) => lm.recipeId === recipeId &&
            ...
            logger_1.logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired got ${usersWithMatchingLoginMethods.length} users with matching login methods`);
            ...
            if (authenticatingUser === undefined && session !== undefined) {
                ...
                if (!sessionUser.isPrimaryUser) {
                    logger_1.logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired session user is non-primary so returning early without checking other tenants`);
                    return undefined;
                }
                const matchingLoginMethodsFromSessionUser = sessionUser.loginMethods.filter((lm) => lm.recipeId === recipeId &&
                    (lm.hasSameEmailAs(accountInfo.email) ||
                        lm.hasSamePhoneNumberAs(accountInfo.phoneNumber) ||
                        lm.hasSameThirdPartyInfoAs(accountInfo.thirdParty)));
                logger_1.logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired session has ${matchingLoginMethodsFromSessionUser.length} matching login methods`);
                ...
                for (const lm of matchingLoginMethodsFromSessionUser) {
                    logger_1.logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired session checking credentials on ${lm.tenantIds[0]}`);
                    if (await checkCredentialsOnTenant(lm.tenantIds[0])) {
                        logger_1.logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired associating user from ${lm.tenantIds[0]} with current tenant`);
                        const associateRes = await multitenancy_1.default.associateUserToTenant(tenantId, lm.recipeUserId, userContext);
                        logger_1.logDebugMessage(`getAuthenticatingUserAndAddToCurrentTenantIfRequired associating returned ${associateRes.status}`);
        ...
        throw new Error("This should never happen: ran out of retries for getAuthenticatingUserAndAddToCurrentTenantIfRequired");
    },
    /**
     ...
     * if the session user should be primary but we couldn't make it primary because of a conflicting primary user.
     */
    checkAuthTypeAndLinkingStatus: async function (session, shouldTryLinkingWithSessionUser, accountInfo, inputUser, skipSessionUserUpdateInCore, userContext) {
        ...
        if (session === undefined) {
            ...
            logger_1.logDebugMessage(`checkAuthTypeAndLinkingStatus returning first factor because there is no session`);
            ...
            if (shouldTryLinkingWithSessionUser === false) {
                logger_1.logDebugMessage(`checkAuthTypeAndLinkingStatus returning first factor because shouldTryLinkingWithSessionUser is false`);
            ...
            if (!utils_3.recipeInitDefinedShouldDoAutomaticAccountLinking(recipe_1.default.getInstance().config)) {
                if (shouldTryLinkingWithSessionUser === true) {
                    throw new Error("Please initialise the account linking recipe and define shouldDoAutomaticAccountLinking to enable MFA");
                }
                else {
                    ...
                    if (recipe_2.default.getInstance() !== undefined) {
                        ...
                        logger_1.logDebugMessage(`checkAuthTypeAndLinkingStatus (legacy behaviour) returning first factor because MFA is not initialised and shouldDoAutomaticAccountLinking is not defined`);
                        return { status: "OK", isFirstFactor: true };
                    }
                }
            }
            ...
            if (inputUser !== undefined && inputUser.id === session.getUserId()) {
                logger_1.logDebugMessage(`checkAuthTypeAndLinkingStatus returning secondary factor, session and input user are the same`);
            ...
            logger_1.logDebugMessage(`checkAuthTypeAndLinkingStatus loading session user, ${inputUser === null || inputUser === void 0 ? void 0 : inputUser.id} === ${session.getUserId()}`);
            // We have to load the session user in order to get the account linking info
            const sessionUserResult = await exports.AuthUtils.tryAndMakeSessionUserIntoAPrimaryUser(session, skipSessionUserUpdateInCore, userContext);
            if (sessionUserResult.status === "SHOULD_AUTOMATICALLY_LINK_FALSE") {
                if (shouldTryLinkingWithSessionUser === true) {
                    ...
                    throw new _1.Error({
                        message: "shouldDoAutomaticAccountLinking returned false when making the session user primary but shouldTryLinkingWithSessionUser is true",
            ...
            const shouldLink = await recipe_1.default.getInstance().config.shouldDoAutomaticAccountLinking(accountInfo, sessionUser, session, session.getTenantId(), userContext);
            logger_1.logDebugMessage(`checkAuthTypeAndLinkingStatus session user <-> input user shouldDoAutomaticAccountLinking returned ${JSON.stringify(shouldLink)}`);
     ...
     * if the session user should be primary but we couldn't make it primary because of a conflicting primary user.
     */
    linkToSessionIfRequiredElseCreatePrimaryUserIdOrLinkByAccountInfo: async function ({ tenantId, inputUser, recipeUserId, session, shouldTryLinkingWithSessionUser, userContext, }) {
        logger_1.logDebugMessage("linkToSessionIfRequiredElseCreatePrimaryUserIdOrLinkByAccountInfo called");
        const retry = () => {
            logger_1.logDebugMessage("linkToSessionIfRequiredElseCreatePrimaryUserIdOrLinkByAccountInfo retrying....");
        ...
        const authLoginMethod = inputUser.loginMethods.find((lm) => lm.recipeUserId.getAsString() === recipeUserId.getAsString());
        if (authLoginMethod === undefined) {
            throw new Error("This should never happen: the recipeUserId and user is inconsistent in createPrimaryUserIdOrLinkByAccountInfo params");
        }
        const authTypeRes = await exports.AuthUtils.checkAuthTypeAndLinkingStatus(session, shouldTryLinkingWithSessionUser, authLoginMethod, inputUser, false, userContext);
        if (authTypeRes.status !== "OK") {
            return authTypeRes;
        }
        if (authTypeRes.isFirstFactor) {
            if (!utils_3.recipeInitDefinedShouldDoAutomaticAccountLinking(recipe_1.default.getInstance().config)) {
                logger_1.logDebugMessage("linkToSessionIfRequiredElseCreatePrimaryUserIdOrLinkByAccountInfo skipping link by account info because this is a first factor auth and the app hasn't defined shouldDoAutomaticAccountLinking");
                return { status: "OK", user: inputUser };
            }
            logger_1.logDebugMessage("linkToSessionIfRequiredElseCreatePrimaryUserIdOrLinkByAccountInfo trying to link by account info because this is a first factor auth");
            ...
            const linkRes = await recipe_1.default.getInstance().tryLinkingByAccountInfoOrCreatePrimaryUser({
                inputUser: inputUser,
                session,
                tenantId,
                userContext,
        ...
        logger_1.logDebugMessage("linkToSessionIfRequiredElseCreatePrimaryUserIdOrLinkByAccountInfo trying to link by session info");
    ...
    tryAndMakeSessionUserIntoAPrimaryUser: async function (session, skipSessionUserUpdateInCore, userContext) {
        logger_1.logDebugMessage(`tryAndMakeSessionUserIntoAPrimaryUser called`);
        const sessionUser = await _1.getUser(session.getUserId(), userContext);
        if (sessionUser === undefined) {
            throw new error_1.default({
                type: error_1.default.UNAUTHORISED,
                message: "Session user not found",
        ...
        if (sessionUser.isPrimaryUser) {
            logger_1.logDebugMessage(`tryAndMakeSessionUserIntoAPrimaryUser session user already primary`);
            ...
            logger_1.logDebugMessage(`tryAndMakeSessionUserIntoAPrimaryUser not primary user yet`);
            ...
            const shouldDoAccountLinking = await recipe_1.default.getInstance().config.shouldDoAutomaticAccountLinking(sessionUser.loginMethods[0], undefined, session, session.getTenantId(userContext), userContext);
            logger_1.logDebugMessage(`tryAndMakeSessionUserIntoAPrimaryUser shouldDoAccountLinking: ${JSON.stringify(shouldDoAccountLinking)}`);
            if (shouldDoAccountLinking.shouldAutomaticallyLink) {
                ...
                if (shouldDoAccountLinking.shouldRequireVerification && !sessionUser.loginMethods[0].verified) {
                    ...
                    if ((await session.getClaimValue(emailverification_1.EmailVerificationClaim, userContext)) !== false) {
                        logger_1.logDebugMessage(`tryAndMakeSessionUserIntoAPrimaryUser updating emailverification status in session`);
                    ...
                    logger_1.logDebugMessage(`tryAndMakeSessionUserIntoAPrimaryUser throwing validation error`);
                    ...
                    await session.assertClaims([emailverification_1.EmailVerificationClaim.validators.isVerified()], userContext);
                    throw new Error("This should never happen: email verification claim validator passed after setting value to false");
                }
                const createPrimaryUserRes = await recipe_1.default.getInstance().recipeInterfaceImpl.createPrimaryUser({
                    recipeUserId: sessionUser.loginMethods[0].recipeUserId,
                    userContext,
                });
                logger_1.logDebugMessage(`tryAndMakeSessionUserIntoAPrimaryUser createPrimaryUser returned ${createPrimaryUserRes.status}`);
     ...
     */
    tryLinkingBySession: async function ({ linkingToSessionUserRequiresVerification, authLoginMethod, authenticatedUser, sessionUser, userContext, }) {
        ...
        const sessionUserHasVerifiedAccountInfo = sessionUser.loginMethods.some((lm) => (lm.hasSameEmailAs(authLoginMethod.email) || lm.hasSamePhoneNumberAs(authLoginMethod.phoneNumber)) &&
            lm.verified);
        ...
        let linkAccountsResult = await recipe_1.default.getInstance().recipeInterfaceImpl.linkAccounts({
        ...
        if (linkAccountsResult.status === "OK") {
            ...
            logger_1.logDebugMessage("tryLinkingBySession linking to session user failed because of a race condition - input user linked to another user");
            ...
            logger_1.logDebugMessage("tryLinkingBySession linking to session user failed because of a race condition - INPUT_USER_IS_NOT_A_PRIMARY_USER, should retry");
            ...
            logger_1.logDebugMessage("tryLinkingBySession linking to session user failed because of a race condition - input user has another primary user it can be linked to");
    ...
    loadSessionInAuthAPIIfNeeded: async function (req, res, shouldTryLinkingWithSessionUser, userContext) {
        const overwriteSessionDuringSignInUp = recipe_3.default.getInstanceOrThrowError().getNormalisedOverwriteSessionDuringSignInUp(req);
        if (shouldTryLinkingWithSessionUser !== false) {
            logger_1.logDebugMessage("loadSessionInAuthAPIIfNeeded: loading session because shouldTryLinkingWithSessionUser is not set to false so we may want to link later");
            return await session_1.default.getSession(req, res, {
        ...
        if (overwriteSessionDuringSignInUp === false) {
            logger_1.logDebugMessage("loadSessionInAuthAPIIfNeeded: loading session in optional mode because overwriteSessionDuringSignInUp is false so if it is not found we will skip session creation");
            return await session_1.default.getSession(req, res, {
                sessionRequired: false,
                overrideGlobalClaimValidators: () => [],
            }, userContext);
        }
        logger_1.logDebugMessage("loadSessionInAuthAPIIfNeeded: skipping session loading because we are not linking and we would overwrite it anyway");
        return undefined;
    },
};
async function filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid(factorIds, inputUserAlreadyLinkedToSessionUser, sessionUser, session, userContext) {
    if (session === undefined) {
        throw new Error("This should never happen: filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid called without a session");
    }
    if (sessionUser === undefined) {
        throw new Error("This should never happen: filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid called without a sessionUser");
    }
    logger_1.logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid called for ${factorIds.join(", ")}`);
    const mfaInstance = recipe_2.default.getInstance();
    if (mfaInstance !== undefined) {
        if (!inputUserAlreadyLinkedToSessionUser) {
            ...
            const memoizedAllowedToSetupFactorInput = {
                ...
                get mfaRequirementsForAuth() {
                    ...
                    mfaInfoProm = utils_1.updateAndGetMFARelatedInfoInSession({
                        session,
                        userContext,
                    });
                    return mfaInfoProm.then((res) => res.mfaRequirementsForAuth);
            ...
            logger_1.logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid checking if linking is allowed by the mfa recipe`);
            ...
            // then we do not want to include a link in the email.
            for (const id of factorIds) {
                logger_1.logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid checking assertAllowedToSetupFactorElseThrowInvalidClaimError`);
                try {
                    memoizedAllowedToSetupFactorInput.factorId = id;
                    await mfaInstance.recipeInterfaceImpl.assertAllowedToSetupFactorElseThrowInvalidClaimError(memoizedAllowedToSetupFactorInput);
                    logger_1.logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid ${id} valid because assertAllowedToSetupFactorElseThrowInvalidClaimError passed`);
                    // we add it to the valid factor ids list since it is either already set up or allowed to be set up
                    validFactorIds.push(id);
                }
                catch (err) {
                    logger_1.logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid assertAllowedToSetupFactorElseThrowInvalidClaimError failed for ${id}`);
                    caughtSetupFactorError = err;
                }
            }
            if (validFactorIds.length === 0) {
                logger_1.logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid rethrowing error from assertAllowedToSetupFactorElseThrowInvalidClaimError because we found no valid factors`);
            ...
            logger_1.logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid allowing all factors because it'll not create new link`);
            return factorIds;
        }
    }
    else {
        logger_1.logDebugMessage(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid allowing all factors because MFA is not enabled`);
