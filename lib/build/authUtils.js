...
exports.AuthUtils = {
    ...
    preAuthChecks: async function ({ authenticatingAccountInfo, tenantId, isSignUp, isVerified, signInVerifiesLoginMethod, authenticatingUser, factorIds, skipSessionUserUpdateInCore, session, shouldTryLinkingWithSessionUser, userContext, }) {
        ...
        if (isSignUp) {
            ...
            if (!(await recipe_1.default.getInstanceOrThrowError().isSignUpAllowed({
            ...
            if (!(await recipe_1.default.getInstanceOrThrowError().isSignInAllowed({
    ...
    checkAuthTypeAndLinkingStatus: async function (session, shouldTryLinkingWithSessionUser, accountInfo, inputUser, skipSessionUserUpdateInCore, userContext) {
        ...
        if (session === undefined) {
            ...
            if (!(0, utils_3.recipeInitDefinedShouldDoAutomaticAccountLinking)(recipe_1.default.getInstanceOrThrowError().config)) {
                if (shouldTryLinkingWithSessionUser === true) {
            ...
            const shouldLink = await recipe_1.default.getInstanceOrThrowError().config.shouldDoAutomaticAccountLinking(accountInfo, sessionUser, session, session.getTenantId(), userContext);
    ...
    tryAndMakeSessionUserIntoAPrimaryUser: async function (session, skipSessionUserUpdateInCore, userContext) {
        ...
        if (sessionUser.isPrimaryUser) {
            ...
            const shouldDoAccountLinking = await recipe_1.default.getInstanceOrThrowError().config.shouldDoAutomaticAccountLinking(sessionUser.loginMethods[0], undefined, session, session.getTenantId(userContext), userContext);
            ...
            if (shouldDoAccountLinking.shouldAutomaticallyLink) {
                ...
                const createPrimaryUserRes = await recipe_1.default.getInstanceOrThrowError().recipeInterfaceImpl.createPrimaryUser({
