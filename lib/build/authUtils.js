...
exports.AuthUtils = {
     ...
     */
    preAuthChecks: async function ({ stInstance, authenticatingAccountInfo, tenantId, isSignUp, isVerified, signInVerifiesLoginMethod, authenticatingUser, factorIds, skipSessionUserUpdateInCore, session, shouldTryLinkingWithSessionUser, userContext, }) {
        let validFactorIds;
        ...
        const authTypeInfo = await exports.AuthUtils.checkAuthTypeAndLinkingStatus(stInstance, session, shouldTryLinkingWithSessionUser, authenticatingAccountInfo, authenticatingUser, skipSessionUserUpdateInCore, userContext);
        ...
        if (authTypeInfo.isFirstFactor) {
            ...
            const validFirstFactors = await exports.AuthUtils.filterOutInvalidFirstFactorsOrThrowIfAllAreInvalid(stInstance, factorIds, tenantId, session !== undefined, userContext);
            ...
            validFactorIds = await filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid(stInstance, factorIds, authTypeInfo.inputUserAlreadyLinkedToSessionUser, authTypeInfo.sessionUser, session, userContext);
        }
        ...
        if (isSignUp) {
            ...
            if (!(await stInstance.getRecipeInstanceOrThrow("accountlinking").isSignUpAllowed({
            ...
            if (!(await stInstance.getRecipeInstanceOrThrow("accountlinking").isSignInAllowed({
     ...
     */
    postAuthChecks: async function ({ stInstance, authenticatedUser, recipeUserId, isSignUp, factorId, session, req, res, tenantId, userContext, }) {
        (0, logger_1.logDebugMessage)(`postAuthChecks called ${session !== undefined ? "with" : "without"} a session to ${isSignUp ? "sign in" : "sign up"} with ${factorId}`);
     ...
     */
    getAuthenticatingUserAndAddToCurrentTenantIfRequired: async ({ stInstance, recipeId, accountInfo, checkCredentialsOnTenant, tenantId, session, userContext, }) => {
        let i = 0;
        while (i++ < 300) {
            (0, logger_1.logDebugMessage)(`getAuthenticatingUserAndAddToCurrentTenantIfRequired called with ${JSON.stringify(accountInfo)}`);
            ...
            (0, logger_1.logDebugMessage)(`getAuthenticatingUserAndAddToCurrentTenantIfRequired got ${existingUsers.length} users from the core resp`);
            ...
            (0, logger_1.logDebugMessage)(`getAuthenticatingUserAndAddToCurrentTenantIfRequired got ${usersWithMatchingLoginMethods.length} users with matching login methods`);
            ...
            if (authenticatingUser === undefined && session !== undefined) {
                (0, logger_1.logDebugMessage)(`getAuthenticatingUserAndAddToCurrentTenantIfRequired checking session user`);
                ...
                if (!sessionUser.isPrimaryUser) {
                    (0, logger_1.logDebugMessage)(`getAuthenticatingUserAndAddToCurrentTenantIfRequired session user is non-primary so returning early without checking other tenants`);
                    return undefined;
                }
                const matchingLoginMethodsFromSessionUser = sessionUser.loginMethods.filter((lm) => lm.recipeId === recipeId &&
                    (lm.hasSameEmailAs(accountInfo.email) ||
                        lm.hasSamePhoneNumberAs(accountInfo.phoneNumber) ||
                        lm.hasSameThirdPartyInfoAs(accountInfo.thirdParty)));
                (0, logger_1.logDebugMessage)(`getAuthenticatingUserAndAddToCurrentTenantIfRequired session has ${matchingLoginMethodsFromSessionUser.length} matching login methods`);
                ...
                for (const lm of matchingLoginMethodsFromSessionUser) {
                    (0, logger_1.logDebugMessage)(`getAuthenticatingUserAndAddToCurrentTenantIfRequired session checking credentials on ${lm.tenantIds[0]}`);
                    if (await checkCredentialsOnTenant(lm.tenantIds[0])) {
                        (0, logger_1.logDebugMessage)(`getAuthenticatingUserAndAddToCurrentTenantIfRequired associating user from ${lm.tenantIds[0]} with current tenant`);
                        ...
                        (0, logger_1.logDebugMessage)(`getAuthenticatingUserAndAddToCurrentTenantIfRequired associating returned ${associateRes.status}`);
     ...
     * if the session user should be primary but we couldn't make it primary because of a conflicting primary user.
     */
    checkAuthTypeAndLinkingStatus: async function (stInstance, session, shouldTryLinkingWithSessionUser, accountInfo, inputUser, skipSessionUserUpdateInCore, userContext) {
        ...
        if (session === undefined) {
            ...
            (0, logger_1.logDebugMessage)(`checkAuthTypeAndLinkingStatus returning first factor because there is no session`);
            ...
            if (shouldTryLinkingWithSessionUser === false) {
                (0, logger_1.logDebugMessage)(`checkAuthTypeAndLinkingStatus returning first factor because shouldTryLinkingWithSessionUser is false`);
            ...
            if (!(0, utils_3.recipeInitDefinedShouldDoAutomaticAccountLinking)(stInstance.getRecipeInstanceOrThrow("accountlinking").config)) {
                if (shouldTryLinkingWithSessionUser === true) {
                    ...
                    if (stInstance.getRecipeInstance("multifactorauth") !== undefined) {
                        ...
                        (0, logger_1.logDebugMessage)(`checkAuthTypeAndLinkingStatus (legacy behaviour) returning first factor because MFA is not initialised and shouldDoAutomaticAccountLinking is not defined`);
                        return { status: "OK", isFirstFactor: true };
                    }
                }
            }
            ...
            if (inputUser !== undefined && inputUser.id === session.getUserId()) {
                (0, logger_1.logDebugMessage)(`checkAuthTypeAndLinkingStatus returning secondary factor, session and input user are the same`);
            ...
            (0, logger_1.logDebugMessage)(`checkAuthTypeAndLinkingStatus loading session user, ${inputUser === null || inputUser === void 0 ? void 0 : inputUser.id} === ${session.getUserId()}`);
            // We have to load the session user in order to get the account linking info
            const sessionUserResult = await exports.AuthUtils.tryAndMakeSessionUserIntoAPrimaryUser(stInstance, session, skipSessionUserUpdateInCore, userContext);
            ...
            const shouldLink = await stInstance
                .getRecipeInstanceOrThrow("accountlinking")
                .config.shouldDoAutomaticAccountLinking(accountInfo, sessionUser, session, session.getTenantId(), userContext);
            (0, logger_1.logDebugMessage)(`checkAuthTypeAndLinkingStatus session user <-> input user shouldDoAutomaticAccountLinking returned ${JSON.stringify(shouldLink)}`);
     ...
     * if the session user should be primary but we couldn't make it primary because of a conflicting primary user.
     */
    linkToSessionIfRequiredElseCreatePrimaryUserIdOrLinkByAccountInfo: async function ({ stInstance, tenantId, inputUser, recipeUserId, session, shouldTryLinkingWithSessionUser, userContext, }) {
        (0, logger_1.logDebugMessage)("linkToSessionIfRequiredElseCreatePrimaryUserIdOrLinkByAccountInfo called");
        const retry = () => {
            (0, logger_1.logDebugMessage)("linkToSessionIfRequiredElseCreatePrimaryUserIdOrLinkByAccountInfo retrying....");
        ...
        const authTypeRes = await exports.AuthUtils.checkAuthTypeAndLinkingStatus(stInstance, session, shouldTryLinkingWithSessionUser, authLoginMethod, inputUser, false, userContext);
        if (authTypeRes.status !== "OK") {
            return authTypeRes;
        }
        if (authTypeRes.isFirstFactor) {
            if (!(0, utils_3.recipeInitDefinedShouldDoAutomaticAccountLinking)(stInstance.getRecipeInstanceOrThrow("accountlinking").config)) {
                (0, logger_1.logDebugMessage)("linkToSessionIfRequiredElseCreatePrimaryUserIdOrLinkByAccountInfo skipping link by account info because this is a first factor auth and the app hasn't defined shouldDoAutomaticAccountLinking");
                return { status: "OK", user: inputUser };
            }
            (0, logger_1.logDebugMessage)("linkToSessionIfRequiredElseCreatePrimaryUserIdOrLinkByAccountInfo trying to link by account info because this is a first factor auth");
        ...
        (0, logger_1.logDebugMessage)("linkToSessionIfRequiredElseCreatePrimaryUserIdOrLinkByAccountInfo trying to link by session info");
     ...
     */
    tryAndMakeSessionUserIntoAPrimaryUser: async function (stInstance, session, skipSessionUserUpdateInCore, userContext) {
        ...
        if (sessionUser.isPrimaryUser) {
            ...
            const shouldDoAccountLinking = await stInstance
                .getRecipeInstanceOrThrow("accountlinking")
                .config.shouldDoAutomaticAccountLinking(sessionUser.loginMethods[0], undefined, session, session.getTenantId(userContext), userContext);
            (0, logger_1.logDebugMessage)(`tryAndMakeSessionUserIntoAPrimaryUser shouldDoAccountLinking: ${JSON.stringify(shouldDoAccountLinking)}`);
            if (shouldDoAccountLinking.shouldAutomaticallyLink) {
                ...
                if (shouldDoAccountLinking.shouldRequireVerification && !sessionUser.loginMethods[0].verified) {
                    ...
                    if ((await session.getClaimValue(emailverification_1.EmailVerificationClaim, userContext)) !== false) {
                        (0, logger_1.logDebugMessage)(`tryAndMakeSessionUserIntoAPrimaryUser updating emailverification status in session`);
                ...
                (0, logger_1.logDebugMessage)(`tryAndMakeSessionUserIntoAPrimaryUser createPrimaryUser returned ${createPrimaryUserRes.status}`);
     ...
     */
    tryLinkingBySession: async function ({ stInstance, linkingToSessionUserRequiresVerification, authLoginMethod, authenticatedUser, sessionUser, userContext, }) {
        ...
        if (linkAccountsResult.status === "OK") {
            ...
            (0, logger_1.logDebugMessage)("tryLinkingBySession linking to session user failed because of a race condition - input user linked to another user");
            ...
            (0, logger_1.logDebugMessage)("tryLinkingBySession linking to session user failed because of a race condition - INPUT_USER_IS_NOT_A_PRIMARY_USER, should retry");
            ...
            (0, logger_1.logDebugMessage)("tryLinkingBySession linking to session user failed because of a race condition - input user has another primary user it can be linked to");
    ...
    filterOutInvalidFirstFactorsOrThrowIfAllAreInvalid: async function (stInstance, factorIds, tenantId, hasSession, userContext) {
        let validFactorIds = [];
        for (const id of factorIds) {
            ...
            let validRes = await (0, utils_2.isValidFirstFactor)(stInstance, tenantId, id, userContext);
    ...
    loadSessionInAuthAPIIfNeeded: async function (stInstance, req, res, shouldTryLinkingWithSessionUser, userContext) {
        if (shouldTryLinkingWithSessionUser !== false) {
            (0, logger_1.logDebugMessage)("loadSessionInAuthAPIIfNeeded: loading session because shouldTryLinkingWithSessionUser is not set to false so we may want to link later");
            return await stInstance.getRecipeInstanceOrThrow("session").getSession({
                req,
                res,
                options: {
        ...
        (0, logger_1.logDebugMessage)("loadSessionInAuthAPIIfNeeded: skipping session loading because we are not linking and we would overwrite it anyway");
        return undefined;
    },
};
async function filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid(stInstance, factorIds, inputUserAlreadyLinkedToSessionUser, sessionUser, session, userContext) {
    if (session === undefined) {
    ...
    (0, logger_1.logDebugMessage)(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid called for ${factorIds.join(", ")}`);
    const mfaInstance = stInstance.getRecipeInstance("multifactorauth");
    if (mfaInstance !== undefined) {
        if (!inputUserAlreadyLinkedToSessionUser) {
            ...
            (0, logger_1.logDebugMessage)(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid checking if linking is allowed by the mfa recipe`);
            ...
            // then we do not want to include a link in the email.
            for (const id of factorIds) {
                (0, logger_1.logDebugMessage)(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid checking assertAllowedToSetupFactorElseThrowInvalidClaimError`);
                try {
                    ...
                    (0, logger_1.logDebugMessage)(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid ${id} valid because assertAllowedToSetupFactorElseThrowInvalidClaimError passed`);
                    // we add it to the valid factor ids list since it is either already set up or allowed to be set up
                    validFactorIds.push(id);
                }
                catch (err) {
                    (0, logger_1.logDebugMessage)(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid assertAllowedToSetupFactorElseThrowInvalidClaimError failed for ${id}`);
                    caughtSetupFactorError = err;
                }
            }
            if (validFactorIds.length === 0) {
                (0, logger_1.logDebugMessage)(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid rethrowing error from assertAllowedToSetupFactorElseThrowInvalidClaimError because we found no valid factors`);
            ...
            (0, logger_1.logDebugMessage)(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid allowing all factors because it'll not create new link`);
            return factorIds;
        }
    }
    else {
        (0, logger_1.logDebugMessage)(`filterOutInvalidSecondFactorsOrThrowIfAllAreInvalid allowing all factors because MFA is not enabled`);
