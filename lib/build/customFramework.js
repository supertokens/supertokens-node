...
async function getSessionForSSR(request, jwks) {
    ...
    if (!jwks) {
        ...
        jwksToUse = combinedRemoteJWKSet_1.getCombinedJWKS(sessionRecipe.config);
    }
    try {
        if (accessToken) {
...
async function withSession(request, handler, options, userContext, getCookieFn = getCookieFromRequest) {
    try {
