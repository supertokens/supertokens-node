...
async function getSessionForSSRUsingAccessToken(accessToken) {
    ...
    try {
        const sessionRecipe = supertokens_1.default.getInstanceOrThrowError().getRecipeInstanceOrThrow("session");
        const querier = querier_1.Querier.getNewInstanceOrThrowError(supertokens_1.default.getInstanceOrThrowError(), sessionRecipe.getRecipeId());
        const jwksToUse = (0, combinedRemoteJWKSet_1.getCombinedJWKS)(querier, sessionRecipe.config);
        try {
            if (accessToken) {
...
function addCookies(baseResponse, userResponse) {
    ...
    for (const respCookie of baseResponse.cookies) {
        didAddCookies = true;
        userResponse.headers.append("Set-Cookie", (0, cookie_1.serialize)(respCookie.key, respCookie.value, {
