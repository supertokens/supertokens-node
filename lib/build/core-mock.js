...
const getMockQuerier = (recipeId) => {
    ...
    const sendPostRequest = async (path, body, _userContext) => {
        ...
        if (path.getAsStringDangerous().includes("/recipe/webauthn/options/register")) {
            ...
            const now = new Date();
            writeDb("generatedOptions", id, Object.assign(Object.assign({}, registrationOptions), { id, origin: body.origin, tenantId: body.tenantId, email: body.email, rpId: registrationOptions.rp.id, createdAt: now.getTime(), expiresAt: now.getTime() + body.timeout * 1000 }));
            // @ts-ignore
            ...
            const id = crypto_1.default.randomUUID();
            const now = new Date();
            writeDb("generatedOptions", id, Object.assign(Object.assign({}, signInOptions), { id, origin: body.origin, tenantId: body.tenantId, email: body.email, createdAt: now.getTime(), expiresAt: now.getTime() + body.timeout * 1000 }));
            // @ts-ignore
