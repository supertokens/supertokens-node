...
function getRecipeInterface(stInstance, querier, getWebauthnConfig) {
    return {
        registerOptions: async function (_a) {
            var _b, _c;
            var { relyingPartyId, relyingPartyName, origin, timeout, attestation = "none", tenantId, userContext, supportedAlgorithmIds, userVerification, userPresence, residentKey } = _a, rest = __rest(_a, ["relyingPartyId", "relyingPartyName", "origin", "timeout", "attestation", "tenantId", "userContext", "supportedAlgorithmIds", "userVerification", "userPresence", "residentKey"]);
            ...
            if (emailInput !== undefined) {
                ...
                const userId = ((_b = result.recipeUserId) === null || _b === void 0 ? void 0 : _b.getAsString()) || user.id;
                email = (_c = user.loginMethods.find((lm) => lm.recipeUserId.getAsString() === userId)) === null || _c === void 0 ? void 0 : _c.email;
            }
            if (!email) {
                return {
        ...
        signInOptions: async function ({ relyingPartyId, relyingPartyName, origin, timeout, userVerification, userPresence, tenantId, userContext, }) {
            return await querier.sendPostRequest({
        ...
        completeSignIn: async function ({ verifiedCredentials, tenantId, session, shouldTryLinkingWithSessionUser, userContext }) {
            const { recipeUserId } = verifiedCredentials;
            let user = verifiedCredentials.user;
            const loginMethod = user.loginMethods.find((lm) => lm.recipeUserId.getAsString() === recipeUserId.getAsString());
        ...
        registerCredential: async function ({ webauthnGeneratedOptionsId, credential, userContext, recipeUserId }) {
            return await querier.sendPostRequest("/recipe/webauthn/user/credential/register", {
                recipeUserId,
                webauthnGeneratedOptionsId,
                credential,
            }, userContext);
        ...
        getUserFromRecoverAccountToken: async function ({ token, tenantId, userContext }) {
            ...
            if (resp.status === "OK") {
                return Object.assign(Object.assign({}, resp), { user: new user_1.User(resp.user), recipeUserId: resp.recipeUserId ? new recipeUserId_1.default(resp.recipeUserId) : undefined });
            }
            return resp;
        },
        removeCredential: async function ({ webauthnCredentialId, recipeUserId, userContext }) {
            return await querier.sendDeleteRequest("/recipe/webauthn/user/credential/remove", undefined, { recipeUserId, webauthnCredentialId }, userContext);
        },
        getCredential: async function ({ webauthnCredentialId, recipeUserId, userContext }) {
            const resp = await querier.sendGetRequest("/recipe/webauthn/user/credential", { webauthnCredentialId, recipeUserId }, userContext);
            if (resp.status === "OK") {
                return Object.assign(Object.assign({}, resp), { recipeUserId: new recipeUserId_1.default(resp.recipeUserId) });
            }
            return resp;
        },
        ...
        removeGeneratedOptions: async function ({ webauthnGeneratedOptionsId, tenantId, userContext }) {
            ...
            }, undefined, { webauthnGeneratedOptionsId }, userContext);
        },
        getGeneratedOptions: async function ({ webauthnGeneratedOptionsId, tenantId, userContext }) {
            ...
            }, { webauthnGeneratedOptionsId }, userContext);
        },
        updateUserEmail: async function ({ email, recipeUserId, tenantId, userContext }) {
            ...
            }, { email, recipeUserId }, {}, userContext);
        },
    };
}
