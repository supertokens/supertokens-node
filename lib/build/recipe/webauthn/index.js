...
class Wrapper {
    static async registerOptions(_a) {
        ...
        if ("request" in rest) {
            ...
            if (!rest.relyingPartyName) {
                throw new exports.Error({ type: "BAD_INPUT_ERROR", message: "RelyingPartyName missing from the input" });
            }
            origin = rest.origin;
            relyingPartyId = rest.relyingPartyId;
            relyingPartyName = rest.relyingPartyName;
        }
        return recipe_1.default.getInstanceOrThrowError().recipeInterfaceImpl.registerOptions(Object.assign(Object.assign({}, emailOrRecoverAccountToken), { residentKey,
            userVerification,
            ...
            timeout,
            attestation,
            tenantId, userContext: utils_2.getUserContext(userContext) }));
    }
    static async signInOptions(_a) {
        var { email, tenantId = constants_1.DEFAULT_TENANT_ID, userVerification = constants_2.DEFAULT_SIGNIN_OPTIONS_USER_VERIFICATION, timeout = constants_2.DEFAULT_SIGNIN_OPTIONS_TIMEOUT, userContext } = _a, rest = __rest(_a, ["email", "tenantId", "userVerification", "timeout", "userContext"]);
        let origin;
        let relyingPartyId;
        if ("request" in rest) {
            relyingPartyId =
    ...
    static signIn({ tenantId = constants_1.DEFAULT_TENANT_ID, webauthnGeneratedOptionsId, credential, session, userContext, }) {
    ...
    static async verifyCredentials({ tenantId = constants_1.DEFAULT_TENANT_ID, webauthnGeneratedOptionsId, credential, userContext, }) {
    ...
    static async sendEmail(input) {
        ...
        return await recipeInstance.emailDelivery.ingredientInterfaceImpl.sendEmail(Object.assign(Object.assign({}, input), { tenantId: input.tenantId || constants_1.DEFAULT_TENANT_ID, userContext: utils_2.getUserContext(input.userContext) }));
    }
}
