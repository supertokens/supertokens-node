...
function getAPIImplementation() {
    return {
        registerOptionsPOST: async function (_a) {
            var { tenantId, options, userContext } = _a, props = __rest(_a, ["tenantId", "options", "userContext"]);
            ...
            const response = await options.recipeImplementation.registerOptions(Object.assign(Object.assign({}, props), { displayName: "displayName" in props ? props.displayName : undefined, attestation,
                residentKey,
                userVerification,
                userPresence,
                origin,
        ...
        signUpPOST: async function ({ webauthnGeneratedOptionsId, credential, tenantId, session, shouldTryLinkingWithSessionUser, options, userContext, }) {
            const errorCodeMap = {
            ...
            if (!email) {
                throw new Error("Should never come here since we already check that the email value is a string in validateEmailAddress");
            }
            ...
            if (preAuthCheckRes.status === "SIGN_UP_NOT_ALLOWED") {
                const conflictingUsers = await recipe_1.default.getInstanceOrThrowError().recipeInterfaceImpl.listUsersByAccountInfo({
                    tenantId,
                    accountInfo: {
                        email,
                    },
                ...
                if (conflictingUsers.some((u) => u.loginMethods.some((lm) => lm.recipeId === "webauthn" && lm.hasSameEmailAs(email)))) {
                    return {
            ...
            if (preAuthCheckRes.status !== "OK") {
                return authUtils_1.AuthUtils.getErrorStatusResponseWithReason(preAuthCheckRes, errorCodeMap, "SIGN_UP_NOT_ALLOWED");
            }
            ...
            if (signUpResponse.status === "EMAIL_ALREADY_EXISTS_ERROR" ||
                ...
                return authUtils_1.AuthUtils.getErrorStatusResponseWithReason(signUpResponse, errorCodeMap, "SIGN_UP_NOT_ALLOWED");
            }
            ...
            if (postAuthChecks.status !== "OK") {
                ...
                authUtils_1.AuthUtils.getErrorStatusResponseWithReason(postAuthChecks, errorCodeMap, "SIGN_UP_NOT_ALLOWED");
        ...
        signInPOST: async function ({ webauthnGeneratedOptionsId, credential, tenantId, session, shouldTryLinkingWithSessionUser, options, userContext, }) {
            var _a;
            const errorCodeMap = {
            ...
            const email = (_a = authenticatingUser.user.loginMethods.find((lm) => { var _a; return lm.recipeId === "webauthn" && ((_a = lm.webauthn) === null || _a === void 0 ? void 0 : _a.credentialIds.includes(credential.id)); })) === null || _a === void 0 ? void 0 : _a.email;
            if (email === undefined) {
            ...
            if (preAuthChecks.status !== "OK") {
                return authUtils_1.AuthUtils.getErrorStatusResponseWithReason(preAuthChecks, errorCodeMap, "SIGN_IN_NOT_ALLOWED");
            }
            ...
            if (signInResponse.status === "INVALID_OPTIONS_ERROR" ||
                ...
                return authUtils_1.AuthUtils.getErrorStatusResponseWithReason(signInResponse, errorCodeMap, "SIGN_IN_NOT_ALLOWED");
            }
            const postAuthChecks = await authUtils_1.AuthUtils.postAuthChecks({
            ...
            if (postAuthChecks.status !== "OK") {
                return authUtils_1.AuthUtils.getErrorStatusResponseWithReason(postAuthChecks, errorCodeMap, "SIGN_IN_NOT_ALLOWED");
            }
            return {
                status: "OK",
                session: postAuthChecks.session,
        ...
        emailExistsGET: async function ({ email, tenantId, userContext }) {
            ...
            const webauthnUserExists = users.find((u) => {
                return (u.loginMethods.find((lm) => lm.recipeId === "webauthn" && lm.hasSameEmailAs(email)) !==
                    undefined);
            }) !== undefined;
            return {
                status: "OK",
        ...
        generateRecoverAccountTokenPOST: async function ({ email, tenantId, options, userContext }) {
            ...
            async function generateAndSendRecoverAccountToken(primaryUserId, recipeUserId) {
                ...
                if (response.status === "UNKNOWN_USER_ID_ERROR") {
                    (0, logger_1.logDebugMessage)(`Recover account email not sent, unknown user id: ${recipeUserId === undefined ? primaryUserId : recipeUserId.getAsString()}`);
                    return {
                        status: "OK",
                    };
                }
            ...
            for (let i = 0; i < users.length; i++) {
                const webauthnAccountTmp = users[i].loginMethods.find((l) => l.recipeId === "webauthn" && l.hasSameEmailAs(email));
            ...
            if (primaryUserAssociatedWithEmail === undefined) {
                ...
                return await generateAndSendRecoverAccountToken(webauthnAccount.recipeUserId.getAsString(), webauthnAccount.recipeUserId);
            }
            ...
            const emailVerified = primaryUserAssociatedWithEmail.loginMethods.find((lm) => {
            ...
            const hasOtherEmailOrPhone = primaryUserAssociatedWithEmail.loginMethods.find((lm) => {
            ...
            const shouldDoAccountLinkingResponse = await recipe_1.default.getInstanceOrThrowError().config.shouldDoAutomaticAccountLinking(webauthnAccount !== undefined
                ? webauthnAccount
                : {
                    recipeId: "webauthn",
                    email,
                }, primaryUserAssociatedWithEmail, undefined, tenantId, userContext);
            ...
            if (webauthnAccount === undefined) {
                ...
                if (!shouldDoAccountLinkingResponse.shouldAutomaticallyLink) {
                    (0, logger_1.logDebugMessage)(`Recover account email not sent, since webauthn user didn't exist, and account linking not enabled`);
                    return {
                        status: "OK",
                    };
                }
                ...
                if (isSignUpAllowed) {
                    ...
                    (0, logger_1.logDebugMessage)(`Recover account email not sent, isSignUpAllowed returned false for email: ${email}`);
                    return {
                        status: "OK",
                    };
                }
            ...
            const areTheTwoAccountsLinked = primaryUserAssociatedWithEmail.loginMethods.find((lm) => {
                return lm.recipeUserId.getAsString() === webauthnAccount.recipeUserId.getAsString();
            }) !== undefined;
            if (areTheTwoAccountsLinked) {
                return await generateAndSendRecoverAccountToken(primaryUserAssociatedWithEmail.id, webauthnAccount.recipeUserId);
            }
        ...
        recoverAccountPOST: async function ({ webauthnGeneratedOptionsId, credential, token, tenantId, options, userContext, }) {
            ...
            async function doRegisterCredentialAndVerifyEmailAndTryLinkIfNotPrimary(recipeUserId) {
                ...
                if (updateResponse.status === "INVALID_AUTHENTICATOR_ERROR") {
                ...
                else if (updateResponse.status === "INVALID_OPTIONS_ERROR" ||
                    ...
                    const updatedUserAfterEmailVerification = await (0, __1.getUser)(recipeUserId.getAsString(), userContext);
            ...
            if (existingUser.isPrimaryUser) {
                ...
                const webauthnUserIsLinkedToExistingUser = existingUser.loginMethods.find((lm) => {
                    ...
                    return (lm.recipeUserId.getAsString() === userIdForWhomTokenWasGenerated &&
                        lm.recipeId === "webauthn");
                }) !== undefined;
                if (webauthnUserIsLinkedToExistingUser) {
                    return doRegisterCredentialAndVerifyEmailAndTryLinkIfNotPrimary(new recipeUserId_1.default(userIdForWhomTokenWasGenerated));
                }
                else {
                    ...
                    if (createUserResponse.status === "INVALID_CREDENTIALS_ERROR" ||
                        ...
                        // as verified.
                        await markEmailAsVerified(createUserResponse.user.loginMethods[0].recipeUserId, tokenConsumptionResponse.email);
        ...
        listCredentialsGET: async function ({ options, userContext, session }) {
            ...
            const recipeUserIds = (_a = existingUser.loginMethods
                .filter((lm) => lm.recipeId === "webauthn")) === null || _a === void 0 ? void 0 : _a.map((lm) => lm.recipeUserId);
            const credentials = [];
        ...
        registerCredentialPOST: async function ({ recipeUserId, webauthnGeneratedOptionsId, credential, tenantId, options, userContext, session, }) {
            const errorCodeMap = {
            ...
            if (mfaInstance) {
                await multifactorauth_1.default.assertAllowedToSetupFactorElseThrowInvalidClaimError(session, multifactorauth_1.default.FactorIds.WEBAUTHN, userContext);
            }
            ...
            const loginMethod = user.loginMethods.find((lm) => lm.recipeId === "webauthn" && lm.recipeUserId.getAsString() === recipeUserId);
            if (!loginMethod) {
                return {
                    status: "GENERAL_ERROR",
                    message: "User not found",
            ...
            if (registerCredentialResponse.status !== "OK") {
                return authUtils_1.AuthUtils.getErrorStatusResponseWithReason(registerCredentialResponse, errorCodeMap, "REGISTER_CREDENTIAL_NOT_ALLOWED");
            }
            return {
                status: "OK",
            };
        ...
        removeCredentialPOST: async function ({ webauthnCredentialId, options, userContext, session }) {
            ...
            const recipeUserId = (_a = user.loginMethods.find((lm) => { var _a; return lm.recipeId === "webauthn" && ((_a = lm.webauthn) === null || _a === void 0 ? void 0 : _a.credentialIds.includes(webauthnCredentialId)); })) === null || _a === void 0 ? void 0 : _a.recipeUserId;
            if (!recipeUserId) {
                return {
                    status: "GENERAL_ERROR",
                    message: "User not found",
