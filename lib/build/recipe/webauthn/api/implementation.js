    ...
    return {
        ...
        signUpPOST: async function ({ webauthnGeneratedOptionsId, credential, tenantId, session, shouldTryLinkingWithSessionUser, options, userContext, }) {
            ...
            const preAuthCheckRes = await authUtils_1.AuthUtils.preAuthChecks({
                ...
                factorIds: [types_1.FactorIds.WEBAUTHN],
                isSignUp: true,
        ...
        signInPOST: async function ({ webauthnGeneratedOptionsId, credential, tenantId, session, shouldTryLinkingWithSessionUser, options, userContext, }) {
            ...
            if (verifyResult.status !== "OK") {
                (0, logger_1.logDebugMessage)(`signInPOST: returning INVALID_CREDENTIALS_ERROR because verifyCredentials returned ${verifyResult.status}${verifyResult.reason !== undefined
            ...
            if (generatedOptions.status !== "OK") {
                (0, logger_1.logDebugMessage)(`signInPOST: returning INVALID_CREDENTIALS_ERROR because getGeneratedOptions returned ${generatedOptions.status}`);
                return {
        ...
        generateRecoverAccountTokenPOST: async function ({ email, tenantId, options, userContext }) {
            ...
            const shouldDoAccountLinkingResponse = await stInstance
                .getRecipeInstanceOrThrow("accountlinking")
                .config.shouldDoAutomaticAccountLinking(webauthnAccount !== undefined
                ? webauthnAccount
                : {
                    recipeId: "webauthn",
                    email,
        ...
        registerCredentialPOST: async function ({ recipeUserId, webauthnGeneratedOptionsId, credential, tenantId, options, userContext, session, }) {
            ...
            if (mfaInstance) {
                await mfaInstance.assertAllowedToSetupFactorElseThrowInvalidClaimError(session, types_1.FactorIds.WEBAUTHN, userContext);
            }
