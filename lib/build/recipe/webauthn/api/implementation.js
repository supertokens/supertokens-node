...
function getAPIImplementation() {
    return {
        registerOptionsPOST: async function (_a) {
            ...
            let response = await options.recipeImplementation.registerOptions(Object.assign(Object.assign({}, props), { attestation,
                residentKey,
                userVerification,
                origin,
                relyingPartyId,
        ...
        signUpPOST: async function ({ webauthnGeneratedOptionsId, credential, tenantId, session, shouldTryLinkingWithSessionUser, options, userContext, }) {
            ...
            const preAuthCheckRes = await authUtils_1.AuthUtils.preAuthChecks({
                ...
                factorIds: ["webauthn"],
                isSignUp: true,
                isVerified: utils_1.isFakeEmail(email),
            ...
            if (preAuthCheckRes.status === "SIGN_UP_NOT_ALLOWED") {
                const conflictingUsers = await recipe_1.default.getInstance().recipeInterfaceImpl.listUsersByAccountInfo({
                    tenantId,
                    accountInfo: {
                        email,
                    },
            ...
            if (utils_1.isFakeEmail(email) && preAuthCheckRes.isFirstFactor) {
        ...
        signInPOST: async function ({ webauthnGeneratedOptionsId, credential, tenantId, session, shouldTryLinkingWithSessionUser, options, userContext, }) {
            const errorCodeMap = {
                SIGN_IN_NOT_ALLOWED: "Cannot sign in due to security reasons. Please try recovering your account, use a different login method or contact support. (ERR_CODE_008)",
                LINKING_TO_SESSION_USER_FAILED: {
        ...
        emailExistsGET: async function ({ email, tenantId, userContext, }) {
            ...
            let webauthnUserExists = users.find((u) => {
        ...
        generateRecoverAccountTokenPOST: async function ({ email, tenantId, options, userContext, }) {
            ...
            async function generateAndSendRecoverAccountToken(primaryUserId, recipeUserId) {
                ...
                if (response.status === "UNKNOWN_USER_ID_ERROR") {
                    logger_1.logDebugMessage(`Recover account email not sent, unknown user id: ${recipeUserId === undefined ? primaryUserId : recipeUserId.getAsString()}`);
                    return {
                        status: "OK",
                    };
                }
                let recoverAccountLink = utils_2.getRecoverAccountLink({
                ...
                logger_1.logDebugMessage(`Sending recover account email to ${email}`);
            ...
            for (let i = 0; i < users.length; i++) {
                let webauthnAccountTmp = users[i].loginMethods.find((l) => l.recipeId === "webauthn" && l.hasSameEmailAs(email));
            ...
            if (primaryUserAssociatedWithEmail === undefined) {
                if (webauthnAccount === undefined) {
                    logger_1.logDebugMessage(`Recover account email not sent, unknown user email: ${email}`);
                    return {
                        status: "OK",
                    };
                }
            ...
            let shouldDoAccountLinkingResponse = await recipe_1.default.getInstance().config.shouldDoAutomaticAccountLinking(webauthnAccount !== undefined
                ? webauthnAccount
                : {
                    recipeId: "webauthn",
                    email,
            ...
            if (webauthnAccount === undefined) {
                ...
                if (!shouldDoAccountLinkingResponse.shouldAutomaticallyLink) {
                    logger_1.logDebugMessage(`Recover account email not sent, since webauthn user didn't exist, and account linking not enabled`);
                    return {
                        status: "OK",
                    };
                }
                ...
                if (isSignUpAllowed) {
                    ...
                    logger_1.logDebugMessage(`Recover account email not sent, isSignUpAllowed returned false for email: ${email}`);
                    return {
                        status: "OK",
                    };
                }
        ...
        recoverAccountPOST: async function ({ webauthnGeneratedOptionsId, credential, token, tenantId, options, userContext, }) {
            ...
            async function doRegisterCredentialAndVerifyEmailAndTryLinkIfNotPrimary(recipeUserId) {
                ...
                if (updateResponse.status === "INVALID_AUTHENTICATOR_ERROR") {
                    ...
                    const updatedUserAfterEmailVerification = await __1.getUser(recipeUserId.getAsString(), userContext);
            ...
            if (existingUser.isPrimaryUser) {
                ...
                let webauthnUserIsLinkedToExistingUser = existingUser.loginMethods.find((lm) => {
