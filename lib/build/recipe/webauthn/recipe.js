...
const core_mock_1 = require("./core-mock");
    ...
    constructor(recipeId, appInfo, isInServerlessEnv, config, ingredients) {
        ...
        this.getAPIsHandled = () => {
            return [
                ...
                {
                    method: "post",
                    pathWithoutApiBasePath: new normalisedURLPath_1.default(constants_1.GENERATE_RECOVER_ACCOUNT_TOKEN_API),
        ...
        this.handleAPIRequest = async (id, tenantId, req, res, _path, _method, userContext) => {
            ...
            if (id === constants_1.REGISTER_OPTIONS_API) {
                return await registerOptions_1.default(this.apiImpl, tenantId, options, userContext);
            }
            else if (id === constants_1.SIGNIN_OPTIONS_API) {
                return await signInOptions_1.default(this.apiImpl, tenantId, options, userContext);
            }
                ...
                return await generateRecoverAccountToken_1.default(this.apiImpl, tenantId, options, userContext);
            }
            else if (id === constants_1.RECOVER_ACCOUNT_API) {
                return await recoverAccount_1.default(this.apiImpl, tenantId, options, userContext);
            }
        ...
        this.config = utils_1.validateAndNormaliseUserInput(this, appInfo, config);
        {
            ...
            const querier = core_mock_1.getMockQuerier(recipeId);
            let builder = new supertokens_js_override_1.default(recipeImplementation_1.default(querier, getWebauthnConfig));
        ...
        postSuperTokensInitCallbacks_1.PostSuperTokensInitCallbacks.addPostInitCallback(() => {
            ...
            if (mfaInstance !== undefined) {
                ...
                mfaInstance.addFuncToGetEmailsForFactorFromOtherRecipes((user, sessionRecipeUserId) => {
                    ...
                    const recipeLoginMethodsOrderedByTimeJoinedOldestFirst = orderedLoginMethodsByTimeJoinedOldestFirst.filter((lm) => lm.recipeId === Recipe.RECIPE_ID);
                    let result;
                    if (recipeLoginMethodsOrderedByTimeJoinedOldestFirst.length !== 0) {
                        ...
                        result = [
                            ...
                            ...recipeLoginMethodsOrderedByTimeJoinedOldestFirst
                                .filter((lm) => utils_3.isFakeEmail(lm.email))
                                .map((lm) => lm.email),
                        ];
                        ...
                        if (orderedLoginMethodsByTimeJoinedOldestFirst.some((lm) => lm.email !== undefined && !utils_3.isFakeEmail(lm.email))) {
                            ...
                            result = orderedLoginMethodsByTimeJoinedOldestFirst
                                .filter((lm) => lm.email !== undefined && !utils_3.isFakeEmail(lm.email))
                                .map((lm) => lm.email);
                        }
                        else {
