...
const emailPasswordRecipeImplementation_1 = __importDefault(require("./recipeImplementation/emailPasswordRecipeImplementation"));
    ...
    constructor(recipeId, appInfo, isInServerlessEnv, config, recipes, ingredients) {
        ...
        this.emailDelivery =
            ingredients.emailDelivery === undefined
                ? new emaildelivery_1.default(this.config.getEmailDeliveryConfig(emailPasswordRecipeImplementation, this.isInServerlessEnv))
                : ingredients.emailDelivery;
        this.emailPasswordRecipe =
            recipes.emailPasswordInstance !== undefined
                ? recipes.emailPasswordInstance
                : new recipe_1.default(recipeId, appInfo, isInServerlessEnv, {
                    override: {
                        functions: (_) => {
    ...
    static init(config) {
        return (appInfo, isInServerlessEnv) => {
            if (Recipe.instance === undefined) {
                ...
                throw new Error("ThirdPartyEmailPassword recipe has already been initialised. Please check your code for bugs.");
            }
        };
    }
    static reset() {
