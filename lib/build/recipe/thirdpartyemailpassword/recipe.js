...
class Recipe extends recipeModule_1.default {
    constructor(recipeId, appInfo, isInServerlessEnv, config, recipes) {
        ...
        this.verifyEmailUsingToken = (token) =>
            __awaiter(this, void 0, void 0, function* () {
                let response = yield this.emailVerificationRecipe.recipeInterfaceImpl.verifyEmailUsingToken({ token });
                if (response.status === "OK") {
                    let userInThisRecipe = yield this.recipeInterfaceImpl.getUserById({ userId: response.user.id });
