...
async function getSession(helpers, parsedAccessToken, antiCsrfToken, doAntiCsrfCheck, alwaysCheckCore, userContext) {
    ...
    try {
         ...
         */
        accessTokenInfo = await accessToken_1.getInfoFromAccessToken(parsedAccessToken, helpers.JWKS, helpers.config.antiCsrfFunctionOrString === "VIA_TOKEN" && doAntiCsrfCheck);
    }
    catch (err) {
        /**
...
async function revokeAllSessionsForUser(helpers, userId, revokeSessionsForLinkedAccounts, tenantId, revokeAcrossAllTenants, userContext) {
    ...
    let response = await helpers.querier.sendPostRequest(new normalisedURLPath_1.default(`/${tenantId}/recipe/session/remove`), {
        userId,
...
async function getAllSessionHandlesForUser(helpers, userId, fetchSessionsForAllLinkedAccounts, tenantId, fetchAcrossAllTenants, userContext) {
    ...
    let response = await helpers.querier.sendGetRequest(new normalisedURLPath_1.default(`/${tenantId}/recipe/session/user`), {
        userId,
