...
async function getSession(helpers, parsedAccessToken, antiCsrfToken, doAntiCsrfCheck, alwaysCheckCore, config, userContext) {
    ...
    try {
         ...
         */
        accessTokenInfo = await accessToken_1.getInfoFromAccessToken(parsedAccessToken, combinedRemoteJWKSet_1.getCombinedJWKS(config), helpers.config.antiCsrfFunctionOrString === "VIA_TOKEN" && doAntiCsrfCheck);
    }
    catch (err) {
        /**
    ...
    if (parsedAccessToken.version >= 3) {
        ...
        if (tokenUsesDynamicKey !== helpers.config.useDynamicAccessTokenSigningKey) {
            logger_1.logDebugMessage("getSession: Returning TRY_REFRESH_TOKEN because the access token doesn't match the useDynamicAccessTokenSigningKey in the config");
            ...
            if (accessTokenInfo !== undefined) {
                if (antiCsrfToken === undefined || antiCsrfToken !== accessTokenInfo.antiCsrfToken) {
                    if (antiCsrfToken === undefined) {
                        logger_1.logDebugMessage("getSession: Returning TRY_REFRESH_TOKEN because antiCsrfToken is missing from request");
                        throw new error_1.default({
                        ...
                        logger_1.logDebugMessage("getSession: Returning TRY_REFRESH_TOKEN because the passed antiCsrfToken is not the same as in the access token");
                        throw new error_1.default({
