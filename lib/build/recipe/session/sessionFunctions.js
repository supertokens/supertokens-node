...
async function getSession(helpers, parsedAccessToken, antiCsrfToken, doAntiCsrfCheck, alwaysCheckCore, config, userContext) {
    ...
    try {
         ...
         */
        accessTokenInfo = await (0, accessToken_1.getInfoFromAccessToken)(parsedAccessToken, (0, combinedRemoteJWKSet_1.getCombinedJWKS)(helpers.querier, config), helpers.config.antiCsrfFunctionOrString === "VIA_TOKEN" && doAntiCsrfCheck);
    }
    catch (err) {
        /**
...
async function getSessionInformation(helpers, sessionHandle, userContext) {
    ...
    let response = await helpers.querier.sendGetRequest("/recipe/session", {
        sessionHandle,
    }, userContext);
    if (response.status === "OK") {
...
async function getAllSessionHandlesForUser(helpers, userId, fetchSessionsForAllLinkedAccounts, tenantId, fetchAcrossAllTenants, userContext) {
    ...
    let response = await helpers.querier.sendGetRequest(fetchAcrossAllTenants
        ? "/recipe/session/user"
        : {
...
async function revokeSession(helpers, sessionHandle, userContext) {
    let response = await helpers.querier.sendPostRequest("/recipe/session/remove", {
...
async function updateSessionDataInDatabase(helpers, sessionHandle, newSessionData, userContext) {
    ...
    let response = await helpers.querier.sendPutRequest("/recipe/session/data", {
        sessionHandle,
...
async function updateAccessTokenPayload(helpers, sessionHandle, newAccessTokenPayload, userContext) {
    ...
    let response = await helpers.querier.sendPutRequest("/recipe/jwt/data", {
        sessionHandle,
