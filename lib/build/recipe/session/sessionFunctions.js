...
async function createNewSession(
    ...
    const requestBody = {
        ...
        userDataInJWT: Object.assign(Object.assign({}, accessTokenPayload), {
            recipeUserId: recipeUserId.getAsString(),
        }),
    ...
    if (process.env.MOCK !== "true") {
        ...
        response = await mockCore_1.mockCreateNewSession(requestBody, helpers.querier);
    }
    return {
        session: {
...
async function getSession(helpers, parsedAccessToken, antiCsrfToken, doAntiCsrfCheck, alwaysCheckCore) {
    ...
    if (accessTokenInfo !== undefined && !alwaysCheckCore && accessTokenInfo.parentRefreshTokenHash1 === undefined) {
        return {
            session: {
                ...
                recipeUserId:
                    (_a = accessTokenInfo.recipeUserId) !== null && _a !== void 0
                        ? _a
                        : accessTokenInfo.userData.recipeUserId,
    ...
    if (process.env.MOCK !== "true") {
        ...
        response = await mockCore_1.mockGetSession(requestBody, helpers.querier);
    }
    if (response.status === "OK") {
        ...
        return Object.assign(Object.assign({}, response), {
            session: {
                handle: response.session.handle,
                userId: response.session.userId,
                recipeUserId: new recipeUserId_1.default(
                    (_b = response.session.recipeUserId) !== null && _b !== void 0
                        ? _b
                        : parsedAccessToken.payload.recipeUserId
                ),
                expiryTime:
                    ((_c = response.accessToken) === null || _c === void 0 ? void 0 : _c.expiry) || // if we got a new accesstoken we take the expiry time from there
                ...
                tenantId:
                    ((_d = response.session) === null || _d === void 0 ? void 0 : _d.tenantId) ||
...
async function getSessionInformation(helpers, sessionHandle) {
    ...
    let response;
    if (process.env.MOCK !== "true") {
        response = await helpers.querier.sendGetRequest(new normalisedURLPath_1.default(`/recipe/session`), {
            sessionHandle,
        });
    } else {
        response = await mockCore_1.mockGetSessionInformation(sessionHandle, helpers.querier);
    }
    if (response.status === "OK") {
        ...
        return {
            ...
            recipeUserId: new recipeUserId_1.default(
                (_a = response.recipeUserId) !== null && _a !== void 0 ? _a : response.userDataInJWT.recipeUserId
            ),
...
async function refreshSession(helpers, refreshToken, antiCsrfToken, disableAntiCsrf) {
    ...
    if (process.env.MOCK !== "true") {
        ...
        response = await mockCore_1.mockGetRefreshAPIResponse(requestBody, helpers.querier);
    }
    if (response.status === "OK") {
        return {
            session: {
                ...
                userId: response.session.userId,
                recipeUserId: new recipeUserId_1.default(
                    (_a = response.session.recipeUserId) !== null && _a !== void 0
                        ? _a
                        : response.session.userDataInJWT.recipeUserId
                ),
                userDataInJWT: response.session.userDataInJWT,
                tenantId: response.session.tenantId,
            },
...
async function revokeAllSessionsForUser(
    ...
    if (process.env.MOCK !== "true") {
        ...
        return await mockCore_1.mockRevokeAllSessionsForUser({
            userId,
            revokeSessionsForLinkedAccounts,
...
async function getAllSessionHandlesForUser(
    ...
    if (process.env.MOCK !== "true" || (userContext !== undefined && userContext.doNotMock === true)) {
        ...
        return await mockCore_1.mockGetAllSessionHandlesForUser({ userId, fetchSessionsForAllLinkedAccounts });
    }
}
...
async function revokeSession(helpers, sessionHandle) {
    if (process.env.MOCK !== "true") {
        ...
        return await mockCore_1.mockRevokeSession(sessionHandle, helpers.querier);
    }
}
...
async function updateAccessTokenPayload(helpers, sessionHandle, newAccessTokenPayload) {
    ...
    if (process.env.MOCK !== "true") {
        ...
        return await mockCore_1.mockUpdateAccessTokenPayload(sessionHandle, newAccessTokenPayload, helpers.querier);
    }
}
