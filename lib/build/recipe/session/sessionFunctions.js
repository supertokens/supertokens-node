...
async function getSession(
    ...
    try {
        ...
        accessTokenInfo = await accessToken_1.getInfoFromAccessToken(
            parsedAccessToken,
            combinedRemoteJWKSet_1.getCombinedJWKS(config),
...
async function revokeAllSessionsForUser(
    ...
    let response = await helpers.querier.sendPostRequest(
        new normalisedURLPath_1.default(
            revokeAcrossAllTenants ? `/recipe/session/remove` : `/${tenantId}/recipe/session/remove`
        ),
        {
            userId,
            revokeSessionsForLinkedAccounts,
...
async function getAllSessionHandlesForUser(
    ...
    let response = await helpers.querier.sendGetRequest(
        new normalisedURLPath_1.default(
            fetchAcrossAllTenants ? `/recipe/session/user` : `/${tenantId}/recipe/session/user`
        ),
        {
            userId,
            fetchSessionsForAllLinkedAccounts,
