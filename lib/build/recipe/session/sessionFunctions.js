...
async function createNewSession(helpers, tenantId, recipeUserId, disableAntiCsrf, accessTokenPayload, sessionDataInDatabase, userContext) {
    ...
    let response = await helpers.querier.sendPostRequest(new normalisedURLPath_1.default(`/${tenantId}/recipe/session`), requestBody, userContext);
    return {
        session: {
...
async function getSession(helpers, parsedAccessToken, antiCsrfToken, doAntiCsrfCheck, alwaysCheckCore, config, userContext) {
    ...
    try {
         ...
         */
        accessTokenInfo = await (0, accessToken_1.getInfoFromAccessToken)(parsedAccessToken, (0, combinedRemoteJWKSet_1.getCombinedJWKS)(config), helpers.config.antiCsrfFunctionOrString === "VIA_TOKEN" && doAntiCsrfCheck);
    }
    catch (err) {
        /**
    ...
    if (parsedAccessToken.version >= 3) {
        ...
        if (tokenUsesDynamicKey !== helpers.config.useDynamicAccessTokenSigningKey) {
            (0, logger_1.logDebugMessage)("getSession: Returning TRY_REFRESH_TOKEN because the access token doesn't match the useDynamicAccessTokenSigningKey in the config");
    ...
    if (doAntiCsrfCheck) {
            ...
            if (accessTokenInfo !== undefined) {
                if (antiCsrfToken === undefined || antiCsrfToken !== accessTokenInfo.antiCsrfToken) {
                    if (antiCsrfToken === undefined) {
                        (0, logger_1.logDebugMessage)("getSession: Returning TRY_REFRESH_TOKEN because antiCsrfToken is missing from request");
                        throw new error_1.default({
                        ...
                        (0, logger_1.logDebugMessage)("getSession: Returning TRY_REFRESH_TOKEN because the passed antiCsrfToken is not the same as in the access token");
                        throw new error_1.default({
        ...
        else if (typeof helpers.config.antiCsrfFunctionOrString === "string" &&
    ...
    let response = await helpers.querier.sendPostRequest(new normalisedURLPath_1.default("/recipe/session/verify"), requestBody, userContext);
    if (response.status === "OK") {
        ...
        return Object.assign(Object.assign({}, response), { session: {
                ...
                userId: response.session.userId,
                recipeUserId: new recipeUserId_1.default(response.session.recipeUserId),
                expiryTime: ((_a = response.accessToken) === null || _a === void 0 ? void 0 : _a.expiry) || // if we got a new accesstoken we take the expiry time from there
                ...
                tenantId: ((_b = response.session) === null || _b === void 0 ? void 0 : _b.tenantId) || (accessTokenInfo === null || accessTokenInfo === void 0 ? void 0 : accessTokenInfo.tenantId) || constants_1.DEFAULT_TENANT_ID,
                userDataInJWT: response.session.userDataInJWT,
            } });
    }
...
async function refreshSession(helpers, refreshToken, antiCsrfToken, disableAntiCsrf, useDynamicAccessTokenSigningKey, userContext) {
    ...
    if (typeof helpers.config.antiCsrfFunctionOrString === "string" &&
        helpers.config.antiCsrfFunctionOrString === "VIA_CUSTOM_HEADER" &&
        !disableAntiCsrf) {
        // The function should never be called by this (we check this outside the function as well)
        // There we can add a bit more information to the error, so that's the primary check, this is just making sure.
    ...
    let response = await helpers.querier.sendPostRequest(new normalisedURLPath_1.default("/recipe/session/refresh"), requestBody, userContext);
    if (response.status === "OK") {
        return {
            session: {
                handle: response.session.handle,
...
async function revokeAllSessionsForUser(helpers, userId, revokeSessionsForLinkedAccounts, tenantId, revokeAcrossAllTenants, userContext) {
    ...
    let response = await helpers.querier.sendPostRequest(new normalisedURLPath_1.default(revokeAcrossAllTenants ? `/recipe/session/remove` : `/${tenantId}/recipe/session/remove`), {
        userId,
...
async function getAllSessionHandlesForUser(helpers, userId, fetchSessionsForAllLinkedAccounts, tenantId, fetchAcrossAllTenants, userContext) {
    ...
    let response = await helpers.querier.sendGetRequest(new normalisedURLPath_1.default(fetchAcrossAllTenants ? `/recipe/session/user` : `/${tenantId}/recipe/session/user`), {
        userId,
