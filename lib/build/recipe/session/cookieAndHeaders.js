...
function getCORSAllowedHeaders() {
    return [constants_2.antiCsrfHeaderKey, constants_1.HEADER_RID, constants_2.authorizationHeaderKey, constants_2.authModeHeaderKey];
}
...
function setToken(config, res, tokenType, value, expires, transferMethod, req, userContext) {
    ...
    if (transferMethod === "cookie") {
        setCookie(config, res, config.getCookieNameForTokenType(req, tokenType, userContext), value, expires, tokenType === "refresh" ? "refreshTokenPath" : "accessTokenPath", req, userContext);
    }
    else if (transferMethod === "header") {
...
function getAuthModeFromHeader(req) {
    var _a;
    return (_a = req.getHeaderValue(constants_2.authModeHeaderKey)) === null || _a === void 0 ? void 0 : _a.toLowerCase();
}
/**
    ...
    for (const token of tokenTypes) {
        if (hasMultipleCookiesForTokenType(config, req, token, userContext)) {
            ...
            if (config.olderCookieDomain === undefined) {
                throw new Error(`The request contains multiple session cookies. This may happen if you've changed the 'cookieDomain' value in your configuration. To clear tokens from the previous domain, set 'olderCookieDomain' in your config.`);
            }
            (0, logger_1.logDebugMessage)(`clearSessionCookiesFromOlderCookieDomain: Clearing duplicate ${token} cookie with domain ${config.olderCookieDomain}`);
            setToken(Object.assign(Object.assign({}, config), { cookieDomain: config.olderCookieDomain }), res, token, "", 0, "cookie", req, userContext);
...
function getCookieNamesFromRequestHeaderAllowingDuplicates(cookieString) {
    ...
    for (const cookiePair of cookiePairs) {
        ...
        try {
            ...
            (0, logger_1.logDebugMessage)(`getCookieNamesFromRequestHeaderAllowingDuplicates: Error decoding cookie name: ${name}`);
        }
