...
function setToken(config, res, tokenType, value, expires, transferMethod, req, userContext) {
    ...
    if (transferMethod === "cookie") {
        setCookie(config, res, getCookieNameFromTokenType(tokenType), value, expires, tokenType === "refresh" ? "refreshTokenPath" : "accessTokenPath", req, userContext);
    }
    else if (transferMethod === "header") {
...
function clearSessionCookiesFromOlderCookieDomain({ req, res, config, userContext, }) {
    ...
    for (const token of tokenTypes) {
        if (hasMultipleCookiesForTokenType(req, token)) {
            ...
            logger_1.logDebugMessage(`clearSessionCookiesFromOlderCookieDomain: Clearing duplicate ${token} cookie with domain ${config.olderCookieDomain}`);
