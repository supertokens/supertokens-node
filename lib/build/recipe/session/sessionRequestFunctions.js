...
async function refreshSessionInRequest({ res, req, userContext, config, recipeInterfaceImpl }) {
    ...
    if (
        ...
        // We need to clear the access token cookie if refresh token is not found.
        // See: https://github.com/supertokens/supertokens-node/issues/790
        cookieAndHeaders_1.setCookie(config, res, "sAccessToken", "", 0, "accessTokenPath", req, userContext);
        logger_1.logDebugMessage(
