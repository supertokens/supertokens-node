...
exports.createNewSessionInRequest = exports.refreshSessionInRequest = exports.getAccessTokenFromRequest = exports.getSessionFromRequest = void 0;
...
async function getSessionFromRequest({ req, res, config, recipeInterfaceImpl, options, userContext, }) {
    ...
    if (req.getCookieValue(LEGACY_ID_REFRESH_TOKEN_COOKIE_NAME) !== undefined) {
        logger_1.logDebugMessage("getSession: Throwing TRY_REFRESH_TOKEN because the request is using a legacy session");
    ...
    if (doAntiCsrfCheck && antiCsrf === "VIA_CUSTOM_HEADER") {
        if (antiCsrf === "VIA_CUSTOM_HEADER") {
            if (utils_2.getRidFromHeader(req) === undefined) {
                logger_1.logDebugMessage("getSession: Returning TRY_REFRESH_TOKEN because custom header (rid) was not passed");
                throw new error_1.default({
    ...
    if (session !== undefined) {
        const claimValidators = await utils_1.getRequiredClaimValidators(session, options === null || options === void 0 ? void 0 : options.overrideGlobalClaimValidators, userContext);
...
function getAccessTokenFromRequest(req, allowedTransferMethod) {
    ...
    if ((allowedTransferMethod === "any" || allowedTransferMethod === "header") &&
        ...
        if (hasMultipleAccessTokenCookies) {
            logger_1.logDebugMessage("getSession: Throwing TRY_REFRESH_TOKEN because multiple access tokens are present in request cookies");
            throw new error_1.default({
...
async function refreshSessionInRequest({ res, req, userContext, config, recipeInterfaceImpl, }) {
    ...
    cookieAndHeaders_1.clearSessionCookiesFromOlderCookieDomain({ req, res, config, userContext });
    const refreshTokens = {};
    ...
    if ((allowedTransferMethod === "any" || allowedTransferMethod === "header") &&
        ...
        if ((allowedTransferMethod === "any" || allowedTransferMethod === "cookie") &&
            ...
            logger_1.logDebugMessage("refreshSession: cleared all session tokens and returning UNAUTHORISED because refresh token in request is undefined");
    ...
    if (antiCsrf === "VIA_CUSTOM_HEADER" && !disableAntiCsrf) {
        if (utils_2.getRidFromHeader(req) === undefined) {
            logger_1.logDebugMessage("refreshSession: Returning UNAUTHORISED because custom header (rid) was not passed");
            throw new error_1.default({
...
async function createNewSessionInRequest({ req, res, userContext, recipeInstance, accessTokenPayload, userId, recipeUserId, config, appInfo, sessionDataInDatabase, tenantId, }) {
    ...
    if (outputTransferMethod === "cookie" &&
        ...
        !((appInfo.topLevelAPIDomain === "localhost" || utils_2.isAnIpAddress(appInfo.topLevelAPIDomain)) &&
            (appInfo.getTopLevelWebsiteDomain({
                ...
                utils_2.isAnIpAddress(appInfo.getTopLevelWebsiteDomain({
                    request: req,
                    userContext,
                }))))) {
