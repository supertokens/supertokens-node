...
class SessionWrapper {
    ...
    static async createNewSessionWithoutRequestResponse(tenantId, recipeUserId, accessTokenPayload = {}, sessionDataInDatabase = {}, disableAntiCsrf = false, userContext) {
        ...
        let user = await __1.getUser(recipeUserId.getAsString(), ctx);
        let userId = recipeUserId.getAsString();
        if (user !== undefined) {
            userId = user.id;
        }
    ...
    static async getSession(req, res, options, userContext) {
        ...
        return sessionRequestFunctions_1.getSessionFromRequest({
            ...
            config,
            options,
            userContext: utils_2.getUserContext(userContext), // userContext is normalized inside the function
        });
    }
    static async getSessionWithoutRequestResponse(accessToken, antiCsrfToken, options, userContext) {
        ...
        if (session !== undefined) {
            const claimValidators = await utils_1.getRequiredClaimValidators(session, options === null || options === void 0 ? void 0 : options.overrideGlobalClaimValidators, ctx);
    ...
    static getJWKS(userContext) {
        return recipe_1.default.getInstanceOrThrowError().openIdRecipe.recipeImplementation.getJWKS({
            userContext: utils_2.getUserContext(userContext),
        });
    }
    static getOpenIdDiscoveryConfiguration(userContext) {
        return recipe_1.default.getInstanceOrThrowError().openIdRecipe.recipeImplementation.getOpenIdDiscoveryConfiguration({
            userContext: utils_2.getUserContext(userContext),
        });
    }
