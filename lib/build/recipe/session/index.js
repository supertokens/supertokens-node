...
class SessionWrapper {
    static async createNewSession(
        ...
        alwaysOverwriteSessionInRequest = true,
        userContext = {}
    ) {
        ...
        let user = await __1.getUser(recipeUserId.getAsString(), userContext, true);
        let userId = recipeUserId.getAsString();
        if (user !== undefined) {
            userId = user.id;
        }
        if (!alwaysOverwriteSessionInRequest && !recipeInstance.config.overwriteSessionDuringSignIn) {
            const currentSession = await exports.getSession(req, res, { sessionRequired: false }, userContext);
