...
const with_jwt_1 = __importDefault(require("./with-jwt"));
...
class SessionRecipe extends recipeModule_1.default {
    constructor(recipeId, appInfo, isInServerlessEnv, config) {
        ...
        this.handleError = (err, request, response) => __awaiter(this, void 0, void 0, function* () {
            if (err.fromRecipe === SessionRecipe.RECIPE_ID) {
                if (err.type === error_1.default.UNAUTHORISED) {
                    ...
                    cookieAndHeaders_1.clearSessionFromAllTokenTransferMethods(this.config, response);
                    return yield this.config.errorHandlers.onTokenTheftDetected(err.payload.sessionHandle, err.payload.userId, err.payload.recipeUserId, request, response);
                }
                else if (err.type === error_1.default.INVALID_CLAIMS) {
                    return yield this.config.errorHandlers.onInvalidClaim(err.payload, request, response);
                }
                else {
                    throw err;
                }
        ...
        this.verifySession = (options, request, response) => __awaiter(this, void 0, void 0, function* () {
        ...
        logger_1.logDebugMessage("session init: refreshTokenPath: " + this.config.refreshTokenPath.getAsStringDangerous());
        ...
        if (this.config.jwt.enable === true) {
            ...
            let builder = new supertokens_js_override_1.default(recipeImplementation_1.default(querier_1.Querier.getNewInstanceOrThrowError(recipeId), this.config, this.getAppInfo(), () => this.recipeInterfaceImpl));
            this.recipeInterfaceImpl = builder
                .override((oI) => {
                return with_jwt_1.default(oI, 
                // this.jwtRecipe is never undefined here
                this.openIdRecipe.recipeImplementation, this.config);
            })
