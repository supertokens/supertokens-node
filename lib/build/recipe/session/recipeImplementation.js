...
class HandshakeInfo {
    constructor(antiCsrf, accessTokenBlacklistingEnabled, accessTokenValidity, refreshTokenValidity, rawJwtSigningPublicKeyList) {
    ...
    clone() {
        return new HandshakeInfo(this.antiCsrf, this.accessTokenBlacklistingEnabled, this.accessTokenValidity, this.refreshTokenValidity, this.rawJwtSigningPublicKeyList);
    }
}
...
function getRecipeInterface(querier, config, appInfo, getRecipeImplAfterOverrides) {
    ...
    function getHandshakeInfo(forceRefetch = false) {
        return __awaiter(this, void 0, void 0, function* () {
            if (handshakeInfo === undefined || handshakeInfo.getJwtSigningPublicKeyList().length === 0 || forceRefetch) {
                ...
                processState_1.ProcessState.getInstance().addState(processState_1.PROCESS_STATE.CALLING_SERVICE_IN_GET_HANDSHAKE_INFO);
                let response = yield querier.sendPostRequest(new normalisedURLPath_1.default("/recipe/handshake"), {});
                handshakeInfo = new HandshakeInfo(antiCsrf, response.accessTokenBlacklistingEnabled, response.accessTokenValidity, response.refreshTokenValidity, response.jwtSigningPublicKeyList);
                updateJwtSigningPublicKeyInfo(response.jwtSigningPublicKeyList, response.jwtSigningPublicKey, response.jwtSigningPublicKeyExpiryTime);
            }
    ...
    let obj = {
        createNewSession: function ({ req, res, userId, recipeUserId, accessTokenPayload = {}, sessionData = {}, userContext, }) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                let response = yield SessionFunctions.createNewSession(helpers, userId, disableAntiCSRF, recipeUserId, accessTokenPayload, sessionData);
                for (const transferMethod of constants_1.availableTokenTransferMethods) {
                    if (transferMethod !== outputTransferMethod && cookieAndHeaders_1.getToken(req, "access", transferMethod) !== undefined) {
                ...
                return new sessionClass_1.default(helpers, response.accessToken.token, response.session.handle, response.session.userId, response.session.recipeUserId, response.session.userDataInJWT, res, req, outputTransferMethod);
            });
        },
        ...
        getSession: function ({ req, res, options, userContext, }) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                for (const transferMethod of constants_1.availableTokenTransferMethods) {
                    ...
                    if (tokenString !== undefined) {
                        try {
                            ...
                            logger_1.logDebugMessage(`getSession: ignoring token in ${transferMethod}, because it doesn't match our access token structure`);
                        }
                    }
                }
                ...
                if ((allowedTransferMethod === "any" || allowedTransferMethod === "header") &&
                    ...
                    if (sessionOptional) {
                        logger_1.logDebugMessage("getSession: returning undefined because accessToken is undefined and sessionRequired is false");
                ...
                let response = yield SessionFunctions.getSession(helpers, accessToken, antiCsrfToken, doAntiCsrfCheck, utils_2.getRidFromHeader(req) !== undefined);
                let accessTokenString = accessToken.rawTokenString;
                if (response.accessToken !== undefined) {
                    cookieAndHeaders_1.setFrontTokenInHeaders(res, response.session.userId, response.accessToken.expiry, response.session.userDataInJWT);
                    cookieAndHeaders_1.setToken(config, res, "access", response.accessToken.token, 
                    ...
                    Date.now() + 3153600000000, requestTransferMethod);
                    accessTokenString = response.accessToken.token;
                }
                logger_1.logDebugMessage("getSession: Success!");
                const session = new sessionClass_1.default(helpers, accessTokenString, response.session.handle, response.session.userId, response.session.recipeUserId, response.session.userDataInJWT, res, req, requestTransferMethod);
                return session;
            });
        },
        validateClaims: function (input) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                for (const validator of input.claimValidators) {
                    ...
                    if ("claim" in validator && (yield validator.shouldRefetch(accessTokenPayload, input.userContext))) {
                        logger_1.logDebugMessage("updateClaimsInPayloadIfNeeded refetching " + validator.id);
                        const value = yield validator.claim.fetchValue(input.userId, input.recipeUserId, input.userContext);
                        logger_1.logDebugMessage("updateClaimsInPayloadIfNeeded " + validator.id + " refetch result " + JSON.stringify(value));
                        if (value !== undefined) {
                            accessTokenPayload = validator.claim.addToPayload_internal(accessTokenPayload, value, input.userContext);
                        }
                    }
                }
                ...
                const invalidClaims = yield utils_1.validateClaimsInPayload(input.claimValidators, accessTokenPayload, input.userContext);
                return {
                    invalidClaims,
        ...
        refreshSession: function ({ req, res, userContext }) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if ((allowedTransferMethod === "any" || allowedTransferMethod === "header") &&
                    ...
                    if (req.getCookieValue(LEGACY_ID_REFRESH_TOKEN_COOKIE_NAME) !== undefined) {
                        logger_1.logDebugMessage("refreshSession: cleared legacy id refresh token because refresh token was not found");
                        cookieAndHeaders_1.setCookie(config, res, LEGACY_ID_REFRESH_TOKEN_COOKIE_NAME, "", 0, "accessTokenPath");
                    }
                ...
                try {
                    let antiCsrfToken = cookieAndHeaders_1.getAntiCsrfTokenFromHeaders(req);
                    let response = yield SessionFunctions.refreshSession(helpers, refreshToken, antiCsrfToken, utils_2.getRidFromHeader(req) !== undefined, requestTransferMethod);
                    ...
                    return new sessionClass_1.default(helpers, response.accessToken.token, response.session.handle, response.session.userId, response.session.recipeUserId, response.session.userDataInJWT, res, req, requestTransferMethod);
                }
                catch (err) {
                    if (err.type === error_1.default.TOKEN_THEFT_DETECTED || err.payload.clearTokens) {
                        // This token isn't handled by getToken/setToken to limit the scope of this legacy/migration code
                        if (req.getCookieValue(LEGACY_ID_REFRESH_TOKEN_COOKIE_NAME) !== undefined) {
                            logger_1.logDebugMessage("refreshSession: cleared legacy id refresh token because refresh is clearing other tokens");
                            cookieAndHeaders_1.setCookie(config, res, LEGACY_ID_REFRESH_TOKEN_COOKIE_NAME, "", 0, "accessTokenPath");
                        }
                    }
                    throw err;
        ...
        regenerateAccessToken: function (input) {
            return __awaiter(this, void 0, void 0, function* () {
                let newAccessTokenPayload = input.newAccessTokenPayload === null || input.newAccessTokenPayload === undefined
                    ? {}
        ...
        mergeIntoAccessTokenPayload: function ({ sessionHandle, accessTokenPayloadUpdate, userContext, }) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                const newAccessTokenPayload = Object.assign(Object.assign({}, sessionInfo.accessTokenPayload), accessTokenPayloadUpdate);
        ...
        fetchAndSetClaim: function (input) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                const accessTokenPayloadUpdate = yield input.claim.build(sessionInfo.userId, sessionInfo.recipeUserId, input.userContext);
