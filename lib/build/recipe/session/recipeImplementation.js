    ...
    let obj = {
        createNewSession: function ({ res, userId, accessTokenPayload = {}, sessionData = {}, }) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                let response = yield SessionFunctions.createNewSession(helpers, userId, accessTokenPayload, sessionData);
                utils_1.attachCreateOrRefreshSessionResponseToExpressRes(config, res, response);
                return new sessionClass_1.default(helpers, response.accessToken.token, response.session.handle, response.session.userId, response.session.userDataInJWT, res);
            });
        },
            ...
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (idRefreshToken === undefined) {
                    ...
                    if (options !== undefined && typeof options !== "boolean" && options.sessionRequired === false) {
                        logger_1.logDebugMessage("getSession: returning undefined because idRefreshToken is undefined and sessionRequired is false");
                        // there is no session that exists here, and the user wants session verification
                        // to be optional. So we return undefined.
                        return undefined;
                    }
                    logger_1.logDebugMessage("getSession: UNAUTHORISED because idRefreshToken from cookies is undefined");
                ...
                if (accessToken === undefined) {
                    ...
                    if (options === undefined ||
                        ...
                        logger_1.logDebugMessage("getSession: Returning try refresh token because access token from cookies is undefined");
                        throw new error_1.default({
                ...
                try {
                    ...
                    let response = yield SessionFunctions.getSession(helpers, accessToken, antiCsrfToken, doAntiCsrfCheck, cookieAndHeaders_1.getRidFromHeader(req) !== undefined);
                    if (response.accessToken !== undefined) {
                        ...
                        cookieAndHeaders_1.attachAccessTokenToCookie(config, res, response.accessToken.token, response.accessToken.expiry);
        ...
        refreshSession: function ({ req, res }) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (inputIdRefreshToken === undefined) {
                    logger_1.logDebugMessage("refreshSession: UNAUTHORISED because idRefreshToken from cookies is undefined");
                    // we do not clear cookies here because of a
                    // race condition mentioned here: https://github.com/supertokens/supertokens-node/issues/17
                    throw new error_1.default({
                        message: "Session does not exist. Are you sending the session tokens in the request as cookies?",
                ...
                try {
                    ...
                    if (inputRefreshToken === undefined) {
                        logger_1.logDebugMessage("refreshSession: UNAUTHORISED because refresh token from cookies is undefined");
                        throw new error_1.default({
                    ...
                    let antiCsrfToken = cookieAndHeaders_1.getAntiCsrfTokenFromHeaders(req);
                    let response = yield SessionFunctions.refreshSession(helpers, inputRefreshToken, antiCsrfToken, cookieAndHeaders_1.getRidFromHeader(req) !== undefined);
                    utils_1.attachCreateOrRefreshSessionResponseToExpressRes(config, res, response);
                    logger_1.logDebugMessage("refreshSession: Success!");
                    return new sessionClass_1.default(helpers, response.accessToken.token, response.session.handle, response.session.userId, response.session.userDataInJWT, res);
                }
                catch (err) {
                    if ((err.type === error_1.default.UNAUTHORISED && err.payload.clearCookies) ||
                        err.type === error_1.default.TOKEN_THEFT_DETECTED) {
                        logger_1.logDebugMessage("refreshSession: Clearing cookies because of UNAUTHORISED or TOKEN_THEFT_DETECTED response");
                        cookieAndHeaders_1.clearSessionFromCookie(config, res);
                    }
                    throw err;
                }
