...
function getRecipeInterface(querier, config, appInfo, getRecipeImplAfterOverrides) {
    const JWKS = querier
        ...
        .map((url) => jose_1.createRemoteJWKSet(new URL(url), {
    ...
    */
    const combinedJWKS = async (...args) => {
        ...
        if (JWKS.length === 0) {
            throw Error("No SuperTokens core available to query. Please pass supertokens > connectionURI to the init function, or override all the functions of the recipe you are using.");
        }
    ...
    let obj = {
        ...
        getSession: async function ({ accessToken: accessTokenString, antiCsrfToken, options, userContext, }) {
            ...
            try {
                ...
                if ((options === null || options === void 0 ? void 0 : options.sessionRequired) === false) {
                    logger_1.logDebugMessage("getSession: Returning undefined because parsing failed and sessionRequired is false");
                    return undefined;
                }
                logger_1.logDebugMessage("getSession: UNAUTHORISED because the accessToken couldn't be parsed or had an invalid structure");
                throw new error_1.default({
            ...
            const response = await SessionFunctions.getSession(helpers, accessToken, antiCsrfToken, (options === null || options === void 0 ? void 0 : options.antiCsrfCheck) !== false, (options === null || options === void 0 ? void 0 : options.checkDatabase) === true, userContext);
            ...
            const session = new sessionClass_1.default(helpers, response.accessToken !== undefined ? response.accessToken.token : accessTokenString, cookieAndHeaders_1.buildFrontToken(response.session.userId, response.accessToken !== undefined ? response.accessToken.expiry : response.session.expiryTime, payload), undefined, // refresh
        ...
        validateClaims: async function (input) {
            ...
            const invalidClaims = await utils_1.validateClaimsInPayload(input.claimValidators, accessTokenPayload, input.userContext);
            return {
                invalidClaims,
        ...
        refreshSession: async function ({ refreshToken, antiCsrfToken, disableAntiCsrf, userContext, }) {
            ...
            const response = await SessionFunctions.refreshSession(helpers, refreshToken, antiCsrfToken, disableAntiCsrf, userContext);
