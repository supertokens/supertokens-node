...
function getRecipeInterface(querier, config) {
    ...
    let obj = {
        ...
        assertClaims: function (input) {
            return __awaiter(this, void 0, void 0, function* () {
                const claimValidatorsAddedByOtherRecipes = recipe_1.default.getClaimValidatorsAddedByOtherRecipes();
                const globalClaimValidators = yield this.getGlobalClaimValidators({
                ...
                const reqClaimsValidators =
                    input.overrideGlobalClaimValidators !== undefined
                        ? yield input.overrideGlobalClaimValidators(
                              input.session,
                ...
                yield input.session.assertClaims(reqClaimsValidators, input.userContext);
                logger_1.logDebugMessage("getSession: claim assertion successful");
            });
        },
        validateClaimsForSessionHandle: function (input) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                const reqClaimsValidators =
                    input.overrideGlobalClaimValidators !== undefined
                        ? yield input.overrideGlobalClaimValidators(sessionInfo, globalClaimValidators, userContext)
                        : globalClaimValidators;
                ...
                for (const validator of reqClaimsValidators) {
                    logger_1.logDebugMessage("Session.validateClaimsForSessionHandle checking " + validator.id);
                    if ("claim" in validator && (yield validator.shouldRefetch(newAccessTokenPayload, userContext))) {
                        logger_1.logDebugMessage("Session.validateClaimsForSessionHandle refetching " + validator.id);
                        const value = yield validator.claim.fetchValue(sessionInfo.userId, userContext);
                        logger_1.logDebugMessage(
                ...
                for (const validator of reqClaimsValidators) {
                    logger_1.logDebugMessage("Session.validateClaimsForSessionHandle " + " validating " + validator.id);
                ...
                if (JSON.stringify(newAccessTokenPayload) !== origSessionClaimPayloadJSON) {
                    yield this.mergeIntoAccessTokenPayload({
        ...
        validateClaimsInJWTPayload: function (input) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                const reqClaimsValidators =
                    input.overrideGlobalClaimValidators !== undefined
                        ? yield input.overrideGlobalClaimValidators(input.userId, globalClaimValidators, userContext)
                        : globalClaimValidators;
                const validationErrors = [];
                for (const validator of reqClaimsValidators) {
                    logger_1.logDebugMessage("Session.validateClaimsInJWTPayload " + " validating " + validator.id);
                    const claimValidationResult = yield validator.validate(input.jwtPayload, userContext);
                    logger_1.logDebugMessage(
