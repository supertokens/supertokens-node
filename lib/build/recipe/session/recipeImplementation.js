...
function getRecipeInterface(querier, config, appInfo, getRecipeImplAfterOverrides) {
    let obj = {
        createNewSession: async function ({ recipeUserId, accessTokenPayload = {}, sessionDataInDatabase = {}, disableAntiCsrf, tenantId, userContext, }) {
            (0, logger_1.logDebugMessage)("createNewSession: Started");
            let response = await SessionFunctions.createNewSession(helpers, tenantId, recipeUserId, disableAntiCsrf === true, accessTokenPayload, sessionDataInDatabase, userContext);
        ...
        getSession: async function ({ accessToken: accessTokenString, antiCsrfToken, options, userContext, }) {
            if ((options === null || options === void 0 ? void 0 : options.antiCsrfCheck) !== false &&
                typeof config.antiCsrfFunctionOrString === "string" &&
                config.antiCsrfFunctionOrString === "VIA_CUSTOM_HEADER") {
                throw new Error("Since the anti-csrf mode is VIA_CUSTOM_HEADER getSession can't check the CSRF token. Please either use VIA_TOKEN or set antiCsrfCheck to false");
            }
            ...
            if (accessTokenString === undefined) {
                if ((options === null || options === void 0 ? void 0 : options.sessionRequired) === false) {
                    (0, logger_1.logDebugMessage)("getSession: returning undefined because accessToken is undefined and sessionRequired is false");
            ...
            try {
                ...
                if ((options === null || options === void 0 ? void 0 : options.sessionRequired) === false) {
                    (0, logger_1.logDebugMessage)("getSession: Returning undefined because parsing failed and sessionRequired is false");
                    return undefined;
                }
                (0, logger_1.logDebugMessage)("getSession: UNAUTHORISED because the accessToken couldn't be parsed or had an invalid structure");
                throw new error_1.default({
            ...
            const response = await SessionFunctions.getSession(helpers, accessToken, antiCsrfToken, (options === null || options === void 0 ? void 0 : options.antiCsrfCheck) !== false, (options === null || options === void 0 ? void 0 : options.checkDatabase) === true, config, userContext);
            (0, logger_1.logDebugMessage)("getSession: Success!");
            const payload = accessToken.version >= 3
                ? response.accessToken !== undefined
                    ? (0, jwt_1.parseJWTWithoutSignatureVerification)(response.accessToken.token).payload
                    : accessToken.payload
                : response.session.userDataInJWT;
            const session = new sessionClass_1.default(helpers, response.accessToken !== undefined ? response.accessToken.token : accessTokenString, (0, cookieAndHeaders_1.buildFrontToken)(response.session.userId, response.accessToken !== undefined ? response.accessToken.expiry : response.session.expiryTime, payload), undefined, // refresh
            antiCsrfToken, response.session.handle, response.session.userId, response.session.recipeUserId, payload, undefined, response.accessToken !== undefined, response.session.tenantId);
            return session;
        },
        validateClaims: async function (input) {
            ...
            for (const validator of input.claimValidators) {
                (0, logger_1.logDebugMessage)("updateClaimsInPayloadIfNeeded checking shouldRefetch for " + validator.id);
                if ("claim" in validator && (await validator.shouldRefetch(accessTokenPayload, input.userContext))) {
                    ...
                    const value = await validator.claim.fetchValue(input.userId, input.recipeUserId, accessTokenPayload.tId === undefined ? constants_1.DEFAULT_TENANT_ID : accessTokenPayload.tId, accessTokenPayload, input.userContext);
                    (0, logger_1.logDebugMessage)("updateClaimsInPayloadIfNeeded " + validator.id + " refetch result " + JSON.stringify(value));
                    if (value !== undefined) {
            ...
            const invalidClaims = await (0, utils_1.validateClaimsInPayload)(input.claimValidators, accessTokenPayload, input.userContext);
            return {
                invalidClaims,
        ...
        refreshSession: async function ({ refreshToken, antiCsrfToken, disableAntiCsrf, userContext, }) {
            if (disableAntiCsrf !== true &&
                typeof config.antiCsrfFunctionOrString === "string" &&
                config.antiCsrfFunctionOrString === "VIA_CUSTOM_HEADER") {
                throw new Error("Since the anti-csrf mode is VIA_CUSTOM_HEADER getSession can't check the CSRF token. Please either use VIA_TOKEN or set antiCsrfCheck to false");
            }
            (0, logger_1.logDebugMessage)("refreshSession: Started");
            const response = await SessionFunctions.refreshSession(helpers, refreshToken, antiCsrfToken, disableAntiCsrf, config.useDynamicAccessTokenSigningKey, userContext);
            (0, logger_1.logDebugMessage)("refreshSession: Success!");
            const payload = (0, jwt_1.parseJWTWithoutSignatureVerification)(response.accessToken.token).payload;
            return new sessionClass_1.default(helpers, response.accessToken.token, (0, cookieAndHeaders_1.buildFrontToken)(response.session.userId, response.accessToken.expiry, payload), response.refreshToken, response.antiCsrfToken, response.session.handle, response.session.userId, response.session.recipeUserId, payload, undefined, true, payload.tId);
        },
        regenerateAccessToken: async function (input) {
            ...
            let response = await querier.sendPostRequest("/recipe/session/regenerate", {
        ...
        revokeAllSessionsForUser: function ({ userId, tenantId, revokeAcrossAllTenants, revokeSessionsForLinkedAccounts, userContext, }) {
            return SessionFunctions.revokeAllSessionsForUser(helpers, userId, revokeSessionsForLinkedAccounts, tenantId, revokeAcrossAllTenants, userContext);
        },
        getAllSessionHandlesForUser: function ({ userId, fetchSessionsForAllLinkedAccounts, tenantId, fetchAcrossAllTenants, userContext, }) {
            return SessionFunctions.getAllSessionHandlesForUser(helpers, userId, fetchSessionsForAllLinkedAccounts, tenantId, fetchAcrossAllTenants, userContext);
        },
        ...
        mergeIntoAccessTokenPayload: async function ({ sessionHandle, accessTokenPayloadUpdate, userContext, }) {
            ...
            return SessionFunctions.updateAccessTokenPayload(helpers, sessionHandle, newAccessTokenPayload, userContext);
        },
        fetchAndSetClaim: async function (input) {
            ...
            const accessTokenPayloadUpdate = await input.claim.build(sessionInfo.userId, sessionInfo.recipeUserId, sessionInfo.tenantId, sessionInfo.customClaimsInAccessTokenPayload, input.userContext);
