...
async function sendTryRefreshTokenResponse(recipeInstance, _, __, response, ___) {
    (0, utils_2.sendNon200ResponseWithMessage)(response, "try refresh token", recipeInstance.config.sessionExpiredStatusCode);
}
async function sendUnauthorisedResponse(recipeInstance, _, __, response, ___) {
    (0, utils_2.sendNon200ResponseWithMessage)(response, "unauthorised", recipeInstance.config.sessionExpiredStatusCode);
}
...
async function sendTokenTheftDetectedResponse(recipeInstance, sessionHandle, _, __, ___, response, userContext) {
    ...
    (0, utils_2.sendNon200ResponseWithMessage)(response, "token theft detected", recipeInstance.config.sessionExpiredStatusCode);
}
...
function validateAndNormaliseUserInput(recipeInstance, appInfo, config) {
    ...
    let cookieDomain = config === undefined || config.cookieDomain === undefined
        ? undefined
        : normaliseSessionScopeOrThrowError(config.cookieDomain);
    let olderCookieDomain = config === undefined || config.olderCookieDomain === undefined || config.olderCookieDomain === ""
        ? config === null || config === void 0 ? void 0 : config.olderCookieDomain
        : normaliseSessionScopeOrThrowError(config.olderCookieDomain);
    let accessTokenPath = config === undefined || config.accessTokenPath === undefined
    ...
    let cookieSameSite = (input) => {
        let protocolOfWebsiteDomain = getURLProtocol(appInfo
            .getOrigin({
    ...
    let cookieSecure = config === undefined || config.cookieSecure === undefined
        ? appInfo.apiDomain.getAsStringDangerous().startsWith("https")
        : config.cookieSecure;
    let sessionExpiredStatusCode = config === undefined || config.sessionExpiredStatusCode === undefined ? 401 : config.sessionExpiredStatusCode;
    const invalidClaimStatusCode = (_a = config === null || config === void 0 ? void 0 : config.invalidClaimStatusCode) !== null && _a !== void 0 ? _a : 403;
    ...
    let errorHandlers = {
        onTokenTheftDetected: async (sessionHandle, userId, recipeUserId, request, response, userContext) => {
            return await sendTokenTheftDetectedResponse(recipeInstance, sessionHandle, userId, recipeUserId, request, response, userContext);
        },
    ...
    return {
        useDynamicAccessTokenSigningKey: (_b = config === null || config === void 0 ? void 0 : config.useDynamicAccessTokenSigningKey) !== null && _b !== void 0 ? _b : true,
        exposeAccessTokenToFrontendInCookieBasedAuth: (_c = config === null || config === void 0 ? void 0 : config.exposeAccessTokenToFrontendInCookieBasedAuth) !== null && _c !== void 0 ? _c : false,
        refreshTokenPath: appInfo.apiBasePath.appendPath(new normalisedURLPath_1.default(constants_1.REFRESH_API_PATH)),
        accessTokenPath,
        getTokenTransferMethod: (config === null || config === void 0 ? void 0 : config.getTokenTransferMethod) === undefined
            ? defaultGetTokenTransferMethod
            : config.getTokenTransferMethod,
        getCookieNameForTokenType: (_d = config === null || config === void 0 ? void 0 : config.getCookieNameForTokenType) !== null && _d !== void 0 ? _d : getCookieNameForTokenType,
        getResponseHeaderNameForTokenType: (_e = config === null || config === void 0 ? void 0 : config.getResponseHeaderNameForTokenType) !== null && _e !== void 0 ? _e : getResponseHeaderNameForTokenType,
        cookieDomain,
        olderCookieDomain,
        ...
        jwksRefreshIntervalSec: (_f = config === null || config === void 0 ? void 0 : config.jwksRefreshIntervalSec) !== null && _f !== void 0 ? _f : 3600 * 4,
    };
}
...
function setAccessTokenInResponse(res, accessToken, frontToken, config, transferMethod, req, userContext) {
    ...
    (0, cookieAndHeaders_1.setToken)(config, res, "access", accessToken, 
    ...
    Date.now() + constants_1.oneYearInMs, transferMethod, req, userContext);
...
async function getRequiredClaimValidators(session, overrideGlobalClaimValidators, userContext) {
    const claimValidatorsAddedByOtherRecipes = recipe_1.default.getInstanceOrThrowError().getClaimValidatorsAddedByOtherRecipes();
    const globalClaimValidators = await recipe_1.default.getInstanceOrThrowError().recipeInterfaceImpl.getGlobalClaimValidators({
