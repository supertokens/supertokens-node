...
function validateAndNormaliseUserInput(recipeInstance, appInfo, config) {
    ...
    let cookieSameSite = config === undefined || config.cookieSameSite === undefined
        ? "lax"
    ...
    let errorHandlers = {
        onTokenTheftDetected: (sessionHandle, userId, request, response, next) => {
            return middleware_1.sendTokenTheftDetectedResponse(recipeInstance, sessionHandle, userId, request, response, next);
        },
    ...
    return {
        refreshTokenPath: appInfo.apiBasePath.appendPath(recipeInstance.getRecipeId(), new normalisedURLPath_1.default(recipeInstance.getRecipeId(), constants_1.REFRESH_API_PATH)),
        cookieDomain,
        cookieSameSite,
        cookieSecure,
...
function attachCreateOrRefreshSessionResponseToExpressRes(recipeInstance, res, response) {
    ...
    cookieAndHeaders_1.setFrontTokenInHeaders(recipeInstance, res, response.session.userId, response.accessToken.expiry, response.session.userDataInJWT);
    ...
    cookieAndHeaders_1.setIdRefreshTokenInHeaderAndCookie(recipeInstance, res, idRefreshToken.token, idRefreshToken.expiry);
