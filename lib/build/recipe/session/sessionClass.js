...
class Session {
    ...
    async revokeSession(userContext) {
        const ctx = userContext === undefined && this.reqResInfo !== undefined
            ? utils_2.makeDefaultUserContextFromAPI(this.reqResInfo.req)
            : utils_2.getUserContext(userContext);
        ...
        if (this.reqResInfo !== undefined) {
            ...
            cookieAndHeaders_1.clearSession(this.helpers.config, this.reqResInfo.res, this.reqResInfo.transferMethod, this.reqResInfo.req, ctx);
        }
    }
    async getSessionDataFromDatabase(userContext) {
        ...
        if (sessionInfo === undefined) {
            logger_1.logDebugMessage("getSessionDataFromDatabase: Throwing UNAUTHORISED because session does not exist anymore");
    ...
    async updateSessionDataInDatabase(newSessionData, userContext) {
        if (!(await this.helpers.getRecipeImpl().updateSessionDataInDatabase({
            ...
            logger_1.logDebugMessage("updateSessionDataInDatabase: Throwing UNAUTHORISED because session does not exist anymore");
            throw new error_1.default({
                message: "Session does not exist anymore",
                type: error_1.default.UNAUTHORISED,
            });
    ...
    async mergeIntoAccessTokenPayload(accessTokenPayloadUpdate, userContext) {
        ...
        if (response === undefined) {
            logger_1.logDebugMessage("mergeIntoAccessTokenPayload: Throwing UNAUTHORISED because session does not exist anymore");
            throw new error_1.default({
                message: "Session does not exist anymore",
                type: error_1.default.UNAUTHORISED,
            });
        ...
        if (response.accessToken !== undefined) {
            ...
            if (this.reqResInfo !== undefined) {
                ...
                utils_1.setAccessTokenInResponse(this.reqResInfo.res, this.accessToken, this.frontToken, this.helpers.config, this.reqResInfo.transferMethod, this.reqResInfo.req, ctx);
            }
        }
        else {
            ...
            this.userDataInAccessToken = Object.assign(Object.assign({}, this.getAccessTokenPayload(ctx)), response.session.userDataInJWT);
        }
    }
    ...
    async fetchAndSetClaim(claim, userContext) {
        const ctx = utils_2.getUserContext(userContext);
        const update = await claim.build(this.getUserId(ctx), this.getRecipeUserId(ctx), this.getTenantId(ctx), this.getAccessTokenPayload(ctx), ctx);
        return this.mergeIntoAccessTokenPayload(update, ctx);
    }
    // Any update to this function should also be reflected in the respective JWT version
    setClaimValue(claim, value, userContext) {
        const ctx = utils_2.getUserContext(userContext);
        const update = claim.addToPayload_internal({}, value, utils_2.getUserContext(ctx));
        return this.mergeIntoAccessTokenPayload(update, ctx);
    }
    // Any update to this function should also be reflected in the respective JWT version
    ...
    attachToRequestResponse(info, userContext) {
        ...
        if (this.accessTokenUpdated) {
            ...
            const ctx = userContext !== undefined ? utils_2.getUserContext(userContext) : utils_2.makeDefaultUserContextFromAPI(info.req);
            utils_1.setAccessTokenInResponse(res, this.accessToken, this.frontToken, this.helpers.config, transferMethod, info.req, ctx);
            if (this.refreshToken !== undefined) {
                cookieAndHeaders_1.setToken(this.helpers.config, res, "refresh", this.refreshToken.token, this.refreshToken.expiry, transferMethod, info.req, ctx);
            }
