...
const passwordlessRecipeImplementation_1 = __importDefault(
    require("./recipeImplementation/passwordlessRecipeImplementation")
);
...
class Recipe extends recipeModule_1.default {
    constructor(recipeId, appInfo, isInServerlessEnv, config, recipes, ingredients) {
        ...
        this.handleAPIRequest = async (id, tenantId, req, res, path, method, userContext) => {
            if (
                (await this.passwordlessRecipe.returnAPIIdIfCanHandleRequest(path, method, userContext)) !== undefined
            ) {
                return await this.passwordlessRecipe.handleAPIRequest(
                    id,
                    tenantId,
                    req,
                    res,
    ...
    static getInstanceOrThrowError() {
        ...
        throw new Error("Initialisation not done. Did you forget to call the ThirdPartyPasswordless.init function?");
    }
}
