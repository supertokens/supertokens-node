            ...
            if (Recipe.instance === undefined) {
                ...
                postSuperTokensInitCallbacks_1.PostSuperTokensInitCallbacks.addPostInitCallback(() => {
                    ...
                    if (mfaInstance !== undefined) {
                        ...
                        mfaInstance.addGetFactorsSetupForUserFromOtherRecipes(async (_, user) => {
                            ...
                            function isFactorSetupForUser(user, factorId) {
                                for (const loginMethod of user.loginMethods) {
                                    ...
                                    if (loginMethod.email !== undefined && !utils_2.isFakeEmail(loginMethod.email)) {
                        ...
                        mfaInstance.addGetEmailsForFactorFromOtherRecipes((user, sessionRecipeUserId) => {
                            ...
                            for (let i = 0; i < orderedLoginMethodsByTimeJoinedOldestFirst.length; i++) {
                                ...
                                if (orderedLoginMethodsByTimeJoinedOldestFirst[i].recipeId === Recipe.RECIPE_ID) {
                                    if (orderedLoginMethodsByTimeJoinedOldestFirst[i].email !== undefined && !utils_2.isFakeEmail(orderedLoginMethodsByTimeJoinedOldestFirst[i].email)) {
                            ...
                            if (nonFakeEmailsThatPasswordlessLoginMethodOrderedByTimeJoined.length === 0) {
                                ...
                                let emailsResult = [];
                                if (sessionLoginMethod.email !== undefined && !utils_2.isFakeEmail(sessionLoginMethod.email)) {
                                ...
                                let factorIdToEmailsMap = {};
                                if (allFactors.includes(multifactorauth_1.FactorIds.OTP_EMAIL)) {
                                    factorIdToEmailsMap[multifactorauth_1.FactorIds.OTP_EMAIL] = nonFakeEmailsThatPasswordlessLoginMethodOrderedByTimeJoined;
                                }
                                if (allFactors.includes(multifactorauth_1.FactorIds.LINK_EMAIL)) {
                        ...
                        mfaInstance.addGetPhoneNumbersForFactorsFromOtherRecipes((user, sessionRecipeUserId) => {
                            ...
                            if (phoneNumbersThatPasswordlessLoginMethodOrderedByTimeJoined.length === 0) {
                                ...
                                let factorIdToPhoneNumberMap = {};
                                if (allFactors.includes(multifactorauth_1.FactorIds.OTP_PHONE)) {
                                    factorIdToPhoneNumberMap[multifactorauth_1.FactorIds.OTP_PHONE] = phoneNumbersThatPasswordlessLoginMethodOrderedByTimeJoined;
                                }
                                if (allFactors.includes(multifactorauth_1.FactorIds.LINK_PHONE)) {
                                    factorIdToPhoneNumberMap[multifactorauth_1.FactorIds.LINK_PHONE] = phoneNumbersThatPasswordlessLoginMethodOrderedByTimeJoined;
                                }
                                return {
                                    status: "OK",
                                    factorIdToPhoneNumberMap
