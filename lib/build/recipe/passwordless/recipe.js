...
class Recipe extends recipeModule_1.default {
    constructor(recipeId, appInfo, isInServerlessEnv, config, ingredients) {
        ...
        this.createMagicLink = (input) => __awaiter(this, void 0, void 0, function* () {
            let userInputCode = this.config.getCustomUserInputCode !== undefined
                ? yield this.config.getCustomUserInputCode(input.userContext)
                : undefined;
            const codeInfo = yield this.recipeInterfaceImpl.createCode("email" in input
                ? {
                    email: input.email,
                    userInputCode,
            ...
            let magicLink = appInfo.websiteDomain.getAsStringDangerous() +
        ...
        this.signInUp = (input) => __awaiter(this, void 0, void 0, function* () {
            let codeInfo = yield this.recipeInterfaceImpl.createCode("email" in input
                ? {
                    email: input.email,
                    userContext: input.userContext,
                }
            ...
            let consumeCodeResponse = yield this.recipeInterfaceImpl.consumeCode(this.config.flowType === "MAGIC_LINK"
                ? {
