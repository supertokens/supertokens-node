...
function getRecipeInterface(stInstance, querier) {
    ...
    return {
        consumeCode: async function (input) {
            ...
            }, copyAndRemoveUserContextAndTenantId(input), input.userContext);
            ...
            return Object.assign(Object.assign({}, response), { consumedDevice: response.consumedDevice, createdNewRecipeUser: response.createdNewUser, user: updatedUser, recipeUserId: recipeUserIdAsObj });
        },
        ...
        updateUser: async function (input) {
            ...
            let response = await querier.sendPutRequest("/recipe/user", copyAndRemoveUserContextAndTenantId(input), {}, input.userContext);
            if (response.status !== "OK") {
                return response;
            }
            const user = await stInstance.getRecipeInstanceOrThrow("accountlinking").recipeInterfaceImpl.getUser({
