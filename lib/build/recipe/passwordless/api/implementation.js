    ...
    return {
        consumeCodePOST: function (input) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                // TODO: multiple pwless users linked?
                const loginMethod = user.loginMethods.find((m) => m.recipeId === "passwordless");
                ...
                const session = yield session_1.default.createNewSession(
                    ...
                    input.options.res,
                    input.tenantId,
                    new recipeUserId_1.default(user.id), // TODO: change to recipeUserId
                    {},
                    {},
                    input.userContext
                );
                        ...
                        ? {
                              ...
                              userInputCode:
                                  input.options.config.getCustomUserInputCode === undefined
                                      ? undefined
                                      : yield input.options.config.getCustomUserInputCode(
                                            input.tenantId,
                                            input.userContext
                                        ),
                              tenantId: input.tenantId,
                ...
                while (true) {
                    ...
                    let response = yield input.options.recipeImplementation.createNewCodeForDevice({
                        ...
                        userInputCode:
                            input.options.config.getCustomUserInputCode === undefined
                                ? undefined
                                : yield input.options.config.getCustomUserInputCode(input.tenantId, input.userContext),
                        tenantId: input.tenantId,
                    });
