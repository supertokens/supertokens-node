    ...
    return {
        consumeCodePOST: async function (input) {
            ...
            while (true) {
                try {
                    ...
                    existingUsers = existingUsers.filter((u) => u.loginMethods.some((m) => m.recipeId === "passwordless" &&
                    ...
                    if (mfaInstance === undefined) {
                        if (session === undefined) {
                            if (isSignIn) {
                                ...
                                let consumeCodeResponse = await input.options.recipeImplementation.consumeCode("deviceId" in input
                                    ? {
                                        preAuthSessionId: input.preAuthSessionId,
                                ...
                                await assertThatSignInIsAllowed(input.tenantId, consumeCodeResponse.user, input.userContext);
                                consumeCodeResponse.user = await recipe_1.default.getInstance().createPrimaryUserIdOrLinkAccounts({
                                    tenantId: input.tenantId,
                                    user: consumeCodeResponse.user,
                                    userContext: input.userContext,
                                });
                                session = await session_1.default.createNewSession(input.options.req, input.options.res, input.tenantId, consumeCodeResponse.recipeUserId, {}, {}, input.userContext);
                                return {
                                    status: "OK",
                                    session,
                                ...
                                await assertThatSignUpIsAllowed(input.tenantId, deviceInfo.email, deviceInfo.phoneNumber, input.userContext);
                                let consumeCodeResponse = await input.options.recipeImplementation.consumeCode("deviceId" in input
                                    ? {
                                        preAuthSessionId: input.preAuthSessionId,
                                        deviceId: input.deviceId,
                        ...
                        if (session === undefined) {
                            ...
                            if (isSignIn) {
                                ...
                                if (consumeCodeResponse.user.id !== sessionUser.id) {
                                    throw new SignInUpError({
                                        status: "SIGN_IN_UP_NOT_ALLOWED",
                                        reason: "Cannot complete MFA because of security reasons. Please contact support. (ERR_CODE_013)",
                                    });
                                }
                                await mfaInstance.recipeInterfaceImpl.markFactorAsCompleteInSession({
                                    session,
                                ...
                                await multifactorauth_1.default.assertAllowedToSetupFactorElseThrowInvalidClaimError(session, factorId, input.userContext);
                                await assertThatFactorUserBeingCreatedCanBeLinkedWithSessionUser(input.tenantId, sessionUser, { email: deviceInfo.email, phoneNumber: deviceInfo.phoneNumber }, input.userContext);
                                let consumeCodeResponse = await input.options.recipeImplementation.consumeCode("deviceId" in input
                                    ? {
                                        preAuthSessionId: input.preAuthSessionId,
                                        deviceId: input.deviceId,
                                ...
                                consumeCodeResponse.user = await linkAccountsForFactorSetup(sessionUser, consumeCodeResponse.recipeUserId, input.userContext);
                                await mfaInstance.recipeInterfaceImpl.markFactorAsCompleteInSession({
                                    session,
                                    factorId,
                                    userContext: input.userContext,
        ...
        createCodePOST: async function (input) {
            ...
            if (existingUsers.length === 0) {
                ...
                let loginMethod = existingUsers[0].loginMethods.find((m) => m.recipeId === "passwordless" &&
                    (m.hasSameEmailAs(accountInfo.email) || m.hasSamePhoneNumberAs(accountInfo.phoneNumber)));
