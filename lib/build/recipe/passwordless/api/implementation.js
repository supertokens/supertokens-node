...
function getAPIImplementation() {
    return {
        ...
        phoneNumberExistsGET: async function (input) {
            let users = await __1.listUsersByAccountInfo(input.tenantId, {
                phoneNumber: input.phoneNumber,
                // tenantId: input.tenantId,
            }, false, input.userContext);
            return {
        ...
        resendCodePOST: async function (input) {
            ...
            const userWithMatchingLoginMethod = await getPasswordlessUserByAccountInfo(Object.assign(Object.assign({}, input), { accountInfo: deviceInfo }));
            const authTypeInfo = await authUtils_1.AuthUtils.checkAuthTypeAndLinkingStatus(input.session, input.shouldTryLinkingWithSessionUser, {
                recipeId: "passwordless",
                email: deviceInfo.email,
                phoneNumber: deviceInfo.phoneNumber,
            ...
            while (true) {
                ...
                if (response.status === "OK") {
                    ...
                    if (!authTypeInfo.isFirstFactor) {
                        ...
                        factorIds = utils_1.getEnabledPwlessFactors(input.options.config);
                        factorIds = await authUtils_1.AuthUtils.filterOutInvalidFirstFactorsOrThrowIfAllAreInvalid(factorIds, input.tenantId, false, input.userContext);
                    }
...
async function getPasswordlessUserByAccountInfo(input) {
    ...
    logger_1.logDebugMessage(`getPasswordlessUserByAccountInfo got ${existingUsers.length} from core resp ${JSON.stringify(input.accountInfo)}`);
    ...
    logger_1.logDebugMessage(`getPasswordlessUserByAccountInfo ${usersWithMatchingLoginMethods.length} has matching login methods`);
