                ...
                for (const user of users) {
                    if (user.isPrimaryUser && user.id !== sessionUser.id) {
                        return {
                            status: "MFA_FLOW_ERROR",
                            reason:
                                "Cannot setup factor as the account info for factor being is already associated with another primary user. Please contact support. (ERR_CODE_012)",
                        };
                    }
                }
                        ...
                        for (const user of users) {
                            if (user.isPrimaryUser) {
                                return {
                                    status: "MFA_FLOW_ERROR",
                                    reason:
                                        "Cannot setup factor as the session user cannot become a primary user. Please contact support. (ERR_CODE_014)",
                                };
                            }
                        }
                    }
