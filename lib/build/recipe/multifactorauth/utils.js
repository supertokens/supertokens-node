        ...
        for (const lM of sessionUser.loginMethods) {
            if (lM.recipeUserId.getAsString() === sessionRecipeUserId.getAsString()) {
                if (lM.recipeId === "emailpassword") {
                    if (
                        await exports.isValidFirstFactor(tenantId, types_1.FactorIds.EMAILPASSWORD, input.userContext)
                    ) {
                        computedFirstFactorIdForSession = types_1.FactorIds.EMAILPASSWORD;
                        break;
                    }
                } else if (lM.recipeId === "thirdparty") {
                    if (await exports.isValidFirstFactor(tenantId, types_1.FactorIds.THIRDPARTY, input.userContext)) {
                    ...
                    if (lM.email !== undefined) {
                        if (
                            await exports.isValidFirstFactor(tenantId, types_1.FactorIds.LINK_EMAIL, input.userContext)
                        ) {
                            computedFirstFactorIdForSession = types_1.FactorIds.LINK_EMAIL;
                            break;
                        }
                        if (
                            await exports.isValidFirstFactor(tenantId, types_1.FactorIds.OTP_EMAIL, input.userContext)
                        ) {
                            computedFirstFactorIdForSession = types_1.FactorIds.LINK_EMAIL;
                            break;
                        }
                    ...
                    if (lM.phoneNumber !== undefined) {
                        if (
                            await exports.isValidFirstFactor(tenantId, types_1.FactorIds.LINK_PHONE, input.userContext)
                        ) {
                            computedFirstFactorIdForSession = types_1.FactorIds.LINK_EMAIL;
                            break;
                        }
                        if (
                            await exports.isValidFirstFactor(tenantId, types_1.FactorIds.OTP_PHONE, input.userContext)
                        ) {
                            computedFirstFactorIdForSession = types_1.FactorIds.LINK_EMAIL;
                            break;
                        }
