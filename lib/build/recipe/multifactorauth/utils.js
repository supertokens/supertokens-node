...
exports.updateAndGetMFARelatedInfoInSession = exports.validateAndNormaliseUserInput = void 0;
...
const updateAndGetMFARelatedInfoInSession = async function (input) {
        ...
        for (const lM of sessionUser.loginMethods) {
            if (lM.recipeUserId.getAsString() === sessionRecipeUserId.getAsString()) {
                if (lM.recipeId === "emailpassword") {
                    let validRes = await utils_1.isValidFirstFactor(tenantId, types_1.FactorIds.EMAILPASSWORD, input.userContext);
                    ...
                    let validRes = await utils_1.isValidFirstFactor(tenantId, types_1.FactorIds.THIRDPARTY, input.userContext);
                    if (validRes.status === "TENANT_NOT_FOUND_ERROR") {
                        throw new error_1.default({
                            type: error_1.default.UNAUTHORISED,
                            message: "Tenant not found",
    ...
    function userGetter() {
        ...
        userProm = __1.getUser(sessionRecipeUserId.getAsString(), input.userContext).then((sessionUser) => {
            if (sessionUser === undefined) {
                throw new error_1.default({
                    type: error_1.default.UNAUTHORISED,
                    message: "Session user not found",
    ...
    const mfaRequirementsForAuth = await recipe_1.default.getInstanceOrThrowError().recipeInterfaceImpl.getMFARequirementsForAuth({
        accessTokenPayload,
        tenantId,
        get user() {
            return userGetter();
        ...
        get factorsSetUpForUser() {
            return userGetter().then((user) => recipe_1.default.getInstanceOrThrowError().recipeInterfaceImpl.getFactorsSetupForUser({
                user,
    ...
    const areAuthReqsComplete = multiFactorAuthClaim_1.MultiFactorAuthClaim.getNextSetOfUnsatisfiedFactors(completedFactors, mfaRequirementsForAuth).factorIds
        .length === 0;
    ...
    if ("session" in input && updatedClaimVal) {
        await input.session.setClaimValue(multiFactorAuthClaim_1.MultiFactorAuthClaim, mfaClaimValue, input.userContext);
    }
    return {
        completedFactors,
