...
function getRecipeInterface(stInstance, recipeInstance) {
    return {
        ...
        assertAllowedToSetupFactorElseThrowInvalidClaimError: async function (input) {
            ...
            const validator = {
                ...
                validate: async (payload) => {
                    ...
                    const setOfUnsatisfiedFactors = recipeInstance.multiFactorAuthClaim.getNextSetOfUnsatisfiedFactors(claimVal.c, await input.mfaRequirementsForAuth);
                    ...
                    if (setOfUnsatisfiedFactors.factorIds.some((id) => factorsSetUpForUserRes.includes(id))) {
                        (0, logger_1.logDebugMessage)(`assertAllowedToSetupFactorElseThrowInvalidClaimError ${input.factorId}: false because there are items already set up in the next set of unsatisfied factors: ${setOfUnsatisfiedFactors.factorIds.join(", ")}`);
                        return {
                            isValid: false,
                            reason: "Completed factors in the session does not satisfy the MFA requirements for auth",
                        };
                    }
                    if (setOfUnsatisfiedFactors.factorIds.length > 0 &&
                        ...
                        (0, logger_1.logDebugMessage)(`assertAllowedToSetupFactorElseThrowInvalidClaimError ${input.factorId}: false because user is trying to set up factor that is not in the next set of unsatisfied factors: ${setOfUnsatisfiedFactors.factorIds.join(", ")}`);
                        return {
                            isValid: false,
                            reason: "Not allowed to setup factor that is not in the next set of unsatisfied factors",
                        };
                    }
                    (0, logger_1.logDebugMessage)(`assertAllowedToSetupFactorElseThrowInvalidClaimError ${input.factorId}: true because the next set of unsatisfied factors is ${setOfUnsatisfiedFactors.factorIds.length === 0 ? "empty" : "cannot be completed otherwise"}`);
                    return { isValid: true };
                },
            };
        ...
        removeFromRequiredSecondaryFactorsForUser: async function ({ userId, factorId, userContext }) {
            ...
            let factorIds = (_b = metadata.metadata._supertokens.requiredSecondaryFactors) !== null && _b !== void 0 ? _b : [];
            if (!factorIds.includes(factorId)) {
                return;
            }
