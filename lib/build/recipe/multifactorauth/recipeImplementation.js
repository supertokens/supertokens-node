...
function getRecipeInterface(recipeInstance) {
    return {
        ...
        assertAllowedToSetupFactorElseThrowInvalidClaimError: async function (input) {
            ...
            const validator = {
                ...
                validate: async (payload) => {
                    ...
                    const setOfUnsatisfiedFactors = multiFactorAuthClaim_1.MultiFactorAuthClaim.getNextSetOfUnsatisfiedFactors(claimVal.c, await input.mfaRequirementsForAuth);
                    ...
                    if (setOfUnsatisfiedFactors.factorIds.some((id) => factorsSetUpForUserRes.includes(id))) {
                        logger_1.logDebugMessage(`assertAllowedToSetupFactorElseThrowInvalidClaimError ${input.factorId}: false because there are items already set up in the next set of unsatisfied factors: ${setOfUnsatisfiedFactors.factorIds.join(", ")}`);
                        return {
                            isValid: false,
                    ...
                    if (setOfUnsatisfiedFactors.factorIds.length > 0 &&
                        ...
                        logger_1.logDebugMessage(`assertAllowedToSetupFactorElseThrowInvalidClaimError ${input.factorId}: false because user is trying to set up factor that is not in the next set of unsatisfied factors: ${setOfUnsatisfiedFactors.factorIds.join(", ")}`);
                        return {
                            isValid: false,
                            reason: "Not allowed to setup factor that is not in the next set of unsatisfied factors",
                        };
                    }
                    logger_1.logDebugMessage(`assertAllowedToSetupFactorElseThrowInvalidClaimError ${input.factorId}: true because the next set of unsatisfied factors is ${setOfUnsatisfiedFactors.factorIds.length === 0 ? "empty" : "cannot be completed otherwise"}`);
                    return { isValid: true };
                },
            };
        ...
        markFactorAsCompleteInSession: async function ({ session, factorId, userContext }) {
            await utils_1.updateAndGetMFARelatedInfoInSession({
                session,
