...
export default class Recipe extends RecipeModule {
    ...
    getIdentitiesForUser: (user: User) => {
        verified: {
            emails: string[];
            phoneNumbers: string[];
            thirdpartyInfo: {
    ...
    isSignUpAllowed: ({ info, userContext, }: {
        info: AccountInfoAndEmailWithRecipeId;
        userContext: any;
    }) => Promise<boolean>;
    doPostSignUpAccountLinkingOperations: ({ info, infoVerified, recipeUserId, userContext, }: {
        info: AccountInfoAndEmailWithRecipeId;
        infoVerified: boolean;
        recipeUserId: string;
        userContext: any;
    }) => Promise<string>;
    accountLinkPostSignInViaSession: ({ session, info, infoVerified, userContext, }: {
        ...
        accountsLinked: false;
        reason: "ACCOUNT_INFO_ALREADY_LINKED_WITH_ANOTHER_PRIMARY_USER_ID_ERROR" | "RECIPE_USER_ID_ALREADY_LINKED_WITH_ANOTHER_PRIMARY_USER_ID_ERROR";
        primaryUserId: string;
    }))>;
    getPrimaryUserIdThatCanBeLinkedToRecipeUserId: ({ recipeUserId, userContext, }: {
        recipeUserId: string;
        userContext: any;
