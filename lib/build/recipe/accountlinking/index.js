...
class Wrapper {
    ...
    static async createPrimaryUserIdOrLinkAccounts(tenantId, recipeUserId, session, userContext) {
        ...
        return await recipe_1.default.getInstance().createPrimaryUserIdOrLinkByAccountInfoOrLinkToSessionIfProvided({
            tenantId,
            user,
            recipeUserId,
            session,
     ...
     */
    static async getUsersThatCanBeLinkedToRecipeUser(tenantId, recipeUserId, userContext) {
        const user = await __1.getUser(recipeUserId.getAsString(), userContext);
        if (user === undefined) {
            // Should never really come here unless a programming error happened in the app
            throw new Error("Unknown recipeUserId");
        }
        return await recipe_1.default.getInstance().getUsersThatCanBeLinkedToRecipeUser({
            tenantId,
            user,
            userContext: utils_1.getUserContext(userContext),
        });
...
exports.getUsersThatCanBeLinkedToRecipeUser = Wrapper.getUsersThatCanBeLinkedToRecipeUser;
