...
class Wrapper {
    ...
    static async getPrimaryUserIdThatCanBeLinkedToRecipeUserId(input) {
        return await recipe_1.default.getInstance().getPrimaryUserIdThatCanBeLinkedToRecipeUserId({
            recipeUserId: input.recipeUserId,
            userContext: input.userContext === undefined ? {} : input.userContext,
        });
    }
    ...
    static async isSignUpAllowed(newUser, isVerified, userContext) {
    ...
    static async isSignInAllowed(recipeUserId, userContext) {
