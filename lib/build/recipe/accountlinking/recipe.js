            ...
            __awaiter(this, void 0, void 0, function* () {
                ...
                if (canCreatePrimaryUserId.status === "OK") {
                    ...
                    if (createPrimaryUserResult.status !== "OK") {
                        throw Error(
                            "should never come here. Error from createPrimaryUser: " + createPrimaryUserResult.status
                        );
                    }
                ...
                if (result.status === "OK") {
                    ...
                    if (info.email !== undefined) {
                        emailIdentityVerifiedForPrimaryUser = identitiesForPrimaryUser.verified.emails.includes(
                            info.email
                        );
                    }
                    if (info.email !== undefined && (emailIdentityVerifiedForPrimaryUser || infoVerified)) {
                        let recipeUserIdsForEmailVerificationUpdate = primaryUser.loginMethods
                            ...
                            .map((l) => l.email);
                        if (!infoVerified) {
                            recipeUserIdsForEmailVerificationUpdate.push(recipeUserId);
                        }
        ...
        this.accountLinkPostSignInViaSession = ({ session, info, infoVerified, userContext }) =>
            __awaiter(this, void 0, void 0, function* () {
                ...
                if (!user.isPrimaryUser) {
                    ...
                    let recipeUser = yield supertokens_1.default
                        .getInstanceOrThrowError()
                        ._getUserForRecipeId(user.id, recipeId);
                ...
                let existingRecipeUsersForInputInfo = yield this.recipeInterfaceImpl.listUsersByAccountInfo({
                    info: identifier,
                    userContext,
                });
                    ...
                    u.loginMethods.find((lU) => {
                        ...
                        if (info.recipeId === "thirdparty") {
                            if (info.thirdParty !== undefined) {
                                ...
                                return (
                                    lU.thirdParty.id === info.thirdParty.id &&
                                    lU.thirdParty.userId === info.thirdParty.userId
                                );
                            }
                            return false;
                        }
                ...
                if (recipeUserIdentifyingInfoIsAssociatedWithPrimaryUser) {
                    ...
                    if (info.email !== undefined && (emailIdentityVerifiedForPrimaryUser || infoVerified)) {
                        ...
                        (phoneNumberIdentityVerifiedForPrimaryUser || infoVerified)
                    ) {
