...
class Recipe extends recipeModule_1.default {
    constructor(recipeId, appInfo, config, _recipes, _ingredients) {
        ...
        this.getPrimaryUserThatCanBeLinkedToRecipeUserId = async ({ tenantId, user, userContext, }) => {
            ...
            logger_1.logDebugMessage(`getPrimaryUserThatCanBeLinkedToRecipeUserId found ${users.length} matching users`);
            let pUsers = users.filter((u) => u.isPrimaryUser);
            logger_1.logDebugMessage(`getPrimaryUserThatCanBeLinkedToRecipeUserId found ${pUsers.length} matching primary users`);
            if (pUsers.length > 1) {
        ...
        this.getOldestUserThatCanBeLinkedToRecipeUser = async ({ tenantId, user, userContext, }) => {
            ...
            logger_1.logDebugMessage(`getOldestUserThatCanBeLinkedToRecipeUser found ${users.length} matching users`);
        ...
        this.isSignInUpAllowedHelper = async ({ accountInfo, isVerified, session, tenantId, isSignIn, user, userContext, }) => {
            ...
            if (users.length === 0) {
                logger_1.logDebugMessage("isSignInUpAllowedHelper returning true because no user with given account info");
            ...
            if (users.length === 1 && isSignIn && user !== undefined && users[0].id === user.id) {
                logger_1.logDebugMessage("isSignInUpAllowedHelper returning true because this is sign in and there is only a single user with the given account info");
                return true;
            }
            ...
            if (primaryUsers.length === 0) {
                ...
                if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
                    logger_1.logDebugMessage("isSignInUpAllowedHelper returning true because account linking is disabled");
                    return true;
                }
                if (!shouldDoAccountLinking.shouldRequireVerification) {
                    logger_1.logDebugMessage("isSignInUpAllowedHelper returning true because dec does not require email verification");
                ...
                for (let i = 0; i < users.length; i++) {
                    ...
                    if (accountInfo.email !== undefined) {
                        if (currUser.loginMethods[0].hasSameEmailAs(accountInfo.email) &&
                            ...
                            logger_1.logDebugMessage("isSignInUpAllowedHelper found same email for another user and verified");
                    ...
                    if (accountInfo.phoneNumber !== undefined) {
                        if (currUser.loginMethods[0].hasSamePhoneNumberAs(accountInfo.phoneNumber) &&
                            currUser.loginMethods[0].verified) {
                            logger_1.logDebugMessage("isSignInUpAllowedHelper found same phone number for another user and verified");
                            thisIterationIsVerified = true;
                        }
                    }
                    if (!thisIterationIsVerified) {
                        ...
                        logger_1.logDebugMessage("isSignInUpAllowedHelper returning false cause one of the other recipe level users is not verified");
                        shouldAllow = false;
                        break;
                    }
                }
                ...
                if (!isVerified) {
                    logger_1.logDebugMessage("isSignInUpAllowedHelper returning false because new user's email is not verified, and primary user with the same email was found.");
                ...
                for (let i = 0; i < primaryUser.loginMethods.length; i++) {
                    ...
                    if (lM.email !== undefined) {
                        if (lM.hasSameEmailAs(accountInfo.email) && lM.verified) {
                            logger_1.logDebugMessage("isSignInUpAllowedHelper returning true cause found same email for primary user and verified");
                            return true;
                        }
                    }
                    if (lM.phoneNumber !== undefined) {
                        if (lM.hasSamePhoneNumberAs(accountInfo.phoneNumber) && lM.verified) {
                            logger_1.logDebugMessage("isSignInUpAllowedHelper returning true cause found same phone number for primary user and verified");
                            return true;
                        }
                    }
                }
                logger_1.logDebugMessage("isSignInUpAllowedHelper returning false cause primary user does not have the same email or phone number that is verified"
            ...
            for (const tenantId of inputUser.tenantIds) {
                ...
                if (inputUser.isPrimaryUser) {
                    ...
                    if (otherPrimaryUserForNewEmail.length !== 0) {
                        logger_1.logDebugMessage(`isEmailChangeAllowed: returning false cause email change will lead to two primary users having same email on ${tenantId}`);
                        return { allowed: false, reason: "PRIMARY_USER_CONFLICT" };
                    }
                    if (input.isVerified) {
                        logger_1.logDebugMessage(`isEmailChangeAllowed: can change on ${tenantId} cause input user is primary, new email is verified and doesn't belong to any other primary user`);
                        continue;
                    }
                    if (inputUser.loginMethods.some((lm) => lm.hasSameEmailAs(input.newEmail) && lm.verified)) {
                        logger_1.logDebugMessage(`isEmailChangeAllowed: can change on ${tenantId} cause input user is primary, new email is verified in another login method and doesn't belong to any other primary user`);
                        continue;
                    }
                    if (otherUsersWithNewEmail.length === 0) {
                        logger_1.logDebugMessage(`isEmailChangeAllowed: can change on ${tenantId} cause input user is primary and the new email doesn't belong to any other user (primary or non-primary)`);
                        continue;
                    }
                    ...
                    if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
                        logger_1.logDebugMessage(`isEmailChangeAllowed: can change on ${tenantId} cause linking is disabled`);
                        continue;
                    }
                    if (!shouldDoAccountLinking.shouldRequireVerification) {
                        logger_1.logDebugMessage(`isEmailChangeAllowed: can change on ${tenantId} cause linking is doesn't require email verification`);
                        continue;
                    }
                    logger_1.logDebugMessage(`isEmailChangeAllowed: returning false because the user hasn't verified the new email address and there exists another user with it on ${tenantId} and linking requires verification`);
                    return { allowed: false, reason: "ACCOUNT_TAKEOVER_RISK" };
                }
                else {
                    if (input.isVerified) {
                        logger_1.logDebugMessage(`isEmailChangeAllowed: can change on ${tenantId} cause input user is not a primary and new email is verified`);
                        continue;
                    }
                    if (inputUser.loginMethods[0].hasSameEmailAs(input.newEmail)) {
                        logger_1.logDebugMessage(`isEmailChangeAllowed: can change on ${tenantId} cause input user is not a primary and new email is same as the older one`);
                        continue;
                    }
                    if (otherPrimaryUserForNewEmail.length === 1) {
                        ...
                        if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
                            logger_1.logDebugMessage(`isEmailChangeAllowed: can change on ${tenantId} cause input user is not a primary there exists a primary user exists with the new email, but the dev does not have account linking enabled.`);
                            continue;
                        }
                        if (!shouldDoAccountLinking.shouldRequireVerification) {
                            logger_1.logDebugMessage(`isEmailChangeAllowed: can change on ${tenantId} cause input user is not a primary there exists a primary user exists with the new email, but the dev does not require email verification.`);
                            continue;
                        }
                        logger_1.logDebugMessage("isEmailChangeAllowed: returning false cause input user is not a primary there exists a primary user exists with the new email.");
                        return { allowed: false, reason: "ACCOUNT_TAKEOVER_RISK" };
                    }
                    logger_1.logDebugMessage(`isEmailChangeAllowed: can change on ${tenantId} cause input user is not a primary no primary user exists with the new email`);
                    continue;
                }
            }
            logger_1.logDebugMessage("isEmailChangeAllowed: returning true cause email change can happen on all tenants the user is part of");
            return { allowed: true };
        };
        ...
        {
            let builder = new supertokens_js_override_1.default(recipeImplementation_1.default(querier_1.Querier.getNewInstanceOrThrowError(recipeId), this.config, this));
    ...
    async shouldBecomePrimaryUser(user, tenantId, session, userContext) {
        ...
        if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
            logger_1.logDebugMessage("shouldBecomePrimaryUser returning false because shouldAutomaticallyLink is false");
            return false;
        }
        if (shouldDoAccountLinking.shouldRequireVerification && !user.loginMethods[0].verified) {
            logger_1.logDebugMessage("shouldBecomePrimaryUser returning false because shouldRequireVerification is true but the login method is not verified");
            return false;
        }
    ...
    async tryLinkingByAccountInfoOrCreatePrimaryUser({ inputUser, session, tenantId, userContext, }) {
        ...
        while (tries++ < 100) {
            ...
            if (primaryUserThatCanBeLinkedToTheInputUser !== undefined) {
                logger_1.logDebugMessage("tryLinkingByAccountInfoOrCreatePrimaryUser: got primary user we can try linking");
                ...
                if (!primaryUserThatCanBeLinkedToTheInputUser.loginMethods.some((lm) => lm.recipeUserId.getAsString() === inputUser.loginMethods[0].recipeUserId.getAsString())) {
                    ...
                    if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
                        logger_1.logDebugMessage("tryLinkingByAccountInfoOrCreatePrimaryUser: not linking because shouldAutomaticallyLink is false");
                    ...
                    if (shouldDoAccountLinking.shouldRequireVerification &&
                        ...
                        logger_1.logDebugMessage("tryLinkingByAccountInfoOrCreatePrimaryUser: not linking because shouldRequireVerification is true but the login method is not verified in the new or the primary user");
                        return { status: "NO_LINK" };
                    }
                    logger_1.logDebugMessage("tryLinkingByAccountInfoOrCreatePrimaryUser linking");
                    ...
                    if (linkAccountsResult.status === "OK") {
                        logger_1.logDebugMessage("tryLinkingByAccountInfoOrCreatePrimaryUser successfully linked");
                        ...
                        logger_1.logDebugMessage("tryLinkingByAccountInfoOrCreatePrimaryUser already linked to another user");
                        return {
                            status: "OK",
                        ...
                        logger_1.logDebugMessage("tryLinkingByAccountInfoOrCreatePrimaryUser linking failed because of a race condition");
            ...
            if (oldestUserThatCanBeLinkedToTheInputUser !== undefined &&
                ...
                logger_1.logDebugMessage("tryLinkingByAccountInfoOrCreatePrimaryUser: got an older user we can try linking");
                ...
                if (shouldMakeOlderUserPrimary) {
                    ...
                    if (createPrimaryUserResult.status ===
                        ...
                        logger_1.logDebugMessage("tryLinkingByAccountInfoOrCreatePrimaryUser: retrying because createPrimaryUser returned " +
                    ...
                    if (shouldDoAccountLinking.shouldRequireVerification && !inputUser.loginMethods[0].verified) {
                        logger_1.logDebugMessage("tryLinkingByAccountInfoOrCreatePrimaryUser: not linking because shouldRequireVerification is true but the login method is not verified");
                        return { status: "NO_LINK" };
                    }
                    logger_1.logDebugMessage("tryLinkingByAccountInfoOrCreatePrimaryUser linking");
                    let linkAccountsResult = await this.recipeInterfaceImpl.linkAccounts({
            ...
            logger_1.logDebugMessage("tryLinkingByAccountInfoOrCreatePrimaryUser: trying to make the current user primary");
