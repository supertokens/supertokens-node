...
class Recipe extends recipeModule_1.default {
    constructor(recipeId, appInfo, config, _recipes, _ingredients) {
        ...
        this.createPrimaryUserIdOrLinkAccounts = async ({ tenantId, recipeUserId, userContext }) => {
            let recipeUser = await this.recipeInterfaceImpl.getUser({
        ...
        this.getPrimaryUserIdThatCanBeLinkedToRecipeUserId = async ({ recipeUserId, userContext }) => {
        ...
        this.isSignInAllowed = async ({ recipeUserId, userContext }) => {
            ...
            try {
                ...
                let emailInfo = await recipe_1.default
                    .getInstanceOrThrowError()
                    .getEmailForRecipeUserId(user.loginMethods[0].recipeUserId, userContext);
                if (emailInfo.status === "OK") {
                    isVerified = await recipe_1.default.getInstanceOrThrowError().recipeInterfaceImpl.isEmailVerified({
        ...
        this.isSignUpAllowed = async ({ newUser, isVerified, userContext }) => {
        ...
        this.isSignInUpAllowedHelper = async ({ accountInfo, isVerified, userContext }) => {
