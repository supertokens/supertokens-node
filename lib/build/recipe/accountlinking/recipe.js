    ...
    constructor(recipeId, appInfo, config, _recipes, _ingredients) {
        ...
        this.createPrimaryUserIdOrLinkAccounts = async ({ tenantId, user, userContext, }) => {
            ...
            if (primaryUser === undefined) {
                ...
                // not a primary user.
                let shouldDoAccountLinking = await this.config.shouldDoAutomaticAccountLinking(user.loginMethods[0], undefined, tenantId, userContext);
                if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
                    logger_1.logDebugMessage("createPrimaryUserIdOrLinkAccounts not creating primary user because shouldAutomaticallyLink is false");
                    return user;
                }
                if (shouldDoAccountLinking.shouldRequireVerification && !user.loginMethods[0].verified) {
                    logger_1.logDebugMessage("createPrimaryUserIdOrLinkAccounts not creating primary user because shouldRequireVerification is true but the login method is not verified");
                    return user;
                }
                ...
                // we can use the 0 index cause this user is
                // not a primary user.
                let shouldDoAccountLinking = await this.config.shouldDoAutomaticAccountLinking(user.loginMethods[0], primaryUser, tenantId, userContext);
                if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
                    logger_1.logDebugMessage("createPrimaryUserIdOrLinkAccounts not linking because shouldAutomaticallyLink is false");
                    return user;
                }
                if (shouldDoAccountLinking.shouldRequireVerification && !user.loginMethods[0].verified) {
                    logger_1.logDebugMessage("createPrimaryUserIdOrLinkAccounts not linking because shouldRequireVerification is true but the login method is not verified");
                    return user;
                }
                ...
                if (linkAccountsResult.status === "OK") {
                    ...
                    logger_1.logDebugMessage("createPrimaryUserIdOrLinkAccounts linking failed because of a race condition");
        ...
        this.isSignInUpAllowedHelper = async ({ accountInfo, isVerified, tenantId, isSignIn, userContext, }) => {
            ...
            if (primaryUsers.length === 0) {
                ...
                let shouldDoAccountLinking = await this.config.shouldDoAutomaticAccountLinking(accountInfo, undefined, tenantId, userContext);
                if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
                ...
                let shouldDoAccountLinking = await this.config.shouldDoAutomaticAccountLinking(accountInfo, primaryUser, tenantId, userContext);
                if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
                    logger_1.logDebugMessage("isSignInUpAllowedHelper returning true because account linking is disabled");
                    return true;
                }
            ...
            for (const tenantId of user.tenantIds) {
                ...
                if (user.isPrimaryUser) {
                    ...
                    if (primaryUserForNewEmail.length === 1 && primaryUserForNewEmail[0].id !== user.id) {
                        logger_1.logDebugMessage("isEmailChangeAllowed: returning false cause email change will lead to two primary users having same email");
                        return false;
                    }
                    logger_1.logDebugMessage(`isEmailChangeAllowed: can change on ${tenantId} cause input user is primary and new email doesn't belong to any other primary user`);
                    continue;
                }
                else {
                    if (input.isVerified) {
                    ...
                    if (primaryUserForNewEmail.length === 1) {
                        let shouldDoAccountLinking = await this.config.shouldDoAutomaticAccountLinking(user.loginMethods[0], primaryUserForNewEmail[0], tenantId, input.userContext);
                        if (!shouldDoAccountLinking.shouldAutomaticallyLink) {
