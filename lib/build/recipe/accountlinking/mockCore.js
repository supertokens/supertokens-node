...
function mockLinkAccounts({ recipeUserId, primaryUserId }) {
    return __awaiter(this, void 0, void 0, function* () {
        let pUser = yield mockGetUser({ userId: primaryUserId });
        if (pUser === undefined) {
            throw new Error("Primary user does not exist");
        }
...
function createUserObject(input) {
    ...
    return Object.assign(Object.assign({}, input), {
        ...
        toJson: function () {
            const ret = Object.assign(Object.assign({}, this), {
                loginMethods: this.loginMethods.map((lM) => {
                    const ret = Object.assign(Object.assign({}, lM), { recipeUserId: lM.recipeUserId.getAsString() });
...
function isEmailVerified(userId, email) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let response = yield fetch(`http://localhost:8080/recipe/user/email/verify?userId=${userId}&email=${email}`, {
            headers: {
        ...
        if (accountInfo.email !== undefined) {
            // email password
            {
                ...
                if (respBody.status === "OK") {
                    let user = yield mockGetUser({ userId: respBody.user.id });
            ...
            // third party
            {
                let response = yield fetch(`http://localhost:8080/recipe/users/by-email?email=${accountInfo.email}`, {
                    headers: {
                        rid: "thirdparty",
                    },
                });
                ...
                if (respBody.status === "OK") {
                    for (let i = 0; i < respBody.users.length; i++) {
                        let user = yield mockGetUser({ userId: respBody.users[i].id });
                        let userAlreadyAdded = false;
                        for (let u of users) {
                            if (u.id === user.id) {
                                userAlreadyAdded = true;
            ...
            // passwordless
            {
                let response = yield fetch(`http://localhost:8080/recipe/user?email=${accountInfo.email}`, {
                    headers: {
                        rid: "passwordless",
                    },
                });
...
function mockGetUser({ userId }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let i = 0; i < allRecipeUserIds.length; i++) {
            ...
            // email password
            {
                let response = yield fetch(`http://localhost:8080/recipe/user?userId=${currUser}`, {
                    headers: {
                        rid: "emailpassword",
                    },
                });
