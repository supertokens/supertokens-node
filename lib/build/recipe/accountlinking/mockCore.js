...
function mockLinkAccounts({ recipeUserId, primaryUserId }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            let newAccountIsVerified = yield EmailVerification.isEmailVerified(recipeUserId);
            let user = yield mockGetUser({ userId: primaryUserId });
            let newAccountEmail = undefined;
                ...
                if (newAccountIsVerified) {
                    ...
                    for (let i = 0; i < user.loginMethods.length; i++) {
                        if (user.loginMethods[i].hasSameEmailAs(newAccountEmail) && !user.loginMethods[i].verified) {
                            let tokenResp = yield EmailVerification.createEmailVerificationToken(
                ...
                if (accountInfo.thirdParty !== undefined) {
                    if (
                        u.thirdParty.find((t) => {
                            var _a, _b;
                            return (
                                t.id === ((_a = accountInfo.thirdParty) === null || _a === void 0 ? void 0 : _a.id) &&
                                t.userId ===
                                    ((_b = accountInfo.thirdParty) === null || _b === void 0 ? void 0 : _b.userId)
                            );
                        }) === undefined
                    ) {
                        pass = false;
