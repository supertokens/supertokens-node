...
function getRecipeImplementation(querier, config, recipeInstance) {
    return {
        getUsers: async function ({ tenantId, timeJoinedOrder, limit, paginationToken, includeRecipeIds, query, userContext, }) {
            ...
            }, Object.assign({ includeRecipeIds: includeRecipeIdsStr, timeJoinedOrder: timeJoinedOrder, limit: limit, paginationToken: paginationToken }, query), userContext);
            return {
        ...
        canCreatePrimaryUser: async function ({ recipeUserId, userContext, }) {
            return await querier.sendGetRequest("/recipe/accountlinking/user/primary/check", {
                recipeUserId: recipeUserId.getAsString(),
            }, userContext);
        },
        createPrimaryUser: async function ({ recipeUserId, userContext, }) {
            let response = await querier.sendPostRequest("/recipe/accountlinking/user/primary", {
                recipeUserId: recipeUserId.getAsString(),
            }, userContext);
        ...
        canLinkAccounts: async function ({ recipeUserId, primaryUserId, userContext, }) {
            let result = await querier.sendGetRequest("/recipe/accountlinking/user/link/check", {
                recipeUserId: recipeUserId.getAsString(),
                primaryUserId,
            }, userContext);
            return result;
        ...
        linkAccounts: async function ({ recipeUserId, primaryUserId, userContext, }) {
            const accountsLinkingResult = await querier.sendPostRequest("/recipe/accountlinking/user/link", {
                recipeUserId: recipeUserId.getAsString(),
                primaryUserId,
            }, userContext);
            if (accountsLinkingResult.status === "RECIPE_USER_ID_ALREADY_LINKED_WITH_ANOTHER_PRIMARY_USER_ID_ERROR") {
                return Object.assign(Object.assign({}, accountsLinkingResult), { user: user_1.User.fromApi(accountsLinkingResult.user) });
            }
        ...
        unlinkAccount: async function ({ recipeUserId, userContext, }) {
            let accountsUnlinkingResult = await querier.sendPostRequest("/recipe/accountlinking/user/unlink", {
                recipeUserId: recipeUserId.getAsString(),
            }, userContext);
            return accountsUnlinkingResult;
        },
        getUser: async function ({ userId, userContext }) {
            let result = await querier.sendGetRequest("/user/id", {
                userId,
            }, userContext);
            if (result.status === "OK") {
        ...
        listUsersByAccountInfo: async function ({ tenantId, accountInfo, doUnionOfAccountInfo, userContext, }) {
            ...
            let result = await querier.sendGetRequest({
                ...
                webauthnCredentialId: (_c = accountInfo.webauthn) === null || _c === void 0 ? void 0 : _c.credentialId,
        ...
        deleteUser: async function ({ userId, removeAllLinkedAccounts, userContext, }) {
            return await querier.sendPostRequest("/user/remove", {
                userId,
