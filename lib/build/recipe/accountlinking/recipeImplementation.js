...
function getRecipeImplementation(querier, config, recipeInstance) {
    return {
        getUsers: async function ({ tenantId, timeJoinedOrder, limit, paginationToken, includeRecipeIds, query, userContext, }) {
            let includeRecipeIdsStr = undefined;
            if (includeRecipeIds !== undefined) {
                includeRecipeIdsStr = includeRecipeIds.join(",");
            }
            let response = await querier.sendGetRequest(new normalisedURLPath_1.default(`${tenantId !== null && tenantId !== void 0 ? tenantId : "public"}/users`), Object.assign({ includeRecipeIds: includeRecipeIdsStr, timeJoinedOrder: timeJoinedOrder, limit: limit, paginationToken: paginationToken }, query), userContext);
            return {
        ...
        canCreatePrimaryUser: async function ({ recipeUserId, userContext, }) {
            return await querier.sendGetRequest(new normalisedURLPath_1.default("/recipe/accountlinking/user/primary/check"), {
                recipeUserId: recipeUserId.getAsString(),
            }, userContext);
        },
        createPrimaryUser: async function ({ recipeUserId, userContext, }) {
            let response = await querier.sendPostRequest(new normalisedURLPath_1.default("/recipe/accountlinking/user/primary"), {
                recipeUserId: recipeUserId.getAsString(),
            }, userContext);
        ...
        canLinkAccounts: async function ({ recipeUserId, primaryUserId, userContext, }) {
            let result = await querier.sendGetRequest(new normalisedURLPath_1.default("/recipe/accountlinking/user/link/check"), {
                recipeUserId: recipeUserId.getAsString(),
                primaryUserId,
            }, userContext);
            return result;
        ...
        linkAccounts: async function ({ recipeUserId, primaryUserId, userContext, }) {
            const accountsLinkingResult = await querier.sendPostRequest(new normalisedURLPath_1.default("/recipe/accountlinking/user/link"), {
                recipeUserId: recipeUserId.getAsString(),
                primaryUserId,
            }, userContext);
            if (["OK", "RECIPE_USER_ID_ALREADY_LINKED_WITH_ANOTHER_PRIMARY_USER_ID_ERROR"].includes(accountsLinkingResult.status)) {
            ...
            if (accountsLinkingResult.status === "OK") {
                ...
                if (!accountsLinkingResult.accountsAlreadyLinked) {
                    ...
                    user = updatedUser;
                    let loginMethodInfo = user.loginMethods.find((u) => u.recipeUserId.getAsString() === recipeUserId.getAsString());
        ...
        unlinkAccount: async function ({ recipeUserId, userContext, }) {
            let accountsUnlinkingResult = await querier.sendPostRequest(new normalisedURLPath_1.default("/recipe/accountlinking/user/unlink"), {
                recipeUserId: recipeUserId.getAsString(),
            }, userContext);
        ...
        listUsersByAccountInfo: async function ({ tenantId, accountInfo, doUnionOfAccountInfo, userContext, }) {
            var _a, _b;
            let result = await querier.sendGetRequest(new normalisedURLPath_1.default(`${tenantId !== null && tenantId !== void 0 ? tenantId : "public"}/users/by-accountinfo`), {
