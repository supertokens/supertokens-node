...
class OpenIdRecipe extends recipeModule_1.default {
    ...
    static init(config) {
        return (appInfo, _isInServerlessEnv, plugins) => {
            if (OpenIdRecipe.instance === undefined) {
                OpenIdRecipe.instance = new OpenIdRecipe(OpenIdRecipe.RECIPE_ID, appInfo, (0, plugins_1.applyPlugins)(OpenIdRecipe.RECIPE_ID, config, plugins !== null && plugins !== void 0 ? plugins : []));
                return OpenIdRecipe.instance;
            }
            else {
    ...
    static async getIssuer(userContext) {
        return (await this.getInstanceOrThrowError().recipeImplementation.getOpenIdDiscoveryConfiguration({ userContext })).issuer;
    }
}
