...
function getRecipeInterface(querier, _config, appInfo, getDefaultAccessTokenPayload, getDefaultIdTokenPayload, getDefaultUserInfoPayload) {
    return {
        ...
        validateOAuth2AccessToken: async function (input) {
            var _a, _b, _c;
            const payload = (await jose.jwtVerify(input.token, combinedRemoteJWKSet_1.getCombinedJWKS(recipe_1.default.getInstanceOrThrowError().config))).payload;
            if (payload.stt !== 1) {
        ...
        endSession: async function (input) {
            ...
            if (logoutChallenge !== null) {
                ...
                if (input.session !== undefined || input.shouldTryRefresh) {
                    ...
                    redirectTo = (await this.acceptLogoutRequest({
