...
function getUpdatedRedirectTo(appInfo, redirectTo) {
    return redirectTo.replace("{apiDomain}", appInfo.apiDomain.getAsStringDangerous() + appInfo.apiBasePath.getAsStringDangerous());
}
...
function getRecipeInterface(querier, _config, appInfo, getDefaultAccessTokenPayload, getDefaultIdTokenPayload, getDefaultUserInfoPayload) {
    return {
        getLoginRequest: async function (input) {
            const resp = await querier.sendGetRequest(new normalisedURLPath_1.default("/recipe/oauth/auth/requests/login"), { loginChallenge: input.challenge }, input.userContext);
            if (resp.status !== "OK") {
                return {
                    status: "ERROR",
        ...
        acceptLoginRequest: async function (input) {
            const resp = await querier.sendPutRequest(new normalisedURLPath_1.default(`/recipe/oauth/auth/requests/login/accept`), {
                acr: input.acr,
                amr: input.amr,
        ...
        rejectLoginRequest: async function (input) {
            const resp = await querier.sendPutRequest(new normalisedURLPath_1.default(`/recipe/oauth/auth/requests/login/reject`), {
        ...
        getConsentRequest: async function (input) {
            const resp = await querier.sendGetRequest(new normalisedURLPath_1.default("/recipe/oauth/auth/requests/consent"), { consentChallenge: input.challenge }, input.userContext);
            return {
                acr: resp.acr,
                amr: resp.amr,
                challenge: resp.challenge,
        ...
        acceptConsentRequest: async function (input) {
            const resp = await querier.sendPutRequest(new normalisedURLPath_1.default(`/recipe/oauth/auth/requests/consent/accept`), {
                context: input.context,
        ...
        rejectConsentRequest: async function (input) {
            const resp = await querier.sendPutRequest(new normalisedURLPath_1.default(`/recipe/oauth/auth/requests/consent/reject`), {
                error: input.error.error,
                errorDescription: input.error.errorDescription,
                statusCode: input.error.statusCode,
            }, {
        ...
        authorization: async function (input) {
            ...
            const responseTypes = (_e = (_d = input.params.response_type) === null || _d === void 0 ? void 0 : _d.split(" ")) !== null && _e !== void 0 ? _e : [];
            if (input.session !== undefined) {
                ...
                const idToken = scopes.includes("openid") && (responseTypes.includes("id_token") || responseTypes.includes("code"))
                    ...
                    : {};
                const accessToken = responseTypes.includes("token") || responseTypes.includes("code")
            ...
            const resp = await querier.sendPostRequest(new normalisedURLPath_1.default(`/recipe/oauth/auth`), {
        ...
        tokenExchange: async function (input) {
            ...
            if (input.body.grant_type === "client_credentials") {
                let clientId = input.authorizationHeader !== undefined
                    ? (0, utils_1.decodeBase64)(input.authorizationHeader.replace(/^Basic /, "").trim()).split(":")[0]
                ...
                const scopes = (_b = (_a = input.body.scope) === null || _a === void 0 ? void 0 : _a.split(" ")) !== null && _b !== void 0 ? _b : [];
                const clientInfo = await this.getOAuth2Client({
                    clientId,
                    userContext: input.userContext,
                });
            ...
            if (input.body.grant_type === "refresh_token") {
                const scopes = (_d = (_c = input.body.scope) === null || _c === void 0 ? void 0 : _c.split(" ")) !== null && _d !== void 0 ? _d : [];
            ...
            const res = await querier.sendPostRequest(new normalisedURLPath_1.default(`/recipe/oauth/token`), body, input.userContext);
        ...
        getOAuth2Clients: async function (input) {
            let response = await querier.sendGetRequestWithResponseHeaders(new normalisedURLPath_1.default(`/recipe/oauth/clients/list`), {
        ...
        getOAuth2Client: async function (input) {
            let response = await querier.sendGetRequestWithResponseHeaders(new normalisedURLPath_1.default(`/recipe/oauth/clients`), { clientId: input.clientId }, {}, input.userContext);
            if (response.body.status === "OK") {
                return {
                    status: "OK",
        ...
        createOAuth2Client: async function (input) {
            let response = await querier.sendPostRequest(new normalisedURLPath_1.default(`/recipe/oauth/clients`), copyAndCleanRequestBodyInput(input), input.userContext);
        ...
        updateOAuth2Client: async function (input) {
            let response = await querier.sendPutRequest(new normalisedURLPath_1.default(`/recipe/oauth/clients`), copyAndCleanRequestBodyInput(input), { clientId: input.clientId }, input.userContext);
            if (response.status === "OK") {
                return {
                    status: "OK",
                    client: OAuth2Client_1.OAuth2Client.fromAPIResponse(response),
        ...
        deleteOAuth2Client: async function (input) {
            let response = await querier.sendPostRequest(new normalisedURLPath_1.default(`/recipe/oauth/clients/remove`), { clientId: input.clientId }, input.userContext);
            if (response.status === "OK") {
                return { status: "OK" };
            }
            else {
        ...
        validateOAuth2AccessToken: async function (input) {
            var _a, _b, _c;
            const payload = (await jose.jwtVerify(input.token, (0, combinedRemoteJWKSet_1.getCombinedJWKS)(recipe_1.default.getInstanceOrThrowError().config))).payload;
            if (payload.stt !== 1) {
                throw new Error("Wrong token type");
            }
            if (((_a = input.requirements) === null || _a === void 0 ? void 0 : _a.clientId) !== undefined && payload.client_id !== input.requirements.clientId) {
                throw new Error(`The token doesn't belong to the specified client (${input.requirements.clientId} !== ${payload.client_id})`);
            }
            if (((_b = input.requirements) === null || _b === void 0 ? void 0 : _b.scopes) !== undefined &&
            ...
            if (((_c = input.requirements) === null || _c === void 0 ? void 0 : _c.audience) !== undefined && !aud.includes(input.requirements.audience)) {
                throw new Error("The token doesn't belong to the specified audience");
            }
            if (input.checkDatabase) {
                let response = await querier.sendPostRequest(new normalisedURLPath_1.default(`/recipe/oauth/introspect`), {
                    token: input.token,
                }, input.userContext);
        ...
        revokeToken: async function (input) {
            ...
            const res = await querier.sendPostRequest(new normalisedURLPath_1.default(`/recipe/oauth/token/revoke`), requestBody, input.userContext);
            if (res.status !== "OK") {
                return {
                    status: "ERROR",
                    statusCode: res.statusCode,
        ...
        revokeTokensBySessionHandle: async function (input) {
            await querier.sendPostRequest(new normalisedURLPath_1.default(`/recipe/oauth/session/revoke`), { sessionHandle: input.sessionHandle }, input.userContext);
            return { status: "OK" };
        },
        revokeTokensByClientId: async function (input) {
            await querier.sendPostRequest(new normalisedURLPath_1.default(`/recipe/oauth/tokens/revoke`), { clientId: input.clientId }, input.userContext);
            return { status: "OK" };
        },
        introspectToken: async function ({ token, scopes, userContext }) {
            ...
            const res = await querier.sendPostRequest(new normalisedURLPath_1.default(`/recipe/oauth/introspect`), {
                token,
        ...
        endSession: async function (input) {
             ...
             *        - Redirects to the `post_logout_redirect_uri` or the default logout fallback page.
             */
            const resp = await querier.sendGetRequest(new normalisedURLPath_1.default(`/recipe/oauth/sessions/logout`), {
                clientId: input.params.client_id,
        ...
        acceptLogoutRequest: async function (input) {
            const resp = await querier.sendPutRequest(new normalisedURLPath_1.default(`/recipe/oauth/auth/requests/logout/accept`), { challenge: input.challenge }, {}, input.userContext);
            if (resp.status !== "OK") {
                return {
                    status: "ERROR",
                    statusCode: resp.statusCode,
        ...
        rejectLogoutRequest: async function (input) {
            const resp = await querier.sendPutRequest(new normalisedURLPath_1.default(`/recipe/oauth/auth/requests/logout/reject`), {}, { challenge: input.challenge }, input.userContext);
            if (resp.status != "OK") {
