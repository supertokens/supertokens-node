...
async function login(stInstance, apiImplementation, options, userContext) {
    var _a, _b;
    ...
    const loginChallenge = (_a = options.req.getKeyValueFromQuery("login_challenge")) !== null && _a !== void 0 ? _a : options.req.getKeyValueFromQuery("loginChallenge");
    ...
    if ("frontendRedirectTo" in response) {
        if (response.cookies) {
            ...
            for (const cookie of cookies) {
                options.res.setCookie(cookie.name, cookie.value, cookie.domain, !!cookie.secure, !!cookie.httpOnly, new Date(cookie.expires).getTime(), cookie.path || "/", cookie.sameSite);
            }
        }
