...
async function loginGET({
    ...
    const sessionInfo =
        session !== undefined
            ? await stInstance.getRecipeInstanceOrThrow("session").recipeInterfaceImpl.getSessionInformation({
                  sessionHandle: session === null || session === void 0 ? void 0 : session.getHandle(),
                  userContext,
              })
            : undefined;
    if (!sessionInfo) {
