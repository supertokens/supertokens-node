...
async function loginGET({ recipeImplementation, loginChallenge, shouldTryRefresh, session, cookies, isDirectCall, userContext, }) {
    var _a, _b;
    ...
    const sessionInfo = session !== undefined ? await (0, session_1.getSessionInformation)(session === null || session === void 0 ? void 0 : session.getHandle()) : undefined;
    if (!sessionInfo) {
        session = undefined;
    }
...
async function handleLoginInternalRedirects({ response, recipeImplementation, session, shouldTryRefresh, cookie = "", userContext, }) {
    var _a;
