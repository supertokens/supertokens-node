...
exports.handleInternalRedirects = exports.logoutPOST = exports.logoutGET = exports.loginGET = void 0;
...
async function logoutGET({ logoutChallenge, recipeImplementation, session, userContext }) {
...
async function logoutPOST({ logoutChallenge, session, recipeImplementation, userContext }) {
    if (session != undefined) {
...
function isInternalRedirect(redirectTo) {
    ...
    return [
        ...
        constants_2.END_SESSION_PATH,
        constants_2.LOGOUT_PATH,
        constants_2.LOGIN_PATH.replace("oauth", "oauth2"),
        constants_2.AUTH_PATH.replace("oauth", "oauth2"),
        constants_2.END_SESSION_PATH.replace("oauth", "oauth2"),
        constants_2.LOGOUT_PATH.replace("oauth", "oauth2"),
...
async function handleInternalRedirects({ response, recipeImplementation, session, cookie = "", userContext }) {
    ...
    while (redirectCount < maxRedirects && isInternalRedirect(response.redirectTo)) {
        ...
        } else if (response.redirectTo.includes(constants_2.END_SESSION_PATH)) {
            response = await recipeImplementation.endSession({
                params: Object.fromEntries(params.entries()),
                userContext,
            });
        } else if (response.redirectTo.includes(constants_2.LOGOUT_PATH)) {
            const logoutChallenge =
                (_b = params.get("logout_challenge")) !== null && _b !== void 0 ? _b : params.get("logoutChallenge");
            if (!logoutChallenge) {
