    ...
    return {
        ...
        createOrUpdateTenant: async function ({ tenantId, config, userContext }) {
            let response = await querier.sendPutRequest(
                new normalisedURLPath_1.default(`/recipe/multitenancy/tenant/v2`),
        ...
        listAllTenants: async function ({ userContext }) {
            let response = await querier.sendGetRequest(
                new normalisedURLPath_1.default(`/recipe/multitenancy/tenant/list/v2`),
                {},
                userContext
            );
            return response;
        ...
        getTenantCoreConfig: async function ({ tenantId, userContext }) {
            let response = await querier.sendGetRequest(
                new normalisedURLPath_1.default(
                    `/${
                        tenantId === undefined ? constants_1.DEFAULT_TENANT_ID : tenantId
                    }/recipe/multitenancy/tenant/core-config`
                ),
                {},
                userContext
            );
