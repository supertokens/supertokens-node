...
const isValidFirstFactor = async function (tenantId, factorId, userContext) {
    ...
    const firstFactorsFromMFA =
        (_a = MultiFactorAuthRecipe.getInstance()) === null || _a === void 0 ? void 0 : _a.config.firstFactors;
    logger_1.logDebugMessage(
        `isValidFirstFactor got ${
            (_b = tenantConfig.firstFactors) === null || _b === void 0 ? void 0 : _b.join(", ")
        } from tenant config`
    );
    ...
    if (configuredFirstFactors === undefined) {
        ...
        if (tenantConfig.emailPassword.enabled === false) {
            configuredFirstFactors = configuredFirstFactors.filter((factorId) => factorId !== FactorIds.EMAILPASSWORD);
        }
        ...
        if (tenantConfig.thirdParty.enabled === false) {
            configuredFirstFactors = configuredFirstFactors.filter((factorId) => factorId !== FactorIds.THIRDPARTY);
        }
