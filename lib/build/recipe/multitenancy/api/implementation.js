...
function getAPIInterface(stInstance) {
    return {
        loginMethodsGET: async function ({ tenantId, clientType, options, userContext }) {
            ...
            const mergedProviders = (0, configUtils_1.mergeProvidersFromCoreAndStatic)(providerConfigsFromCore, providerInputsFromStatic, tenantId === constants_1.DEFAULT_TENANT_ID);
            const finalProviderList = [];
            for (const providerInput of mergedProviders) {
                try {
                    const providerInstance = await (0, configUtils_1.findAndCreateProviderInstance)(tenantId, mergedProviders, providerInput.config.thirdPartyId, clientType, userContext);
            ...
            return {
                ...
                passwordless: {
                    enabled: validFirstFactors.includes("otp-email") ||
