...
function getAPIInterface() {
    return {
        loginMethodsGET: async function ({ tenantId, clientType, options, userContext }) {
            ...
            const mergedProviders = configUtils_1.mergeProvidersFromCoreAndStatic(providerConfigsFromCore, providerInputsFromStatic);
            const finalProviderList = [];
            for (const providerInput of mergedProviders) {
                try {
                    const providerInstance = await configUtils_1.findAndCreateProviderInstance(mergedProviders, providerInput.config.thirdPartyId, clientType, userContext);
            ...
            if (tenantConfigRes.firstFactors !== undefined) {
                ...
                if (tenantConfigRes.passwordless.enabled === false) {
                    firstFactors = firstFactors.filter((factor) => ![multifactorauth_1.FactorIds.OTP_EMAIL, multifactorauth_1.FactorIds.OTP_PHONE, multifactorauth_1.FactorIds.LINK_EMAIL, multifactorauth_1.FactorIds.LINK_PHONE].includes(factor));
                }
            }
            return {
                status: "OK",
