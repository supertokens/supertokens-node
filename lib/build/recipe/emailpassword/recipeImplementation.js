...
function getRecipeInterface(querier, getEmailPasswordConfig) {
    return {
        ...
        signIn: async function ({ email, password, tenantId, userContext }) {
            ...
            if (response.status === "OK") {
                ...
                if (!loginMethod.verified) {
                    ...
                    response.user = await __1.getUser(response.recipeUserId.getAsString(), userContext, true);
                }
            }
            return response;
        },
        ...
        updateEmailOrPassword: async function (input) {
            ...
            if (response.status === "OK") {
                const user = await __1.getUser(input.recipeUserId.getAsString(), input.userContext, true);
                if (user === undefined) {
