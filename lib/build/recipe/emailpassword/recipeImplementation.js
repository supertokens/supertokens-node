...
function getRecipeInterface(querier, getEmailPasswordConfig) {
    return {
        signUp: async function ({ email, password, tenantId, shouldAttemptAccountLinkingIfAllowed, userContext, }) {
        ...
        createNewRecipeUser: async function (input) {
            const resp = await querier.sendPostRequest(new normalisedURLPath_1.default(`/${input.tenantId === undefined ? constants_2.DEFAULT_TENANT_ID : input.tenantId}/recipe/signup`), {
                email: input.email,
        ...
        signIn: async function ({ email, password, tenantId, userContext, }) {
            const response = await querier.sendPostRequest(new normalisedURLPath_1.default(`/${tenantId === undefined ? constants_2.DEFAULT_TENANT_ID : tenantId}/recipe/signin`), {
                email,
                password,
            }, userContext);
            if (response.status === "OK") {
        ...
        createResetPasswordToken: async function ({ userId, email, tenantId, userContext, }) {
            ...
            return await querier.sendPostRequest(new normalisedURLPath_1.default(`/${tenantId === undefined ? constants_2.DEFAULT_TENANT_ID : tenantId}/recipe/user/password/reset/token`), {
                userId,
                email,
            }, userContext);
        },
        consumePasswordResetToken: async function ({ token, tenantId, userContext, }) {
            return await querier.sendPostRequest(new normalisedURLPath_1.default(`/${tenantId === undefined ? constants_2.DEFAULT_TENANT_ID : tenantId}/recipe/user/password/reset/token/consume`), {
                method: "token",
                token,
            }, userContext);
        },
        updateEmailOrPassword: async function (input) {
            ...
            let response = await querier.sendPutRequest(new normalisedURLPath_1.default(`${input.tenantIdForPasswordPolicy}/recipe/user`), {
