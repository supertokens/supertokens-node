...
async function signInAPI(apiImplementation, tenantId, options, userContext) {
    ...
    // step 1
    let formFields = await utils_2.validateFormFieldsOrThrowError(options.config.signInFeature.formFields, (await options.req.getJSONBody()).formFields, tenantId, userContext);
    ...
    if (result.status === "OK") {
        utils_1.send200Response(options.res, Object.assign({ status: "OK" }, utils_1.getBackwardsCompatibleUserInfo(options.req, result)));
    }
    else {
