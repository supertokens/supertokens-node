    ...
    return {
        linkAccountToExistingAccountPOST: function ({ formFields, session, options, userContext, }) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                let result = yield accountlinking_1.default.accountLinkPostSignInViaSession(session, {
                    email,
                ...
                if (result.createRecipeUser) {
                    ...
                    if (result.updateVerificationClaim) {
                        ...
                        result = yield accountlinking_1.default.accountLinkPostSignInViaSession(session, {
                            email,
                            recipeId: "emailpassword",
                        }, false, userContext);
                    }
                ...
                if (recipeUser === undefined) {
                    throw Error("this error should never be thrown. Can't find emailPassword recipeUser with email: " +
                        email +
                        "  and primary userId: " +
                        session.getUserId());
                }
                    ...
                    if (primaryUser !== undefined) {
                        ...
                        if (epUser === undefined) {
                            ...
                            if (epRecipeUser === undefined) {
                                 ...
                                 */
                                let shouldDoAccountLinking = yield accountlinking_1.default.shouldDoAutomaticAccountLinking({
                                    email,
                                    recipeId: "emailpassword",
                                ...
                                let identitiesForPrimaryUser = accountlinking_1.default.getIdentitiesForUser(primaryUser);
                                /**
                ...
                if (response.status === "OK") {
                    ...
                    if (user.isPrimaryUser) {
                        ...
                        if (epUser === undefined) {
                            ...
                            yield accountlinking_1.default.linkAccounts(recipeUser.id, user.id, userContext);
                        }
                        ...
                        yield accountlinking_1.default.createPrimaryUserIdOrLinkAccounts(user.id, session, userContext);
                    }
                }
                return response;
            });
        ...
        signInPOST: function ({ formFields, options, userContext, }) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                let session = yield session_1.default.createNewSession(options.req, options.res, user.id, user.recipeUserId, {}, {}, userContext);
                return {
                    status: "OK",
                    session,
                    user,
