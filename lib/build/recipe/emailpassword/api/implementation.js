...
function getAPIImplementation() {
    return {
        emailExistsGET: async function ({ email, userContext, tenantId }) {
            ...
            if (existingUser.isPrimaryUser) {
                ...
                if (emailPasswordUserIsLinkedToExistingUser) {
                    ...
                    if (createUserResponse.status === "EMAIL_ALREADY_EXISTS_ERROR") {
                        ...
                        // email is shared.
                        let linkedToUserId = await recipe_1.default.getInstance().createPrimaryUserIdOrLinkAccounts({
                            tenantId,
                        ...
                        return {
                            status: "OK",
                            email: tokenConsumptionResponse.email,
                            user: await __1.getUser(linkedToUserId, userContext), // we refetch cause we want to return the user object with the updated login methods.
                        };
                    }
                }
            } else {
        ...
        signInPOST: async function ({ formFields, tenantId, options, userContext }) {
            ...
            response.user = await __1.getUser(userId, userContext);
