    ...
    return {
        emailExistsGET: async function ({ email, tenantId, userContext, }) {
            ...
            let emailPasswordUserExists = users.find((u) => {
                return (u.loginMethods.find((lm) => lm.recipeId === "emailpassword" && lm.hasSameEmailAs(email)) !==
                    undefined);
            }) !== undefined;
            return {
                status: "OK",
        ...
        generatePasswordResetTokenPOST: async function ({ formFields, tenantId, options, userContext, }) {
            ...
            async function generateAndSendPasswordResetToken(primaryUserId, recipeUserId) {
                ...
                if (response.status === "UNKNOWN_USER_ID_ERROR") {
                    logger_1.logDebugMessage(`Password reset email not sent, unknown user id: ${recipeUserId === undefined ? primaryUserId : recipeUserId.getAsString()}`);
                    return {
                        status: "OK",
                    };
                }
            ...
            for (let i = 0; i < users.length; i++) {
                let emailPasswordAccountTmp = users[i].loginMethods.find((l) => l.recipeId === "emailpassword" && l.hasSameEmailAs(email));
            ...
            logger_1.logDebugMessage("generatePasswordResetTokenPOST: primary linking candidate: " + (linkingCandidate === null || linkingCandidate === void 0 ? void 0 : linkingCandidate.id));
            logger_1.logDebugMessage("generatePasswordResetTokenPOST: linking candidate count " + users.length);
            ...
            if (linkingCandidate === undefined && users.length > 0) {
                ...
                if (users.length === 1 &&
                    users[0].loginMethods[0].recipeUserId.getAsString() ===
                        (emailPasswordAccount === null || emailPasswordAccount === void 0 ? void 0 : emailPasswordAccount.recipeUserId.getAsString())) {
                    return await generateAndSendPasswordResetToken(emailPasswordAccount.recipeUserId.getAsString(), emailPasswordAccount.recipeUserId);
                }
                const oldestUser = users.sort((a, b) => a.timeJoined - b.timeJoined)[0];
                logger_1.logDebugMessage(`generatePasswordResetTokenPOST: oldest recipe level-linking candidate: ${oldestUser.id} (w/ ${oldestUser.loginMethods[0].verified ? "verified" : "unverified"} email)`);
                // Otherwise, we check if the user can become primary.
                const shouldBecomePrimaryUser = await recipe_1.default.getInstance().shouldBecomePrimaryUser(oldestUser, tenantId, undefined, userContext);
                logger_1.logDebugMessage(`generatePasswordResetTokenPOST: recipe level-linking candidate ${shouldBecomePrimaryUser ? "can" : "can not"} become primary`);
            ...
            */
            ...
            let emailVerified = linkingCandidate.loginMethods.find((lm) => {
            ...
            let hasOtherEmailOrPhone = linkingCandidate.loginMethods.find((lm) => {
            ...
            if (linkingCandidate.isPrimaryUser && emailPasswordAccount !== undefined) {
                ...
                let areTheTwoAccountsLinked = linkingCandidate.loginMethods.find((lm) => {
            ...
            let shouldDoAccountLinkingResponse = await recipe_1.default.getInstance().config.shouldDoAutomaticAccountLinking(emailPasswordAccount !== undefined
                ? emailPasswordAccount
                : {
                    recipeId: "emailpassword",
                    email,
                }, linkingCandidate, undefined, tenantId, userContext);
            ...
            if (emailPasswordAccount === undefined) {
                ...
                if (!shouldDoAccountLinkingResponse.shouldAutomaticallyLink) {
                    logger_1.logDebugMessage(`Password reset email not sent, since email password user didn't exist, and account linking not enabled`);
                    return {
                        status: "OK",
                    };
                }
                ...
                if (isSignUpAllowed) {
                    ...
                    logger_1.logDebugMessage(`Password reset email not sent, isSignUpAllowed returned false for email: ${email}`);
                    return {
                        status: "OK",
                    };
                }
        ...
        passwordResetPOST: async function ({ formFields, token, tenantId, options, userContext, }) {
            ...
            async function doUpdatePasswordAndVerifyEmailAndTryLinkIfNotPrimary(recipeUserId) {
                ...
                if (updateResponse.status === "EMAIL_ALREADY_EXISTS_ERROR" ||
            ...
            let existingUser = await __1.getUser(userIdForWhomTokenWasGenerated, userContext);
            ...
            if (tokenGeneratedForEmailPasswordUser) {
                if (!existingUser.isPrimaryUser) {
                    ...
                    return doUpdatePasswordAndVerifyEmailAndTryLinkIfNotPrimary(new recipeUserId_1.default(userIdForWhomTokenWasGenerated));
                }
                ...
                // If that is the case, then it's proven that the user owns the email and we can
                // trust linking of the email password account.
                let emailVerified = existingUser.loginMethods.find((lm) => {
                ...
                // associated with this account - and if it does, then it means that
                // there is a risk of account takeover, so we do not allow the token to be generated
                let hasOtherEmailOrPhone = existingUser.loginMethods.find((lm) => {
                    // we do the extra undefined check below cause
                    // hasSameEmailAs returns false if the lm.email is undefined, and
                    // we want to check that the email is different as opposed to email
                    // not existing in lm.
                    return ((lm.email !== undefined && !lm.hasSameEmailAs(emailForWhomTokenWasGenerated)) ||
        ...
        signInPOST: async function ({ formFields, tenantId, session, shouldTryLinkingWithSessionUser, options, userContext, }) {
            const errorCodeMap = {
            ...
            const checkCredentialsOnTenant = async (tenantId) => {
                return ((await options.recipeImplementation.verifyCredentials({ email, password, tenantId, userContext }))
                    .status === "OK");
            };
        ...
        signUpPOST: async function ({ formFields, tenantId, session, shouldTryLinkingWithSessionUser, options, userContext, }) {
            const errorCodeMap = {
            ...
            if (preAuthCheckRes.status === "SIGN_UP_NOT_ALLOWED") {
                ...
                if (conflictingUsers.some((u) => u.loginMethods.some((lm) => lm.recipeId === "emailpassword" && lm.hasSameEmailAs(email)))) {
                    return {
