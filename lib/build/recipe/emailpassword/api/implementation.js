    ...
    return {
        ...
        generatePasswordResetTokenPOST: async function ({ formFields, tenantId, options, userContext }) {
            ...
            if (linkingCandidate === undefined && users.length === 1) {
                ...
                const shouldBecomePrimaryUser = await recipe_1.default
                    .getInstance()
                    .shouldBecomePrimaryUser(users[0], tenantId, undefined, userContext);
                logger_1.logDebugMessage(
        ...
        passwordResetPOST: async function ({ formFields, token, tenantId, options, userContext }) {
            ...
            if (!existingUser.isPrimaryUser) {
                ...
                const shouldBecomePrimary = await recipe_1.default
                    .getInstance()
                    .shouldBecomePrimaryUser(existingUser, tenantId, undefined, userContext);
                if (shouldBecomePrimary) {
                    const createPrimaryResp = await accountlinking_1.createPrimaryUser(
