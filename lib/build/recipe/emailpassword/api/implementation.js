        ...
        generatePasswordResetTokenPOST: async function ({ formFields, tenantId, options, userContext, }) {
            ...
            let shouldDoAccountLinkingResponse = await recipe_1.default.getInstance().config.shouldDoAutomaticAccountLinking(emailPasswordAccount !== undefined
                ...
                : {
                    ...
                    email,
                }, primaryUserAssociatedWithEmail, tenantId, userContext);
            ...
            if (areTheTwoAccountsLinked) {
                return await generateAndSendPasswordResetToken(primaryUserAssociatedWithEmail.id, emailPasswordAccount.recipeUserId);
            }
            ...
            try {
                ...
                if (mfaInstance === undefined) {
                    if (session === undefined) {
                        ...
                        session = await session_1.default.createNewSession(options.req, options.res, tenantId, signInResponse.recipeUserId, {}, {}, userContext);
                        return {
                            status: "OK",
                            session,
                            user: signInResponse.user,
                    ...
                    if (session === undefined) {
                        ...
                        if (!(await utils_2.isValidFirstFactor(tenantId, "emailpassword", userContext))) {
                        ...
                        await assertThatSignInIsAllowed(tenantId, signInResponse.user, userContext);
                        signInResponse.user = await await recipe_1.default.getInstance().createPrimaryUserIdOrLinkAccounts({
                            tenantId,
                            user: signInResponse.user,
                            userContext,
                        });
                        session = await session_1.default.createNewSession(options.req, options.res, tenantId, signInResponse.recipeUserId, {}, {}, userContext);
                        await multifactorauth_1.default.markFactorAsCompleteInSession(session, "emailpassword", userContext);
                        return {
                            status: "OK",
                            session,
                            user: signInResponse.user,
                ...
                try {
                    ...
                    if (mfaInstance === undefined) {
                        if (session === undefined) {
                            ...
                            session = await session_1.default.createNewSession(options.req, options.res, tenantId, signUpResponse.recipeUserId, {}, {}, userContext);
                            return {
                                status: "OK",
                                session,
                        ...
                        if (session === undefined) {
                            ...
                            await multifactorauth_1.default.assertAllowedToSetupFactorElseThrowInvalidClaimError(session, "emailpassword", userContext);
                            await assertThatFactorUserBeingCreatedCanBeLinkedWithSessionUser(tenantId, sessionUser, { email }, userContext);
                            ...
                            signUpResponse.user = await linkAccountsForFactorSetup(sessionUser, signUpResponse.recipeUserId, userContext);
                            await multifactorauth_1.default.markFactorAsCompleteInSession(session, "emailpassword", userContext);
                            return {
                                status: "OK",
                                session,
