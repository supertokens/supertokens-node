...
function getAPIImplementation() {
    return {
        ...
        generatePasswordResetTokenPOST: async function ({ formFields, tenantId, options, session, userContext }) {
        ...
        passwordResetPOST: async function ({ formFields, token, session, tenantId, options, userContext }) {
            ...
            if (existingUser.isPrimaryUser) {
                ...
                if (emailPasswordUserIsLinkedToExistingUser) {
                    ...
                    if (createUserResponse.status === "EMAIL_ALREADY_EXISTS_ERROR") {
                        ...
                        // email is shared.
                        const linkRes = await authUtils_1.AuthUtils.linkToSessionIfProvidedElseCreatePrimaryUserIdOrLinkByAccountInfo(
                            {
                                tenantId,
        ...
        signUpPOST: async function ({ formFields, tenantId, session, options, userContext }) {
            ...
            const res = await authUtils_1.AuthUtils.preAuthChecks({
                accountInfo: {
                    recipeId: "emailpassword",
                    email,
                },
            ...
            if (res.status !== "OK") {
                return authUtils_1.AuthUtils.getErrorStatusResponseWithReason(res, errorCodeMap, "SIGN_UP_NOT_ALLOWED");
            }
