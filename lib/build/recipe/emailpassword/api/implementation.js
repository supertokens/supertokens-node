            ...
            try {
                ...
                if (mfaInstance === undefined) {
                    if (session === undefined) {
                        ...
                        // active session
                        let overwriteSessionDuringSignIn = recipe_4.default.getInstanceOrThrowError().config
                ...
                try {
                    ...
                    if (mfaInstance === undefined) {
                        ...
                        if (session === undefined) {
                            ...
                            await multifactorauth_1.default.checkAllowedToSetupFactorElseThrowInvalidClaimError(
                                session,
                                "emailpassword",
                                userContext
                            );
...
const attemptAccountLinking = async (tenantId, user, userContext) => {
...
const checkIfValidFirstFactor = async (tenantId, userContext) => {
    let isValid = utils_2.isValidFirstFactor(tenantId, "emailpassword", userContext);
    if (!isValid) {
        throw new error_1.default({
            type: error_1.default.UNAUTHORISED,
...
const checkIfSignUpIsAllowed = async (tenantId, email, userContext) => {
    let isSignUpAllowed = await recipe_1.default.getInstance().isSignUpAllowed({
        newUser: {
            recipeId: "emailpassword",
            email,
...
const checkIfFactorUserCanBeLinkedWithSessionUser = async (tenantId, sessionUser, accountInfo, userContext) => {
