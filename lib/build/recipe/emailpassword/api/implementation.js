...
const recipe_3 = __importDefault(require("../../emailverification/recipe"));
const recipeUserId_1 = __importDefault(require("../../../recipeUserId"));
const utils_1 = require("../utils");
const recipe_4 = __importDefault(require("../../multifactorauth/recipe"));
function getAPIImplementation() {
    return {
        ...
        generatePasswordResetTokenPOST: async function ({ formFields, tenantId, options, userContext }) {
            ...
            let shouldDoAccountLinkingResponse = await recipe_2.default
                .getInstance()
            ...
            if (emailPasswordAccount === undefined) {
                ...
                let isSignUpAllowed = await recipe_2.default.getInstance().isSignUpAllowed({
                    newUser: {
                        recipeId: "emailpassword",
                        email,
                    },
        ...
        passwordResetPOST: async function ({ formFields, token, tenantId, options, userContext }) {
            async function markEmailAsVerified(recipeUserId, email) {
                const emailVerificationInstance = recipe_3.default.getInstance();
            ...
            if (existingUser.isPrimaryUser) {
                ...
                if (emailPasswordUserIsLinkedToExistingUser) {
                    ...
                    if (createUserResponse.status === "EMAIL_ALREADY_EXISTS_ERROR") {
                        ...
                        // email is shared.
                        let linkedToUser = await recipe_2.default.getInstance().createPrimaryUserIdOrLinkAccounts({
                            tenantId,
        ...
        signInPOST: async function ({ formFields, tenantId, options, userContext }) {
            ...
            let isSignInAllowed = await recipe_2.default.getInstance().isSignInAllowed({
                user: response.user,
                tenantId,
                userContext,
            });
            ...
            response.user = await recipe_2.default.getInstance().createPrimaryUserIdOrLinkAccounts({
                tenantId,
                user: response.user,
                userContext,
            });
            const mfaInstance = recipe_4.default.getInstance();
            if (mfaInstance === undefined) {
                ...
                if (
                    session === undefined ||
                    recipe_1.default.getInstanceOrThrowError().config.overwriteSessionDuringSignIn
                ) {
            ...
            let session;
            session = await session_1.default.getSession(options.req, options.res, { sessionRequired: false });
        ...
        signUpPOST: async function ({ formFields, tenantId, options, userContext }) {
            ...
            if (!isSignUpAllowed) {
                const conflictingUsers = await recipe_2.default
                    .getInstance()
            ...
            const mfaInstance = recipe_4.default.getInstance();
            if (mfaInstance !== undefined) {
                let session = await session_1.default.getSession(options.req, options.res, { sessionRequired: false });
                const mfaValidationRes = await mfaInstance.recipeInterfaceImpl.validateForMultifactorAuthBeforeFactorCompletion(
                    {
                        req: options.req,
                        res: options.res,
