...
function generatePasswordResetToken(recipeInstance, req, res, _) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        // step 2.
        let user = yield recipeInstance.recipeInterfaceImpl.getUserByEmail(email);
        if (user === undefined) {
        ...
        let token;
        try {
            token = yield recipeInstance.recipeInterfaceImpl.createResetPasswordToken(user.id);
        } catch (err) {
        ...
        let passwordResetLink =
            (yield recipeInstance.config.resetPasswordUsingTokenFeature.getResetPasswordURL(user)) +
            "?token=" +
            token +
            "&rid=" +
