...
async function passwordReset(apiImplementation, tenantId, options, userContext) {
    ...
    let formFields = await utils_2.validateFormFieldsOrThrowError(options.config.resetPasswordUsingTokenFeature.formFieldsForPasswordResetForm, requestBody.formFields, tenantId, userContext);
    ...
    utils_1.send200Response(options.res, result.status === "OK"
        ? {
            status: "OK",
        }
        : result);
