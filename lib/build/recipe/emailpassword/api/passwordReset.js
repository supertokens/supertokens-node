...
async function passwordReset(apiImplementation, tenantId, options, userContext) {
    ...
    let formFields = await (0, utils_2.validateFormFieldsOrThrowError)(options.config.resetPasswordUsingTokenFeature.formFieldsForPasswordResetForm, requestBody.formFields, tenantId, userContext);
    ...
    (0, utils_1.send200Response)(options.res, result.status === "OK"
        ? {
            status: "OK",
        }
        : result);
