            ...
            if (Recipe.instance === undefined) {
                ...
                postSuperTokensInitCallbacks_1.PostSuperTokensInitCallbacks.addPostInitCallback(() => {
                    ...
                    if (mfaInstance !== undefined) {
                        ...
                        mfaInstance.addGetFactorsSetupForUserFromOtherRecipes(async (tenantId, user) => {
                            for (const loginMethod of user.loginMethods) {
                                ...
                                if (loginMethod.recipeId === Recipe.RECIPE_ID && loginMethod.tenantIds.includes(tenantId) && !utils_3.isFakeEmail(loginMethod.email)) {
                                    return ["emailpassword"];
                                }
                            }
                            return [];
                        ...
                        mfaInstance.addGetEmailsForFactorFromOtherRecipes((user, sessionRecipeUserId) => {
                            ...
                            for (let i = 0; i < orderedLoginMethodsByTimeJoinedOldestFirst.length; i++) {
                                ...
                                if (orderedLoginMethodsByTimeJoinedOldestFirst[i].recipeId === Recipe.RECIPE_ID && !utils_3.isFakeEmail(orderedLoginMethodsByTimeJoinedOldestFirst[i].email)) {
                            ...
                            if (nonFakeEmailsThatHaveEmailPasswordLoginMethodOrderedByTimeJoined.length === 0) {
                                ...
                                for (let i = 0; i < orderedLoginMethodsByTimeJoinedOldestFirst.length; i++) {
                                    if (orderedLoginMethodsByTimeJoinedOldestFirst[i].email !== undefined && !utils_3.isFakeEmail(orderedLoginMethodsByTimeJoinedOldestFirst[i].email)) {
