...
class Recipe extends recipeModule_1.default {
    constructor(recipeId, appInfo, isInServerlessEnv, config) {
        ...
        this.handleAPIRequest = (id, tenantId, req, res, __, ___, userContext) =>
            __awaiter(this, void 0, void 0, function* () {
                ...
                if (id === constants_1.DASHBOARD_API) {
                    return yield dashboard_1.default(this.apiImpl, options, userContext);
                }
                if (id === constants_1.SIGN_IN_API) {
                    return yield signIn_1.default(this.apiImpl, options, userContext);
                }
                if (id === constants_1.VALIDATE_KEY_API) {
                    return yield validateKey_1.default(this.apiImpl, options, userContext);
                }
                ...
                return yield apiKeyProtector_1.default(this.apiImpl, tenantId, options, apiFunction, userContext);
            });
        ...
        this.returnAPIIdIfCanHandleRequest = (path, method, userContext) =>
            __awaiter(this, void 0, void 0, function* () {
                ...
                let tenantId = constants_2.DEFAULT_TENANT_ID;
                ...
                if (
                    utils_1.isApiPath(path, this.getAppInfo().apiBasePath) ||
                    (remainingPath !== undefined &&
                        utils_1.isApiPath(
                            path,
                            this.getAppInfo().apiBasePath.appendPath(new normalisedURLPath_1.default(`/${tenantId}`))
                        ))
                ) {
                    ...
                    if (remainingPath !== undefined) {
                        const id = utils_1.getApiIdIfMatched(remainingPath, method);
                        if (id !== undefined) {
                            const finalTenantId = yield mtRecipe.recipeInterfaceImpl.getTenantId({
                                tenantIdFromFrontend: tenantId === undefined ? constants_2.DEFAULT_TENANT_ID : tenantId,
                                userContext,
                            });
                            return { id, tenantId: finalTenantId };
                        }
                    }
                    const id = utils_1.getApiIdIfMatched(path, method);
                    if (id !== undefined) {
                        const finalTenantId = yield mtRecipe.recipeInterfaceImpl.getTenantId({
                            tenantIdFromFrontend: constants_2.DEFAULT_TENANT_ID,
                            userContext,
                        });
                        return { id, tenantId: finalTenantId };
                    }
                ...
                if (path.startsWith(dashboardBundlePath)) {
                    return { id: constants_1.DASHBOARD_API, tenantId: constants_2.DEFAULT_TENANT_ID };
                }
