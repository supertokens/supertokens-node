...
async function usersGet(_, tenantId, options, userContext) {
    ...
    let usersResponse = timeJoinedOrder === "DESC"
        ? await __1.getUsersNewestFirst({
            tenantId,
            query,
            limit: parseInt(limit),
            paginationToken,
        })
        : await __1.getUsersOldestFirst({
            tenantId,
            query,
            limit: parseInt(limit),
            paginationToken,
    ...
    for (let i = 0; i < usersResponse.users.length; i++) {
        ...
        metaDataFetchPromises.push(() => new Promise(async (resolve, reject) => {
            try {
                const userMetaDataResponse = await usermetadata_1.default.getUserMetadata(userObj.id, userContext);
