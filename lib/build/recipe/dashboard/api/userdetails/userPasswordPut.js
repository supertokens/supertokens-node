...
const userPasswordPut = (_, options) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (recipeToUse === "emailpassword") {
        let passwordFormFields = recipe_1.default.getInstanceOrThrowError().config.signUpFeature.formFields.filter((field) => field.id === constants_1.FORM_FIELD_PASSWORD_ID);
        ...
        const passwordResetResponse = yield emailpassword_1.default.resetPasswordUsingToken(passwordResetToken.token, newPassword);
    ...
    let passwordFormFields = recipe_2.default.getInstanceOrThrowError().config.signUpFeature.formFields.filter((field) => field.id === constants_1.FORM_FIELD_PASSWORD_ID);
    let passwordValidationError = yield passwordFormFields[0].validate(newPassword);
    if (passwordValidationError !== undefined) {
        return {
            status: "INVALID_PASSWORD_ERROR",
    ...
    const passwordResetResponse = yield thirdpartyemailpassword_1.default.resetPasswordUsingToken(passwordResetToken.token, newPassword);
    if (passwordResetResponse.status === "RESET_PASSWORD_INVALID_TOKEN_ERROR") {
        throw new Error("Should never come here");
    }
    return {
