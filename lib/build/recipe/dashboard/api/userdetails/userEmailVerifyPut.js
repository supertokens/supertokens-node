...
const userEmailVerifyPut = async (_, tenantId, options, userContext) => {
    ...
    if (verified) {
        const tokenResponse = await emailverification_1.default.createEmailVerificationToken(tenantId, new recipeUserId_1.default(recipeUserId), undefined, userContext);
        if (tokenResponse.status === "EMAIL_ALREADY_VERIFIED_ERROR") {
            return {
                status: "OK",
            };
        }
        const verifyResponse = await emailverification_1.default.verifyEmailUsingToken(tenantId, tokenResponse.token, undefined, userContext);
        ...
        await emailverification_1.default.unverifyEmail(new recipeUserId_1.default(recipeUserId), undefined, userContext);
    }
    return {
        status: "OK",
    };
