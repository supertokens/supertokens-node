...
const userEmailVerifyTokenPost = async (_, tenantId, options, userContext) => {
    ...
    const user = await __1.getUser(recipeUserId, userContext);
    if (!user) {
        throw new error_1.default({
    ...
    return await emailverification_1.default.sendEmailVerificationEmail(tenantId, user.id, __1.convertToRecipeUserId(recipeUserId), undefined, userContext);
};
