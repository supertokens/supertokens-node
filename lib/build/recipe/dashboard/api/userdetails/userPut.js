...
const updateEmailForRecipeId = async (recipeId, recipeUserId, email, tenantId, userContext) => {
    ...
    if (recipeId === "webauthn") {
        let validationError = await recipe_3.default.getInstanceOrThrowError().config.validateEmailAddress(email, tenantId, userContext);
        if (validationError !== undefined) {
            return {
                status: "INVALID_EMAIL_ERROR",
                error: validationError,
...
const userPut = async (_, tenantId, options, userContext) => {
    ...
    let userResponse = await (0, utils_1.getUserForRecipeId)(new recipeUserId_1.default(recipeUserId), recipeId, userContext);
