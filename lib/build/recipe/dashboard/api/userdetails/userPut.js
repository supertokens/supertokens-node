...
const recipe_4 = __importDefault(require("../../../thirdpartypasswordless/recipe"));
...
const recipe_5 = __importDefault(require("../../../usermetadata/recipe"));
...
const updateEmailForRecipeId = async (recipeId, recipeUserId, email, tenantId, userContext) => {
    if (recipeId === "emailpassword") {
        let emailFormFields = recipe_1.default.getInstanceOrThrowError().config.signUpFeature.formFields.filter((field) => field.id === constants_1.FORM_FIELD_EMAIL_ID);
    ...
    if (recipeId === "thirdpartyemailpassword") {
        let emailFormFields = recipe_2.default.getInstanceOrThrowError().config.signUpFeature.formFields.filter((field) => field.id === constants_1.FORM_FIELD_EMAIL_ID);
        let validationError = await emailFormFields[0].validate(email, tenantId, userContext);
        if (validationError !== undefined) {
            return {
                status: "INVALID_EMAIL_ERROR",
        ...
        const emailUpdateResponse = await thirdpartyemailpassword_1.default.updateEmailOrPassword({
            recipeUserId,
            email,
            userContext,
        });
    ...
    if (recipeId === "passwordless") {
        ...
        if (passwordlessConfig.contactMethod === "PHONE") {
            const validationResult = await utils_2.defaultValidateEmail(email);
        ...
        if (updateResult.status === "EMAIL_CHANGE_NOT_ALLOWED_ERROR" ||
    ...
    if (recipeId === "thirdpartypasswordless") {
        ...
        const updateResult = await thirdpartypasswordless_1.default.updatePasswordlessUser({
            recipeUserId,
            email,
            userContext,
        });
...
const updatePhoneForRecipeId = async (recipeId, recipeUserId, phone, tenantId, userContext) => {
    if (recipeId === "passwordless") {
        let isValidPhone = true;
        let validationError = "";
        const passwordlessConfig = recipe_3.default.getInstanceOrThrowError().config;
        if (passwordlessConfig.contactMethod === "EMAIL") {
            const validationResult = await utils_2.defaultValidatePhoneNumber(phone);
            if (validationResult !== undefined) {
                isValidPhone = false;
                validationError = validationResult;
            }
...
const userPut = async (_, tenantId, options, userContext) => {
    ...
    let userResponse = await utils_1.getUserForRecipeId(new recipeUserId_1.default(recipeUserId), recipeId, userContext);
    ...
    if (email.trim() !== "") {
        const emailUpdateResponse = await updateEmailForRecipeId(userResponse.recipe, new recipeUserId_1.default(recipeUserId), email.trim(), tenantId, userContext);
    ...
    if (phone.trim() !== "") {
        const phoneUpdateResponse = await updatePhoneForRecipeId(userResponse.recipe, new recipeUserId_1.default(recipeUserId), phone.trim(), tenantId, userContext);
