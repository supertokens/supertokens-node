...
const userPut = async ({ stInstance, tenantId, options, userContext, }) => {
    ...
    let userResponse = await (0, utils_1.getUserForRecipeId)(stInstance, new recipeUserId_1.default(recipeUserId), recipeId, userContext);
    ...
    if (email.trim() !== "") {
        const emailUpdateResponse = await updateEmailForRecipeId(stInstance, userResponse.recipe, new recipeUserId_1.default(recipeUserId), email.trim(), tenantId, userContext);
    ...
    if (phone.trim() !== "") {
        const phoneUpdateResponse = await updatePhoneForRecipeId(stInstance, new recipeUserId_1.default(recipeUserId), phone.trim(), tenantId, userContext);
