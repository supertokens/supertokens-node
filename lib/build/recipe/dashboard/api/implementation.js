...
function getAPIImplementation(stInstance) {
    return {
        dashboardGET: async function (input) {
            ...
            const bundleDomain = new normalisedURLDomain_1.default(bundleBasePathString).getAsStringDangerous() +
            ...
            const cdiVersion = await querier_1.Querier.getNewInstanceOrThrowError(stInstance, input.options.recipeId).getAPIVersion(input.userContext);
