    ...
    if (enable === true) {
        ...
        if (!allAvailableSecondaryFactors.includes(factorId)) {
            return {
                ...
                message: `Please initialise ${utils_1.factorIdToRecipe(
                    factorId
                )} recipe to be able to use this login method`,
            };
        }
    }
    let secondaryFactors = utils_1.getNormalisedRequiredSecondaryFactorsBasedOnTenantConfigAndSDKInit(tenantRes);
    if (enable === true) {
