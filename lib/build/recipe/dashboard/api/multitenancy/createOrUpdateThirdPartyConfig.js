...
async function createOrUpdateThirdPartyConfig(_, tenantId, options, userContext) {
    ...
    if (tenantRes.thirdParty.providers.length === 0) {
        ...
        for (const provider of staticProviders.filter((provider) => provider.includeInNonPublicTenantsByDefault === true || tenantId === constants_1.DEFAULT_TENANT_ID)) {
            await multitenancy_1.default.createOrUpdateThirdPartyConfig(tenantId, {
        ...
        if (boxyAPIKey &&
            ...
            const requestBody = {
                ...
                encodedRawMetadata: providerConfig.clients[0].additionalConfig.samlXML
                    ? utils_1.encodeBase64(providerConfig.clients[0].additionalConfig.samlXML)
                    : "",
            ...
            const resp = await thirdpartyUtils_1.doPostRequest(normalisedDomain.getAsStringDangerous() +
    ...
    const thirdPartyRes = await multitenancy_1.default.createOrUpdateThirdPartyConfig(tenantId, providerConfig, undefined, userContext);
