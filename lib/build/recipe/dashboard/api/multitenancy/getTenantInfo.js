...
async function getTenantInfo(_, tenantId, options, userContext) {
    ...
    let firstFactors = utils_1.getNormalisedFirstFactorsBasedOnTenantConfigFromCoreAndSDKInit(tenantConfig);
    if (tenantRes === undefined) {
        return {
            status: "UNKNOWN_TENANT_ERROR",
        };
    }
    const userCount = await supertokens_1.default.getInstanceOrThrowError().getUserCount(undefined, tenantId, userContext);
    ...
    const mergedProvidersFromCoreAndStatic = configUtils_1.mergeProvidersFromCoreAndStatic(providersFromCore, staticProviders, tenantId === constants_1.DEFAULT_TENANT_ID);
    let querier = querier_1.Querier.getNewInstanceOrThrowError(options.recipeId);
    let coreConfig = await querier.sendGetRequest(new normalisedURLPath_1.default(`/${tenantId}/recipe/dashboard/tenant/core-config`), {}, userContext);
    const tenant = {
        tenantId,
        thirdParty: {
            providers: await Promise.all(mergedProvidersFromCoreAndStatic.map(async (provider) => {
                try {
                    const providerInstance = await configUtils_1.findAndCreateProviderInstance(mergedProvidersFromCoreAndStatic, provider.config.thirdPartyId, provider.config.clients[0].clientType, userContext);
