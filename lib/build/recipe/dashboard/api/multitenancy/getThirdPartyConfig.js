...
async function getThirdPartyConfig({ stInstance, tenantId, options, userContext, }) {
    var _a, _b, _c, _d, _e;
    ...
    let providersFromCore = (_a = tenantRes === null || tenantRes === void 0 ? void 0 : tenantRes.thirdParty) === null || _a === void 0 ? void 0 : _a.providers;
    ...
    if (thirdPartyId.startsWith("okta") ||
        ...
        if (additionalConfig !== undefined) {
            ...
            providersFromCore[0].clients = ((_b = providersFromCore[0].clients) !== null && _b !== void 0 ? _b : []).map((client) => (Object.assign(Object.assign({}, client), { additionalConfig: Object.assign(Object.assign({}, client.additionalConfig), additionalConfig) })));
        }
    }
        ...
        if (mergedProvider.config.thirdPartyId === thirdPartyId) {
            if (mergedProvider.config.clients === undefined || mergedProvider.config.clients.length === 0) {
                mergedProvider.config.clients = [
                    Object.assign({ clientId: "nonguessable-temporary-client-id" }, (additionalConfig !== undefined ? { additionalConfig } : {})),
                ];
            }
        }
    }
    ...
    for (const provider of mergedProvidersFromCoreAndStatic) {
        if (provider.config.thirdPartyId === thirdPartyId) {
            ...
            for (const client of (_d = provider.config.clients) !== null && _d !== void 0 ? _d : []) {
                try {
                    const providerInstance = await (0, configUtils_1.findAndCreateProviderInstance)(tenantId, mergedProvidersFromCoreAndStatic, thirdPartyId, client.clientType, userContext);
                    const _f = providerInstance.config, { clientId, clientSecret, clientType, scope, additionalConfig, forcePKCE } = _f, commonConfig = __rest(_f, ["clientId", "clientSecret", "clientType", "scope", "additionalConfig", "forcePKCE"]);
                    clients.push({
                        clientId,
                        clientSecret,
                        scope,
    ...
    if ((additionalConfig === null || additionalConfig === void 0 ? void 0 : additionalConfig.privateKey) !== undefined) {
    ...
    if (finalClients.length === 0) {
        finalClients.push(Object.assign(Object.assign(Object.assign({}, tempClients[0]), { clientId: "", clientSecret: "" }), (additionalConfig !== undefined ? { additionalConfig } : {})));
    }
    ...
    if (thirdPartyId.startsWith("boxy-saml")) {
        ...
        if (boxyAPIKey) {
            if (finalClients[0].clientId !== "") {
                ...
                const resp = await (0, thirdpartyUtils_1.doGetRequest)(normalisedDomain.getAsStringDangerous() +
                ...
                if (resp.status === 200) {
                    ...
                    finalClients[0].additionalConfig = Object.assign(Object.assign({}, finalClients[0].additionalConfig), { redirectURLs: resp.jsonResponse.redirectUrl, boxyTenant: resp.jsonResponse.tenant, boxyProduct: resp.jsonResponse.product });
                }
            }
        }
    }
    return {
        status: "OK",
        providerConfig: Object.assign(Object.assign({}, commonProviderConfig), { clients: finalClients, isGetAuthorisationRedirectUrlOverridden,
