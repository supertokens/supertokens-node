...
async function getThirdPartyConfig(_, tenantId, options, userContext) {
    ...
    let mergedProvidersFromCoreAndStatic = configUtils_1.mergeProvidersFromCoreAndStatic(providersFromCore, staticProviders, true);
    ...
    for (const provider of mergedProvidersFromCoreAndStatic) {
        if (provider.config.thirdPartyId === thirdPartyId) {
            ...
            for (const client of (_d = provider.config.clients) !== null && _d !== void 0 ? _d : []) {
                try {
                    const providerInstance = await configUtils_1.findAndCreateProviderInstance(mergedProvidersFromCoreAndStatic, thirdPartyId, client.clientType, userContext);
    ...
    if (thirdPartyId.startsWith("boxy-saml")) {
        ...
        if (boxyAPIKey) {
            if (finalClients[0].clientId !== "") {
                ...
                const resp = await thirdpartyUtils_1.doGetRequest(normalisedDomain.getAsStringDangerous() +
