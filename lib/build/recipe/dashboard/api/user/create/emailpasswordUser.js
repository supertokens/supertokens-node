...
const createEmailPasswordUser = async ({ stInstance, tenantId, options, userContext, }) => {
    ...
    if (response.status === "OK" || response.status === "EMAIL_ALREADY_EXISTS_ERROR") {
        ...
        throw new Error("This should never happen: EmailPassword.signUp threw a session user related error without passing a session");
    }
};
