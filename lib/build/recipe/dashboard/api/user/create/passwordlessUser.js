...
const createPasswordlessUser = async (_, tenantId, options, __) => {
    ...
    try {
        ...
        try {
            passwordlessOrThirdpartyPasswordlessRecipe = recipe_2.default.getInstanceOrThrowError();
        } catch (_) {
            return {
    ...
    if (
        ...
        if (passwordlessOrThirdpartyPasswordlessRecipe.config.validateEmailAddress !== undefined) {
            ...
            validationError = await utils_1.defaultValidateEmail(email);
        }
    ...
    if (
        ...
        if (passwordlessOrThirdpartyPasswordlessRecipe.config.validatePhoneNumber !== undefined) {
            ...
            validationError = await utils_1.defaultValidatePhoneNumber(phoneNumber);
        }
        if (validationError !== undefined) {
            return {
