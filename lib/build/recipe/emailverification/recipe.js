...
class Recipe extends recipeModule_1.default {
    constructor(recipeId, appInfo, isInServerlessEnv, config, ingredients) {
        ...
        this.updateSessionIfRequiredPostEmailVerification = async (input) => {
            ...
            if (input.session !== undefined) {
                ...
                if (input.session.getRecipeUserId(input.userContext).getAsString() ===
                    ...
                    (0, logger_1.logDebugMessage)("updateSessionIfRequiredPostEmailVerification the session belongs to the verified user");
                    ...
                    if (input.session.getUserId() === primaryUserId) {
                        (0, logger_1.logDebugMessage)("updateSessionIfRequiredPostEmailVerification the session userId matches the primary user id, so we are only refreshing the claim");
                        ...
                        (0, logger_1.logDebugMessage)("updateSessionIfRequiredPostEmailVerification the session user id doesn't match the primary user id, so we are revoking all sessions and creating a new one");
                    ...
                    (0, logger_1.logDebugMessage)("updateSessionIfRequiredPostEmailVerification the verified user doesn't match the session");
        ...
        {
            let builder = new supertokens_js_override_1.default((0, recipeImplementation_1.default)(querier_1.Querier.getNewInstanceOrThrowError(recipeId), this.getEmailForRecipeUserId));
