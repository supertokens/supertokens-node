    ...
    constructor(recipeId, appInfo, isInServerlessEnv, config, ingredients) {
        ...
        this.updateSessionIfRequiredPostEmailVerification = async (input) => {
            ...
            if (input.session !== undefined) {
                ...
                if (input.session.getRecipeUserId(input.userContext).getAsString() ===
                    ...
                    if (input.session.getUserId() === primaryUserId) {
                        ...
                        await session_1.default.revokeAllSessionsForUser(input.recipeUserIdWhoseEmailGotVerified.getAsString(), false, undefined, input.userContext);
                        // create a new session and return that..
                        return await session_1.default.createNewSession(input.req, input.res, input.session.getTenantId(), input.session.getRecipeUserId(input.userContext), {}, {}, input.userContext);
                    }
                }
                else {
