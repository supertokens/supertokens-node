    ...
    constructor(recipeId, appInfo, isInServerlessEnv, config, ingredients) {
        ...
        this.getAPIsHandled = () => {
            return [
                {
                    method: "post",
                    pathWithoutApiBasePath: new normalisedURLPath_1.default(constants_1.GENERATE_EMAIL_VERIFY_TOKEN_API),
        ...
        this.handleAPIRequest = async (id, tenantId, req, res, _, __, userContext) => {
            ...
            if (id === constants_1.GENERATE_EMAIL_VERIFY_TOKEN_API) {
                return await generateEmailVerifyToken_1.default(this.apiImpl, options, userContext);
            }
            else {
                return await emailVerify_1.default(this.apiImpl, tenantId, options, userContext);
            }
        };
        ...
        this.getEmailForRecipeUserId = async (user, recipeUserId, userContext) => {
            ...
            if (user === undefined) {
                user = await __1.getUser(recipeUserId.getAsString(), userContext);
                if (user === undefined) {
                    return {
        ...
        this.getPrimaryUserIdForRecipeUser = async (recipeUserId, userContext) => {
            ...
            let primaryUser = await __1.getUser(recipeUserId.getAsString(), userContext);
        ...
        this.updateSessionIfRequiredPostEmailVerification = async (input) => {
            let primaryUserId = await this.getPrimaryUserIdForRecipeUser(input.recipeUserIdWhoseEmailGotVerified, input.userContext);
            // if a session exists in the API, then we can update the session
            // claim related to email verification
            if (input.session !== undefined) {
                logger_1.logDebugMessage("updateSessionIfRequiredPostEmailVerification got session");
                ...
                if (input.session.getRecipeUserId().getAsString() === input.recipeUserIdWhoseEmailGotVerified.getAsString()) {
                    logger_1.logDebugMessage("updateSessionIfRequiredPostEmailVerification the session belongs to the verified user");
                    ...
                    if (input.session.getUserId() === primaryUserId) {
                        logger_1.logDebugMessage("updateSessionIfRequiredPostEmailVerification the session userId matches the primary user id, so we are only refreshing the claim");
                        ...
                        try {
                            ...
                            await input.session.fetchAndSetClaim(emailVerificationClaim_1.EmailVerificationClaim, input.userContext);
                        }
                        catch (err) {
                        ...
                        logger_1.logDebugMessage("updateSessionIfRequiredPostEmailVerification the session user id doesn't match the primary user id, so we are revoking all sessions and creating a new one");
                        ...
                        await session_1.default.revokeAllSessionsForUser(input.recipeUserIdWhoseEmailGotVerified.getAsString(), false, input.session.getTenantId(), input.userContext);
                        // create a new session and return that..
                        return await session_1.default.createNewSession(input.req, input.res, input.session.getTenantId(), input.session.getRecipeUserId(), {}, {}, input.userContext);
                    }
                }
                else {
                    logger_1.logDebugMessage("updateSessionIfRequiredPostEmailVerification the verified user doesn't match the session");
                ...
                logger_1.logDebugMessage("updateSessionIfRequiredPostEmailVerification got no session");
        ...
        {
            let builder = new supertokens_js_override_1.default(recipeImplementation_1.default(querier_1.Querier.getNewInstanceOrThrowError(recipeId), this.getEmailForRecipeUserId));
        ...
        return (appInfo, isInServerlessEnv) => {
            if (Recipe.instance === undefined) {
                ...
                postSuperTokensInitCallbacks_1.PostSuperTokensInitCallbacks.addPostInitCallback(() => {
                    recipe_1.default.getInstanceOrThrowError().addClaimFromOtherRecipe(emailVerificationClaim_1.EmailVerificationClaim);
                    if (config.mode === "REQUIRED") {
                        recipe_1.default.getInstanceOrThrowError().addClaimValidatorFromOtherRecipe(emailVerificationClaim_1.EmailVerificationClaim.validators.isVerified());
                    }
                });
                return Recipe.instance;
            }
            else {
                throw new Error("Emailverification recipe has already been initialised. Please check your code for bugs.");
            }
        };
    }
    static reset() {
