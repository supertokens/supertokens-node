...
const bootstrapService_1 = require("../../bootstrapService");
        ...
        this.getEmailForUserId = (userId, userContext) =>
            __awaiter(this, void 0, void 0, function* () {
                if (this.config.getEmailForUserId !== undefined) {
                    const email = yield this.config.getEmailForUserId(userId, userContext);
                    if (email !== undefined) {
                        return email;
                    }
                }
                for (const getEmailForUserId of this.getEmailForUserIdFuncsFromOtherRecipes) {
                    try {
                        const email = yield getEmailForUserId(userId, userContext);
                        return email;
                    } catch (_a) {
    ...
    static init(config) {
        return (appInfo, isInServerlessEnv) => {
            if (Recipe.instance === undefined) {
                ...
                bootstrapService_1.BootstrapService.addBootstrapCallback(() => {
                    if (config.mode !== "OFF") {
                        recipe_1.default.addClaimFromOtherRecipe(emailVerifiedClaim_1.EmailVerifiedClaim);
                    }
                    if (config.mode === "REQUIRED") {
                        recipe_1.default.addClaimValidatorFromOtherRecipe(
                            emailVerifiedClaim_1.EmailVerifiedClaim.validators.isValidated()
                        );
                    }
                });
                return Recipe.instance;
