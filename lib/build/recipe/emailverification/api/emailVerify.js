...
async function emailVerify(apiImplementation, tenantId, options, userContext) {
    ...
    if ((0, utils_1.normaliseHttpMethod)(options.req.getMethod()) === "post") {
        ...
        const session = await session_1.default.getSession(options.req, options.res, { overrideGlobalClaimValidators: () => [], sessionRequired: false }, userContext);
        ...
        const session = await session_1.default.getSession(options.req, options.res, { overrideGlobalClaimValidators: () => [] }, userContext);
