    ...
    return {
        ...
        generateEmailVerifyTokenPOST: function ({ options, userContext, }) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                let emailVerifyLink = (yield options.config.getEmailVerificationURL({ id: userId, email }, userContext)) +
                    "?token=" +
                    response.token +
                    "&rid=" +
                    options.recipeId;
                try {
                    if (!options.isInServerlessEnv) {
                        ...
                        yield options.config.createAndSendCustomEmail({ id: userId, email }, emailVerifyLink, userContext);
                    }
                }
                catch (_) { }
                return {
