...
function getAPIInterface() {
    return {
        verifyEmailPOST: async function ({ token, tenantId, session, options, userContext }) {
            ...
            // status: "OK"
            let newSession = await recipe_1.default.getInstanceOrThrowError().updateSessionIfRequiredPostEmailVerification({
                req: options.req,
                res: options.res,
                session,
        ...
        isEmailVerifiedGET: async function ({ userContext, session, options }) {
            ...
            const emailInfo = await recipe_1.default.getInstanceOrThrowError().getEmailForRecipeUserId(undefined, session.getRecipeUserId(userContext), userContext);
        ...
        generateEmailVerifyTokenPOST: async function ({ options, userContext, session }) {
            ...
            if (emailInfo.status === "EMAIL_DOES_NOT_EXIST_ERROR") {
                logger_1.logDebugMessage(`Email verification email not sent to user ${session
                ...
                // or if it belong to a recipe user ID that got deleted. Either way,
                // we logout the user.
                logger_1.logDebugMessage("generateEmailVerifyTokenPOST: Returning UNAUTHORISED because the user id provided is unknown");
