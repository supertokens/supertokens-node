...
function getRecipeInterface(stInstance, querier, getEmailForRecipeUserId) {
    return {
        ...
        isEmailVerified: async function ({ recipeUserId, email, userContext, }) {
            let response = await querier.sendGetRequest("/recipe/user/email/verify", {
                userId: recipeUserId.getAsString(),
                email,
            }, userContext);
        ...
        unverifyEmail: async function (input) {
            await querier.sendPostRequest("/recipe/user/email/verify/remove", {
                userId: input.recipeUserId.getAsString(),
                email: input.email,
            }, input.userContext);
            return { status: "OK" };
