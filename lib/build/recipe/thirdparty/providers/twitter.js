...
function Twitter(input) {
    ...
    input.config.userInfoMap = Object.assign({ fromUserInfoAPI: Object.assign({ userId: "data.id" }, (_a = input.config.userInfoMap) === null || _a === void 0 ? void 0 : _a.fromUserInfoAPI) }, input.config.userInfoMap);
    ...
    input.override = function (originalImplementation) {
        ...
        originalImplementation.exchangeAuthCodeForOAuthTokens = async function (input) {
            ...
            if ((0, custom_1.isUsingDevelopmentClientId)(originalImplementation.config.clientId)) {
                clientId = (0, custom_1.getActualClientIdFromDevelopmentClientId)(originalImplementation.config.clientId);
                redirectUri = custom_1.DEV_OAUTH_REDIRECT_URL;
            }
            /* Transformation needed for dev keys END */
            const basicAuthToken = (0, utils_1.encodeBase64)(`${clientId}:${originalImplementation.config.clientSecret}`);
            const twitterOauthTokenParams = Object.assign({ grant_type: "authorization_code", client_id: clientId, code_verifier: input.redirectURIInfo.pkceCodeVerifier, redirect_uri: redirectUri, code: input.redirectURIInfo.redirectURIQueryParams.code }, originalImplementation.config.tokenEndpointBodyParams);
            const tokenResponse = await (0, thirdpartyUtils_1.doPostRequest)(originalImplementation.config.tokenEndpoint, twitterOauthTokenParams, {
