...
function doGetRequest(url, queryParams, headers) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let response = yield utils_1.doFetch(finalURL.toString(), {
            headers: headers,
        });
        if (response.status >= 400) {
            logger_1.logDebugMessage(
                `Received response with status ${response.status} and body ${yield response.clone().text()}`
            );
            throw new Error(
                `Received response with status ${response.status} and body ${yield response.clone().text()}`
            );
        }
        const respData = yield response.clone().json();
        logger_1.logDebugMessage(
...
function doPostRequest(url, params, headers) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let response = yield utils_1.doFetch(url, {
            method: "POST",
            body,
            headers,
        });
