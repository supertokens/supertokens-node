...
function Bitbucket(input) {
    ...
    input.override = function (originalImplementation) {
        ...
        originalImplementation.getUserInfo = async function (input) {
            ...
            const userInfoFromAccessToken = await thirdpartyUtils_1.doGetRequest("https://api.bitbucket.org/2.0/user", undefined, headers);
            ...
            const userInfoFromEmail = await thirdpartyUtils_1.doGetRequest("https://api.bitbucket.org/2.0/user/emails", undefined, headers);
            if (userInfoFromEmail.status >= 400) {
                logger_1.logDebugMessage(`Received response with status ${userInfoFromEmail.status} and body ${userInfoFromEmail.stringResponse}`);
