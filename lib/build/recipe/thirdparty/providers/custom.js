...
function getSupertokensUserInfoResultFromRawUserInfo(config, rawUserInfoResponse) {
    ...
    if (((_d = (_c = config.userInfoMap) === null || _c === void 0 ? void 0 : _c.fromIdTokenPayload) === null || _d === void 0 ? void 0 : _d.userId) !== undefined) {
        const userId = accessField(rawUserInfoResponse.fromIdTokenPayload, config.userInfoMap.fromIdTokenPayload.userId);
        if (userId !== undefined) {
            thirdPartyUserId = userId;
        }
    }
    ...
    let email = "";
    if (((_f = (_e = config.userInfoMap) === null || _e === void 0 ? void 0 : _e.fromUserInfoAPI) === null || _f === void 0 ? void 0 : _f.email) !== undefined) {
        const emailVal = accessField(rawUserInfoResponse.fromUserInfoAPI, config.userInfoMap.fromUserInfoAPI.email);
        if (emailVal !== undefined) {
            email = emailVal;
        }
    }
    if (((_h = (_g = config.userInfoMap) === null || _g === void 0 ? void 0 : _g.fromIdTokenPayload) === null || _h === void 0 ? void 0 : _h.email) !== undefined) {
        const emailVal = accessField(rawUserInfoResponse.fromIdTokenPayload, config.userInfoMap.fromIdTokenPayload.email);
        if (emailVal !== undefined) {
            email = emailVal;
        }
    }
    if (email !== "") {
        ...
        if (((_k = (_j = config.userInfoMap) === null || _j === void 0 ? void 0 : _j.fromUserInfoAPI) === null || _k === void 0 ? void 0 : _k.emailVerified) !== undefined) {
            const emailVerifiedVal = accessField(rawUserInfoResponse.fromUserInfoAPI, config.userInfoMap.fromUserInfoAPI.emailVerified);
            result.email.isVerified =
                emailVerifiedVal === true ||
                    (typeof emailVerifiedVal === "string" && emailVerifiedVal.toLowerCase() === "true");
        }
        if (((_m = (_l = config.userInfoMap) === null || _l === void 0 ? void 0 : _l.fromIdTokenPayload) === null || _m === void 0 ? void 0 : _m.emailVerified) !== undefined) {
            const emailVerifiedVal = accessField(rawUserInfoResponse.fromIdTokenPayload, config.userInfoMap.fromIdTokenPayload.emailVerified);
            result.email.isVerified = emailVerifiedVal === true || emailVerifiedVal === "true";
        }
    }
    return result;
}
function NewProvider(input, type = "oauth2") {
    var _a, _b;
    // These are safe defaults common to most providers. Each provider implementations override these
    // as necessary
    input.config.userInfoMap = {
        fromIdTokenPayload: Object.assign({ userId: "sub", email: "email", emailVerified: "email_verified" }, (_a = input.config.userInfoMap) === null || _a === void 0 ? void 0 : _a.fromIdTokenPayload),
        fromUserInfoAPI: Object.assign({ userId: "sub", email: "email", emailVerified: "email_verified" }, (_b = input.config.userInfoMap) === null || _b === void 0 ? void 0 : _b.fromUserInfoAPI),
    };
    ...
    if (type === "oauth2") {
        impl = {
            ...
            getAuthorisationRedirectURL: async function ({ redirectURIOnProviderDashboard }) {
                ...
                const isS256MethodSupported = (_a = impl.config.codeChallengeMethodsSupported) === null || _a === void 0 ? void 0 : _a.includes("S256");
            ...
            exchangeAuthCodeForOAuthTokens: async function ({ redirectURIInfo }) {
                ...
                if (tokenResponse.status >= 400) {
                    (0, logger_1.logDebugMessage)(`Received response with status ${tokenResponse.status} and body ${tokenResponse.stringResponse}`);
                    throw new Error(`Received response with status ${tokenResponse.status} and body ${tokenResponse.stringResponse}`);
                }
            ...
            getUserInfo: async function ({ oAuthTokens, userContext }) {
                ...
                if (idToken && impl.config.jwksURI !== undefined) {
                    ...
                    rawUserInfoFromProvider.fromIdTokenPayload = await (0, thirdpartyUtils_1.verifyIdTokenFromJWKSEndpointAndGetPayload)(idToken, jwks, {
                ...
                if (accessToken && impl.config.userInfoEndpoint !== undefined) {
                    ...
                    const userInfoFromAccessToken = await (0, thirdpartyUtils_1.doGetRequest)(impl.config.userInfoEndpoint, queryParams, headers);
                    if (userInfoFromAccessToken.status >= 400) {
                        (0, logger_1.logDebugMessage)(`Received response with status ${userInfoFromAccessToken.status} and body ${userInfoFromAccessToken.stringResponse}`);
                        throw new Error(`Received response with status ${userInfoFromAccessToken.status} and body ${userInfoFromAccessToken.stringResponse}`);
                    }
        ...
        impl = {
            ...
            getAuthorisationRedirectURL: async function ({ tenantId, redirectURIOnProviderDashboard }) {
                const queryParams = {
                    client_id: impl.config.clientId,
                    redirect_uri: redirectURIOnProviderDashboard,
                };
                return {
                    urlWithQueryParams: appinfo.apiDomain.getAsStringDangerous() +
                        appinfo.apiBasePath
            ...
            getUserInfo: async function ({ tenantId, accessToken, userContext }) {
                const samlRecipe = recipe_1.default.getInstanceOrThrowError();
                const res = await samlRecipe.recipeInterfaceImpl.getUserInfo({
                    tenantId,
                    clientId: impl.config.clientId,
                    accessToken,
                    userContext: (0, utils_1.getUserContext)(userContext),
                });
                if (res.status !== "OK") {
