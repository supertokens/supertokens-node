    ...
    let impl = {
        ...
        exchangeAuthCodeForOAuthTokens: async function ({ redirectURIInfo }) {
            ...
            if (tokenResponse.status >= 400) {
                logger_1.logDebugMessage(`Received response with status ${tokenResponse.status} and body ${tokenResponse.stringResponse}`);
        ...
        getUserInfo: async function ({ oAuthTokens, userContext }) {
            ...
            if (idToken && impl.config.jwksURI !== undefined) {
                ...
                rawUserInfoFromProvider.fromIdTokenPayload = await utils_1.verifyIdTokenFromJWKSEndpointAndGetPayload(idToken, jwks, {
            ...
            if (accessToken && impl.config.userInfoEndpoint !== undefined) {
                ...
                const userInfoFromAccessToken = await utils_1.doGetRequest(impl.config.userInfoEndpoint, queryParams, headers);
                if (userInfoFromAccessToken.status >= 400) {
                    logger_1.logDebugMessage(`Received response with status ${userInfoFromAccessToken.status} and body ${userInfoFromAccessToken.stringResponse}`);
