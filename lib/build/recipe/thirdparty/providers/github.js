...
function Github(input) {
    ...
    if (input.config.validateAccessToken === undefined) {
        input.config.validateAccessToken = async ({ accessToken, clientConfig }) => {
            const basicAuthToken = (0, utils_1.encodeBase64)(`${clientConfig.clientId}:${clientConfig.clientSecret === undefined ? "" : clientConfig.clientSecret}`);
            const applicationResponse = await (0, thirdpartyUtils_1.doPostRequest)(`https://api.github.com/applications/${clientConfig.clientId}/token`, {
            ...
            if (applicationResponse.jsonResponse.app === undefined ||
    ...
    input.override = function (originalImplementation) {
        if (originalImplementation.type !== "oauth2") {
            throw new Error(`Invalid provider type for Github: ${originalImplementation.type}`);
        }
        ...
        originalImplementation.getUserInfo = async function (input) {
            ...
            const rawResponse = {};
            const emailInfoResp = await (0, thirdpartyUtils_1.doGetRequest)("https://api.github.com/user/emails", undefined, headers);
            if (emailInfoResp.status >= 400) {
                throw new Error(`Getting userInfo failed with ${emailInfoResp.status}: ${emailInfoResp.stringResponse}`);
            }
            rawResponse.emails = emailInfoResp.jsonResponse;
            const userInfoResp = await (0, thirdpartyUtils_1.doGetRequest)("https://api.github.com/user", undefined, headers);
