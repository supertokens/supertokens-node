...
function Github(input) {
    ...
    if (input.config.validateAccessToken === undefined) {
        input.config.validateAccessToken = async ({ accessToken, clientConfig }) => {
            const basicAuthToken = utils_1.encodeBase64(
                `${clientConfig.clientId}:${clientConfig.clientSecret === undefined ? "" : clientConfig.clientSecret}`
            );
            const applicationResponse = await utils_2.doPostRequest(
    ...
    input.override = function (originalImplementation) {
        ...
        originalImplementation.getUserInfo = async function (input) {
            ...
            const rawResponse = {};
            const emailInfoResp = await utils_2.doGetRequest("https://api.github.com/user/emails", undefined, headers);
            ...
            const userInfoResp = await utils_2.doGetRequest("https://api.github.com/user", undefined, headers);
