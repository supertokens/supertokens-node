...
function getRecipeImplementation(querier, providers) {
    return {
        manuallyCreateOrUpdateUser: async function ({ thirdPartyId, thirdPartyUserId, email, isVerified, tenantId, shouldAttemptAccountLinkingIfAllowed, userContext, }) {
        ...
        signInUp: async function ({ thirdPartyId, thirdPartyUserId, email, isVerified, tenantId, userContext, oAuthTokens, rawUserInfoFromProvider, shouldAttemptAccountLinkingIfAllowed, }) {
        ...
        getProvider: async function ({ thirdPartyId, tenantId, clientType, userContext }) {
            ...
            const mergedProviders = configUtils_1.mergeProvidersFromCoreAndStatic(tenantConfig.thirdParty.providers, providers);
            const provider = await configUtils_1.findAndCreateProviderInstance(mergedProviders, thirdPartyId, clientType, userContext);
            return provider;
        },
    };
}
