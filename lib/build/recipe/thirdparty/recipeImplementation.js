...
function getRecipeImplementation(querier, providers) {
    return {
        manuallyCreateOrUpdateUser: async function ({ thirdPartyId, thirdPartyUserId, email, isVerified, tenantId, session, shouldTryLinkingWithSessionUser, userContext, }) {
            ...
            const users = await (0, __1.listUsersByAccountInfo)(tenantId, { thirdParty: { id: thirdPartyId, userId: thirdPartyUserId } }, false, userContext);
            const user = users[0];
            if (user !== undefined) {
        ...
        getProvider: async function ({ thirdPartyId, tenantId, clientType, userContext }) {
            ...
            const provider = await (0, configUtils_1.findAndCreateProviderInstance)(mergedProviders, thirdPartyId, clientType, userContext);
            return provider;
        },
    };
}
