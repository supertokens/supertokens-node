...
function getAPIInterface() {
    return {
        linkAccountToExistingAccountPOST: function ({
            provider,
            code,
            redirectURI,
            authCodeResponse,
            ...
            return __awaiter(this, void 0, void 0, function* () {
                ...
                let recipeUser = user.loginMethods.find(
                    (u) =>
                        ...
                        u.thirdParty !== undefined &&
                        u.thirdParty.id === provider.id &&
                        u.email === email
                );
                if (recipeUser === undefined) {
                    throw Error(
                        "this error should never be thrown. Can't find thirdparty recipeUser with email: " +
                            email +
                            "  and primary userId: " +
                            session.getUserId()
                    );
        ...
        signInUpPOST: function ({ provider, code, redirectURI, authCodeResponse, options, userContext }) {
            return __awaiter(this, void 0, void 0, function* () {
                 ...
                 */
                let usersBasedOnTPInfo = yield __1.listUsersByAccountInfo({
                    thirdpartyId: provider.id,
                    thirdpartyUserId: userInfo.id,
                });
                if (usersBasedOnTPInfo !== undefined) {
                    ...
                    if (primaryUserBasedOnTPInfo !== undefined) {
                        let usersBasedOnEmailInfo = yield __1.listUsersByAccountInfo({
                            thirdpartyId: provider.id,
                            thirdpartyUserId: userInfo.id,
                        });
