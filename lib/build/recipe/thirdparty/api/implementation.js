...
function getAPIInterface(stInstance) {
    return {
        ...
        signInUpPOST: async function (input) {
            ...
            const userInfo = provider.type === "oauth2"
            ...
            if (preAuthChecks.status !== "OK") {
                (0, logger_1.logDebugMessage)("signInUpPOST: erroring out because preAuthChecks returned " + preAuthChecks.status);
            ...
            if (response.status !== "OK") {
                (0, logger_1.logDebugMessage)("signInUpPOST: erroring out because signInUp returned " + response.status);
            ...
            if (postAuthChecks.status !== "OK") {
                (0, logger_1.logDebugMessage)("signInUpPOST: erroring out because postAuthChecks returned " + postAuthChecks.status);
