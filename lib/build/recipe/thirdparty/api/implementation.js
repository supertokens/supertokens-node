    ...
    return {
        ...
        signInUpPOST: function ({ provider, code, redirectURI, authCodeResponse, options, userContext, }) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                let session = yield session_1.default.createNewSession(options.req, options.res, response.user.id, response.user.recipeUserId, {}, {}, userContext);
                return {
                    status: "OK",
        ...
        appleRedirectHandlerPOST: function ({ code, state, options }) {
            return __awaiter(this, void 0, void 0, function* () {
                const redirectURL = options.appInfo.websiteDomain.getAsStringDangerous() +
                    options.appInfo.websiteBasePath.getAsStringDangerous() +
                    "/callback/apple?state=" +
                    state +
                    "&code=" +
                    code;
                options.res.sendHTMLResponse(`<html><head><script>window.location.replace("${redirectURL}");</script></head></html>`);
            });
        },
    };
}
