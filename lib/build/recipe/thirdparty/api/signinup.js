...
async function signInUpAPI(apiImplementation, tenantId, options, userContext) {
    ...
    const shouldTryLinkingWithSessionUser = utils_1.getNormalisedShouldTryLinkingWithSessionUserFlag(options.req, bodyParams);
    ...
    if (result.status === "OK") {
        utils_1.send200Response(options.res, Object.assign({ status: result.status }, utils_1.getBackwardsCompatibleUserInfo(options.req, result, userContext)));
    }
    else {
