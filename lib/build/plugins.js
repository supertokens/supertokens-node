...
function loadPlugins({ plugins, config, normalisedAppInfo, }) {
    ...
    for (const plugin of inputPluginList) {
        ...
        if (!versionContraints.includes(version_1.version)) {
            // TODO: better checks
            throw new Error(`Plugin version mismatch. Version ${version_1.version} not found in compatible versions: ${versionContraints.join(", ")}`);
        }
    ...
    for (const [pluginIndex, plugin] of finalPluginList.entries()) {
        if (plugin.config) {
            // @ts-ignore
            const _a = plugin.config((0, utils_1.getPublicConfig)(Object.assign(Object.assign({}, _config), { appInfo: normalisedAppInfo }))), { appInfo } = _a, pluginConfigOverride = __rest(_a, ["appInfo"]);
