...
function getPluginDependencies({ plugin, config, pluginsAbove, sdkVersion, normalisedAppInfo, }) {
    const publicConfig = (0, utils_1.getPublicConfig)(Object.assign(Object.assign({}, config), { appInfo: normalisedAppInfo }));
...
function applyPlugins(recipeId, config, plugins) {
    ...
    if (recipeId !== "accountlinking" && apiLayers.length > 0) {
        config.override = Object.assign(Object.assign({}, config.override), { apis: (oI, builder) => {
    ...
    if (functionLayers.length > 0) {
        config.override = Object.assign(Object.assign({}, config.override), { functions: (oI, builder) => {
...
function loadPlugins({ plugins, config, normalisedAppInfo, }) {
    ...
    for (const plugin of inputPluginList) {
        ...
        if (plugin.compatibleSDKVersions) {
            const versionCheck = (0, versionChecker_1.isVersionCompatible)(version_1.version, plugin.compatibleSDKVersions);
            if (!versionCheck) {
                throw new Error(`Incompatible SDK version for plugin ${plugin.id}. Version "${version_1.version}" not found in compatible versions: ${JSON.stringify(plugin.compatibleSDKVersions)}`);
            }
        }
    ...
    const duplicatePluginIds = finalPluginList.filter((plugin, index) => finalPluginList.some((elem, idx) => elem.id === plugin.id && idx !== index));
    ...
    for (const [pluginIndex, plugin] of finalPluginList.entries()) {
        if (plugin.config) {
            // @ts-ignore
            const _a = plugin.config((0, utils_1.getPublicConfig)(Object.assign(Object.assign({}, _config), { appInfo: normalisedAppInfo }))), { appInfo, recipeList, experimental } = _a, pluginConfigOverride = __rest(_a, ["appInfo", "recipeList", "experimental"]);
            _config = Object.assign(Object.assign({}, _config), pluginConfigOverride);
        }
        const publicConfig = (0, utils_1.getPublicConfig)(Object.assign(Object.assign({}, _config), { appInfo: normalisedAppInfo }));
        if (plugin.routeHandlers) {
            ...
            if (typeof plugin.routeHandlers === "function") {
                ...
                handlers = result.routeHandlers.map((handler) => (Object.assign(Object.assign({}, handler), { pluginId: plugin.id })));
            }
            else {
                handlers = plugin.routeHandlers.map((handler) => (Object.assign(Object.assign({}, handler), { pluginId: plugin.id })));
            }
