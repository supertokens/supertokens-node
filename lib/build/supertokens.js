...
class SuperTokens {
    constructor(config) {
        ...
        this.getUserCount = async (includeRecipeIds, tenantId, userContext) => {
            ...
            let response = await querier.sendGetRequest(new normalisedURLPath_1.default(`/${tenantId === undefined ? constants_2.DEFAULT_TENANT_ID : tenantId}/users/count`), {
        ...
        this.updateOrDeleteUserIdMappingInfo = async function (input) {
            ...
            let cdiVersion = await querier.getAPIVersion();
            if (utils_1.maxVersion("2.15", cdiVersion) === cdiVersion) {
                return await querier.sendPutRequest(new normalisedURLPath_1.default("/recipe/userid/external-user-id-info"), {
                    userId: input.userId,
                    userIdType: input.userIdType,
                    externalUserIdInfo: input.externalUserIdInfo,
                }, input.userContext);
        ...
        this.middleware = async (request, response, userContext) => {
            ...
            if (requestRID !== undefined) {
                ...
                let requestHandled = await matchedRecipe.handleAPIRequest(idResult.id, idResult.tenantId, request, response, path, method, userContext);
                if (!requestHandled) {
                ...
                for (let i = 0; i < this.recipeModules.length; i++) {
                    ...
                    if (idResult !== undefined) {
                        logger_1.logDebugMessage("middleware: Request being handled by recipe. ID is: " + idResult.id);
                        let requestHandled = await this.recipeModules[i].handleAPIRequest(idResult.id, idResult.tenantId, request, response, path, method, userContext);
                        if (!requestHandled) {
                            logger_1.logDebugMessage("middleware: Not handled because API returned requestHandled as false");
                            return false;
                        }
        ...
        const originToPrint = config.appInfo.origin === undefined
            ...
            : typeof config.appInfo.origin === "string"
                ...
                : "function";
        logger_1.logDebugMessage("appInfo: " +
        ...
        }), (_b = config.supertokens) === null || _b === void 0 ? void 0 : _b.apiKey, (_c = config.supertokens) === null || _c === void 0 ? void 0 : _c.networkInterceptor);
