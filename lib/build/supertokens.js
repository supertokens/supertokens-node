...
class SuperTokens {
    constructor(config) {
        ...
        const inputPluginList =
            (_b = (_a = config.experimental) === null || _a === void 0 ? void 0 : _a.plugins) !== null && _b !== void 0
                ? _b
                : [];
        ...
        for (const [pluginIndex, plugin] of finalPluginList.entries()) {
            if (plugin.config) {
                config = Object.assign(Object.assign({}, config), plugin.config(config));
            }
            if (plugin.routeHandlers) {
                let handlers = [];
                if (typeof plugin.routeHandlers === "function") {
                    const result = plugin.routeHandlers(config, this.pluginList, version_1.version);
                    if (result.status === "ERROR") {
                        throw new Error(result.message);
                    }
            ...
            if (plugin.init) {
                postSuperTokensInitCallbacks_1.PostSuperTokensInitCallbacks.addPostInitCallback(() => {
                    plugin.init(config, this.pluginList, version_1.version);
