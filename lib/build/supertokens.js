...
class SuperTokens {
    constructor(config) {
        ...
        for (let pluginIndex = 0; pluginIndex < this.pluginList.length; pluginIndex += 1) {
            ...
            if (pluginConfig) {
                config = Object.assign(Object.assign({}, config), pluginConfig(config));
            }
            const pluginRouteHandlers = finalPluginList[pluginIndex].routeHandlers;
            if (pluginRouteHandlers) {
                const result = pluginRouteHandlers(config, this.pluginList, version_1.version);
                if (result.status === "ERROR") {
                    throw new Error(result.message);
                }
            ...
            if (pluginInit) {
                postSuperTokensInitCallbacks_1.PostSuperTokensInitCallbacks.addPostInitCallback(() => {
                    pluginInit(config, this.pluginList, version_1.version);
