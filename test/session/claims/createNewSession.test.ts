...
describe(`sessionClaims/createNewSession: ${printPath('[test/session/claims/createNewSession.test.ts]')}`, () => {
  beforeEach(async () => {
    await killAllST()
    await setupST()
  ...
  describe('createNewSession', () => {
    it('should create access token payload w/ session claims', async () => {
      await startST()
    ...
    it('should create access token payload wo/ session claims with an undefined value', async () => {
      await startST()
      SuperTokens.init({
        supertokens: {
          connectionURI: 'http://localhost:8080',
    ...
    it('should merge claims and the passed access token payload obj', async () => {
      await startST()
