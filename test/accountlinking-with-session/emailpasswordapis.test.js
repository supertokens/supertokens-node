...
describe(`emailpassword accountlinkingTests w/ session: ${printPath(
        ...
        describe("during sign up", () => {
            describe("linking without email verification requirements", () => {
                it("should link to session user if the session user is already primary", async () => {
                    ...
                    const resp = await signUpPOST(app, email2, true, session);
    ...
    //                 const session = await getSessionForUser(sessionUser);
    //                 const resp = await signInPOST(app, email2, true, session);
    //                 assert.strictEqual(resp.status, 200);
    //                 assert.ok(resp.body);
    ...
    //             const session = await getSessionForUser(sessionUser);
    //             const resp = await signInPOST(app, email2, true, session, undefined, "error");
...
async function signUpPOST(app, email, isVerified, session, password = testPassword) {
    return post(
        app,
        "/auth/signup",
        {
...
async function signInPOST(app, email, isVerified, session, password = testPassword) {
    return post(
        app,
        "/auth/signin",
        {
