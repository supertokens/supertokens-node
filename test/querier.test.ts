...
describe(`Querier: ${printPath('[test/querier.test.ts]')}`, () => {
  ...
  it('test that if that once API version is there, it doesn\'t need to query again', async () => {
    await startST()
    ST.init({
      supertokens: {
  ...
  it('test that rid is added to the header if it\'s a recipe request', async () => {
    await startST()
    ST.init({
      supertokens: {
        connectionURI: 'http://localhost:8080',
    ...
    const querier = Querier.getNewInstanceOrThrowError(SessionRecipe.getInstanceOrThrowError().getRecipeId())
    ...
    const response = await querier.sendGetRequest(new NormalisedURLPath('/recipe'), {})
    ...
    const response2 = await querier.sendGetRequest(new NormalisedURLPath('/recipe/random'), {})
    ...
    const response3 = await querier.sendGetRequest(new NormalisedURLPath('/test'), {})
  ...
  it('three cores and round robin', async () => {
    await startST()
  ...
  it('three cores, one dead and round robin', async () => {
    await startST()
    await startST('localhost', 8082)
    ST.init({
      supertokens: {
  ...
  it('test that no connectionURI given, but recipe used throws an error', async () => {
    await startST()
    ST.init({
      appInfo: {
        apiDomain: 'api.supertokens.io',
  ...
  it('test that no connectionURI given, recipe override and used doesn\'t thrown an error', async () => {
    await startST()
    ST.init({
      appInfo: {
        apiDomain: 'api.supertokens.io',
  ...
  it('test with core base path', async () => {
    ...
    const res = await Session.getAllSessionHandlesForUser('user1')
    assert(res.length === 0)
  })

  it('test with incorrect core base path should fail', async () => {
    // first we need to know if the core used supports base_path config
  ...
  it('test with multiple core base path', async () => {
    // first we need to know if the core used supports base_path config
    await setKeyValueInConfig('base_path', '/some/path')
    await startST()
