...
describe(`configTest: ${printPath("[test/config.test.js]")}`, function () {
    ...
    it("test using zero, one and two recipe modules", async function () {
...

        {
            ...
            assert.strictEqual(SuperTokens.getInstanceOrThrowError().recipeModules.length, 7); // accountlinking,openid,jwt,multitenancy,usermetadata&oauth2provider is initialised by default
            resetAll();
        }

        {
            ...
            assert(SuperTokens.getInstanceOrThrowError().recipeModules.length === 8); // accountlinking,openid,jwt,multitenancy,usermetadata&oauth2provider is initialised by default
            resetAll();
        }
    });
