...
import { SupertokensService, TwilioService } from 'supertokens-node/recipe/thirdpartypasswordless/smsdelivery'
import express from 'express'
import { afterAll, beforeEach, describe, it } from 'vitest'
import { cleanST, delay, isCDIVersionCompatible, killAllST, printPath, setupST, startST } from '../utils'

describe(`smsDelivery: ${printPath('[test/thirdpartypasswordless/smsDelivery.test.ts]')}`, () => {
  beforeEach(async () => {
  ...
  it('test twilio service: passwordless login', async () => {
    await startST()
    let phoneNumber
    let codeLifetime
    let userInputCode
  ...
  it('test supertokens service: passwordless login', async () => {
    await startST()
    let phoneNumber
    let codeLifetime
    let userInputCode
  ...
  it('test default backward compatibility api being called, error message not sent back to user if response code is 429: passwordless login', async () => {
    await startST()
    STExpress.init({
      supertokens: {
        connectionURI: 'http://localhost:8080',
  ...
  it('test twilio service: resend code api', async () => {
    await startST()
    let phoneNumber
    let codeLifetime
    let userInputCode
  ...
  it('test supertokens service: resend code api', async () => {
    await startST()
    let phoneNumber
    let codeLifetime
    let userInputCode
  ...
  it('test default backward compatibility api being called, error message not sent back to user if response code is 429: resend code api', async () => {
    await startST()
    STExpress.init({
      supertokens: {
        connectionURI: 'http://localhost:8080',
