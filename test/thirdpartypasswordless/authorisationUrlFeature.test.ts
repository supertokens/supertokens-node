...
import { TypeProvider } from 'supertokens-node/recipe/thirdpartypasswordless'
...
describe(`authorisationTest: ${printPath('[test/thirdpartyemailpassword/authorisationFeature.test.ts]')}`, () => {
  ...
  it('test with thirdPartyPasswordless, minimum config for thirdparty module', async () => {
    await startST()
    STExpress.init({
      supertokens: {
  ...
  it('test with thirdPartyPasswordless, thirdparty provider doesn\'t exist', async () => {
    await startST()
    STExpress.init({
      supertokens: {
        connectionURI: 'http://localhost:8080',
