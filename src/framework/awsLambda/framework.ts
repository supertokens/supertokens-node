...
export class AWSRequest extends BaseRequest {
  ...
  getKeyValueFromQuery = (key: string): string | undefined => {
    ...
    const value = this.event.queryStringParameters[key]
  ...
  getMethod = (): HTTPMethod => {
    const rawMethod = (this.event as APIGatewayProxyEvent).httpMethod
...
export class AWSResponse extends BaseResponse {
  ...
  setCookie = (
    ...
    const serialisedCookie = serializeCookieValue(key, value, domain, secure, httpOnly, expires, path, sameSite)
  ...
  sendResponse = (response?: APIGatewayProxyResult | APIGatewayProxyStructuredResultV2) => {
    ...
    const supertokensHeaders = this.event.supertokens.response.headers
    const supertokensCookies = this.event.supertokens.response.cookies
    for (let i = 0; i < supertokensHeaders.length; i++) {
      let currentValue
      const currentHeadersSet = Object.keys(headers === undefined ? [] : headers)
      ...
      if (supertokensHeaders[i].allowDuplicateKey && currentValue !== undefined) {
        const newValue = `${currentValue}, ${supertokensHeaders[i].value}`
    ...
    if ((this.event as APIGatewayProxyEventV2).version !== undefined) {
      ...
      const headsersInMultiValueHeaders = Object.keys(multiValueHeaders)
