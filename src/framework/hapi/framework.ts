...
import { getCookieValueFromHeaders, normalizeHeaderValue } from '../utils'
...
const plugin: Plugin<{}> = {
  ...
  async register(server, _) {
    ...
    server.ext('onPreResponse', async (request, h) => {
      ...
      if ((request.response as Boom).isBoom) {
        const err = (request.response as Boom).data || request.response
        const req = new HapiRequest(request)
        const res = new HapiResponse(h as ExtendedResponseToolkit)
        if (err !== undefined && err !== null) {
          try {
            await supertokens.errorHandler(err, req, res)
            if (res.responseSet) {
              const resObj = res.sendResponse(true);
              (((request.app as any).lazyHeaders || []) as {
                key: string
                value: string
                allowDuplicateKey: boolean
    ...
    server.decorate('toolkit', 'lazyHeaderBindings', (
      h: ResponseToolkit,
      key: string,
      value: string | undefined,
      allowDuplicateKey: boolean,
    ...
    const supportedRoutes: ServerRoute[] = []
    const routeMethodSet = new Set<string>()
    for (let i = 0; i < supertokens.recipeModules.length; i++) {
      const apisHandled = supertokens.recipeModules[i].getAPIsHandled()
      for (let j = 0; j < apisHandled.length; j++) {
        const api = apisHandled[j]
        if (!api.disabled) {
          const path = `${supertokens.appInfo.apiBasePath.getAsStringDangerous()}${api.pathWithoutApiBasePath.getAsStringDangerous()}`
          const methodAndPath = `${api.method}-${path}`
