...
import type { Middleware, MiddlewareContext, Request, Response } from '@loopback/rest'
...
export const middleware: Middleware = async (ctx: MiddlewareContext, next: Next) => {
  ...
  const request = new LoopbackRequest(ctx)
  const response = new LoopbackResponse(ctx)
  try {
