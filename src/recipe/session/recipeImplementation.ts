import { getRidFromHeader, isAnIpAddress, normaliseHttpMethod } from '../../utils'
...
export default function getRecipeInterface(
  ...
  async function getHandshakeInfo(forceRefetch = false): Promise<HandshakeInfo> {
    if (handshakeInfo === undefined || handshakeInfo.getJwtSigningPublicKeyList().length === 0 || forceRefetch) {
      const antiCsrf = config.antiCsrf
      ProcessState.getInstance().addState(PROCESS_STATE.CALLING_SERVICE_IN_GET_HANDSHAKE_INFO)
      const response = await querier.sendPostRequest(new NormalisedURLPath('/recipe/handshake'), {})
  ...
  const obj: RecipeInterface = {
    ...
    async getGlobalClaimValidators(input: {
    ...
    async validateClaimsInJWTPayload(
      this: RecipeInterface,
      input: {
        userId: string
