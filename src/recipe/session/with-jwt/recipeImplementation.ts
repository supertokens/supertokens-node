...
export default function (
  ...
  async function jwtAwareUpdateAccessTokenPayload(
    ...
    const accessTokenPayload = sessionInformation.accessTokenPayload
    ...
    const existingJwt = accessTokenPayload[existingJwtPropertyName]
    assert.notStrictEqual(existingJwt, undefined)

    const currentTimeInSeconds = Date.now() / 1000
    const decodedPayload = decode(existingJwt, { json: true })
  ...
  return {
    ...
    async createNewSession(
      ...
      const accessTokenValidityInSeconds = Math.ceil((await this.getAccessTokenLifeTimeMS({ userContext })) / 1000)
      ...
      const sessionContainer = await originalImplementation.createNewSession({
        req,
        res,
        userId,
        accessTokenPayload,
    ...
    async getSession(
      ...
      const sessionContainer = await originalImplementation.getSession({ req, res, options, userContext })
    ...
    async refreshSession(
      ...
      const newSession = await originalImplementation.refreshSession({ req, res, userContext })
    ...
    async mergeIntoAccessTokenPayload(
      ...
      const newAccessTokenPayload = { ...sessionInformation.accessTokenPayload, ...accessTokenPayloadUpdate }
