...
export function normaliseSessionScopeOrThrowError(sessionScope: string): string {
  ...
  const noDotNormalised = helper(sessionScope)
...
export function validateAndNormaliseUserInput(
  ...
  const antiCsrf: 'VIA_TOKEN' | 'VIA_CUSTOM_HEADER' | 'NONE'
  ...
  if (config !== undefined && config.jwt !== undefined && config.jwt.enable === true) {
    enableJWT = true
    const jwtPropertyName = config.jwt.propertyNameInAccessTokenPayload
