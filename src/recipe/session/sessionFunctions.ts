...
export async function createNewSession(
  ...
  const handShakeInfo = await helpers.getHandshakeInfo()
  ...
  for (const key of handShakeInfo.getJwtSigningPublicKeyList()) {
    try {
      ...
      /**
             ...
             */
      const payload = parsedAccessToken.payload
...
export async function getSessionInformation(
  ...
  const response = await helpers.querier.sendGetRequest(new NormalisedURLPath('/recipe/session'), {
    sessionHandle,
  })
...
export async function refreshSession(
  ...
  const response = await helpers.querier.sendPostRequest(new NormalisedURLPath('/recipe/session/refresh'), requestBody)
  if (response.status === 'OK') {
    delete response.status
    return response
  }
...
export async function revokeAllSessionsForUser(helpers: Helpers, userId: string): Promise<string[]> {
  const response = await helpers.querier.sendPostRequest(new NormalisedURLPath('/recipe/session/remove'), {
    userId,
  })
...
export async function getAllSessionHandlesForUser(helpers: Helpers, userId: string): Promise<string[]> {
  const response = await helpers.querier.sendGetRequest(new NormalisedURLPath('/recipe/session/user'), {
    userId,
  })
...
export async function updateSessionData(
  ...
  const response = await helpers.querier.sendPutRequest(new NormalisedURLPath('/recipe/session/data'), {
    sessionHandle,
...
export async function updateAccessTokenPayload(
  ...
  const response = await helpers.querier.sendPutRequest(new NormalisedURLPath('/recipe/jwt/data'), {
    sessionHandle,
