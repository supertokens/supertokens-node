...
> => {
  if (recipeId === 'emailpassword') {
    const emailFormFields = EmailPasswordRecipe.getInstanceOrThrowError().config.signUpFeature.formFields.filter(
      field => field.id === FORM_FIELD_EMAIL_ID,
    )

    const validationError = await emailFormFields[0].validate(email)
  ...
  if (recipeId === 'thirdpartyemailpassword') {
    const emailFormFields = ThirdPartyEmailPasswordRecipe.getInstanceOrThrowError().config.signUpFeature.formFields.filter(
      field => field.id === FORM_FIELD_EMAIL_ID,
    )
...
export const userPut = async (_: APIInterface, options: APIOptions): Promise<Response> => {
  ...
  const userResponse = await getUserForRecipeId(userId, recipeId)
