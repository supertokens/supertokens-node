...
export const userEmailVerifyTokenPost = async (_: APIInterface, options: APIOptions): Promise<Response> => {
  ...
  const emailResponse = await EmailVerificationRecipe.getInstanceOrThrowError().getEmailForUserId(userId, {})

  if (emailResponse.status !== 'OK')
    throw new Error('Should never come here')

  const emailVerificationToken = await EmailVerification.createEmailVerificationToken(userId)
  ...
  const emailVerifyLink = getEmailVerifyLink({
