...
export const userPasswordPut = async (_: APIInterface, options: APIOptions): Promise<Response> => {
  ...
  if (recipeToUse === 'emailpassword') {
    const passwordFormFields = EmailPasswordRecipe.getInstanceOrThrowError().config.signUpFeature.formFields.filter(
      field => field.id === FORM_FIELD_PASSWORD_ID,
    )

    const passwordValidationError = await passwordFormFields[0].validate(newPassword)
  ...
  const passwordFormFields = ThirdPartyEmailPasswordRecipe.getInstanceOrThrowError().config.signUpFeature.formFields.filter(
    field => field.id === FORM_FIELD_PASSWORD_ID,
  )
