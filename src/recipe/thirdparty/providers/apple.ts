...
interface TypeThirdPartyProviderAppleConfig {
  clientId: string
  clientSecret: {
    keyId: string
    privateKey: string
...
export default function Apple(config: TypeThirdPartyProviderAppleConfig): TypeProvider {
  ...
  function get(redirectURI: string | undefined, authCodeFromRequest: string | undefined): TypeProviderGetResponse {
    const accessTokenAPIURL = 'https://appleid.apple.com/auth/token'
    ...
    const authorisationRedirectURL = 'https://appleid.apple.com/auth/authorize'
    ...
    async function getProfileInfo(accessTokenAPIResponse: {
      ...
      const isVerified = (payload as any).email_verified
