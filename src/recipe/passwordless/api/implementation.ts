...
export default function getAPIImplementation(): APIInterface {
  return {
    async consumeCodePOST(input) {
      const response = await input.options.recipeImplementation.consumeCode(
        'deviceId' in input
          ? {
    ...
    async createCodePOST(input) {
      const response = await input.options.recipeImplementation.createCode(
        'email' in input
          ? {
              userContext: input.userContext,
              email: input.email,
    ...
    async emailExistsGET(input) {
      const response = await input.options.recipeImplementation.getUserByEmail({
        userContext: input.userContext,
        email: input.email,
      })
    ...
    async phoneNumberExistsGET(input) {
      const response = await input.options.recipeImplementation.getUserByPhoneNumber({
        userContext: input.userContext,
        phoneNumber: input.phoneNumber,
      })
    ...
    async resendCodePOST(input) {
      const deviceInfo = await input.options.recipeImplementation.listCodesByDeviceId({
        userContext: input.userContext,
        deviceId: input.deviceId,
      })
      ...
      while (true) {
        ...
        const response = await input.options.recipeImplementation.createNewCodeForDevice({
          userContext: input.userContext,
          deviceId: input.deviceId,
          userInputCode:
                        input.options.config.getCustomUserInputCode === undefined
