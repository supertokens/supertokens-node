...
export default class SuperTokens {
  ...
  getAllCORSHeaders = (): string[] => {
    ...
    this.recipeModules.forEach((recipe) => {
      const headers = recipe.getAllCORSHeaders()
      headers.forEach((h) => {
        headerSet.add(h)
      })
    })
  ...
  getUserCount = async (includeRecipeIds?: string[]): Promise<number> => {
    ...
    const response = await querier.sendGetRequest(new NormalisedURLPath('/users/count'), {
  ...
  > {
    ...
    if (maxVersion('2.15', cdiVersion) === cdiVersion) {
      // create userId mapping is only available >= CDI 2.15
      const response = await querier.sendGetRequest(new NormalisedURLPath('/recipe/userid/map'), {
        userId: input.userId,
  ...
  middleware = async (request: BaseRequest, response: BaseResponse): Promise<boolean> => {
    ...
    const path = this.appInfo.apiGatewayPath.appendPath(new NormalisedURLPath(request.getOriginalURL()))
    const method: HTTPMethod = normaliseHttpMethod(request.getMethod())
    ...
    if (requestRID !== undefined) {
      let matchedRecipe: RecipeModule | undefined
      ...
      const id = matchedRecipe.returnAPIIdIfCanHandleRequest(path, method)
      if (id === undefined) {
        logDebugMessage(
      ...
      const requestHandled = await matchedRecipe.handleAPIRequest(id, request, response, path, method)
      if (!requestHandled) {
      ...
      for (let i = 0; i < this.recipeModules.length; i++) {
        logDebugMessage(`middleware: Checking recipe ID for match: ${this.recipeModules[i].getRecipeId()}`)
        const id = this.recipeModules[i].returnAPIIdIfCanHandleRequest(path, method)
        if (id !== undefined) {
          logDebugMessage(`middleware: Request being handled by recipe. ID is: ${id}`)
          const requestHandled = await this.recipeModules[i].handleAPIRequest(
            id,
            request,
            response,
            path,
