...
### Fixed
- webauthn: fix sign in failing for counter-incrementing authenticators (e.g. Windows Hello) — the assertion was verified against the core twice ([supertokens-core#1195](https://github.com/supertokens/supertokens-core/issues/1195)). Adds `completeSignIn` to the recipe interface; `signInPOST` no longer calls `signIn`, so move `signIn` override logic for the API flow to `completeSignIn`. Masked sign-in errors are now debug-logged
