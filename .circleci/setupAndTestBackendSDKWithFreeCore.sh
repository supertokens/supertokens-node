...
echo "$TEST_FILES" | circleci tests run --command="xargs npx mocha --reporter mocha-multi --node-option no-experimental-fetch -r test/fetch-polyfill.mjs --timeout 500000 --no-config" --verbose --split-by=timings
